...
############################### 指数 ###############################
    # 查询指数行情
    MARKET_HQ = UrlCfg('http://ams.howbuy.test/server/fund/brinson/zsjy', 'post',
                       lambda x: {
                           'zqdm': x['zqdm'],
                           'jyrq': {
...
################################ 公募 ################################
    # 公募基金回报
    FUND_JJHB = UrlCfg('http://ams.howbuy.test/server/fund/brinson/jjhb', 'post',
                       lambda x: {
                           'jjdm': x['jjdm'],
                           'jzrq': {
                               'startDate': x['startDate'],
    ...
    '''公募基金回报'''
    # 公募基金公募持仓数据
    FUND_JJ_GPZH = UrlCfg('http://ams.howbuy.test/server/fund/brinson/gpzh', 'post',
                          lambda x: {
                              'jjdm': x['jjdm'],
                              'ggrq': {
                                  'startDate': x['startDate'],
    ...
    '''公募基金公募持仓数据'''
    # 公募持仓资产比例数据
    FUND_JJ_ZCPZ = UrlCfg('http://ams.howbuy.test/server/fund/brinson/zcpz', 'post',
                          lambda x: {
                              'jjdm': x['jjdm'],
                              'jsrq': {
                                  'startDate': x['startDate'],
    ...
    # 指数成分股数据
    MARKET_CFG = UrlCfg('http://ams.howbuy.test/server/fund/brinson/lc_icw', 'post',
                        lambda x: {
                            'secuCode': x['secuCode'],
                            'endDate': x['endDate'],
                            'fields': x['fields'],
    ...
    # 公私募净值归因 - Barra风格因子数据
    BARRA_FACTORY = UrlCfg('http://ams.howbuy.test/server/fund/jzgy/barra_factor_return', 'post',
                                lambda x: {
                                    'tradeDate': {
                                        'startDate': x['startDate'],
                                        'endDate': x['endDate']
    ...
    #  全量指数行情数据
    MARKET_HQQL = UrlCfg('http://ams.howbuy.test/server/fund/jzgy/hqql', 'post',
                         lambda x: {
                             'zqdm': x['zqdm'],
                             'jyrq': {
                                 'startDate': x['startDate'],
    ...
    #  公私募净值归因 - 板块指数数据
    SECTOR_FACTOR = UrlCfg('http://ams.howbuy.test/server/fund/jzgy/sector_factor', 'post',
                           lambda x: {
                               'tradeDate': {
                                   'startDate': x['startDate'],
                                   'endDate': x['endDate']
    ...
    #  私募基金回报数据
    SIMU_RHB = UrlCfg('http://ams.howbuy.test/server/fund/jzgy/rhb', 'post',
                      lambda x: {
                          'jjdm': x['jjdm'],
                          'jzrq': {
                              'startDate': x['startDate'],
    ...
    ''' 私募基金回报数据 '''
    # A股市场估值数据/科创板估值数据
    SC_VALUATION = UrlCfg('http://ams.howbuy.test/server/fund/sac/dfv_stibdifv', 'post',
                          lambda x: {
                              'tradingDay': {
                                  'startDate': x['startDate'],
                                  'endDate': x['endDate']
    ...
    SC_FQ = UrlCfg('http://ams.howbuy.test/server/fund/brinson/pd_stibpd', 'post',
                          lambda x: {
                              'tradingDay': {
                                  'startDate': x['startDate'],
                                  'endDate': x['endDate']
    ...
    SC_CHDQUOTE =  UrlCfg('http://192.168.152.89:8081/server/fund/brinson/chdquote', 'post',
                          lambda x: {
                              'symbol': x['symbol'],
                              'tdate': {
                                  'startDate': x['startDate'],
