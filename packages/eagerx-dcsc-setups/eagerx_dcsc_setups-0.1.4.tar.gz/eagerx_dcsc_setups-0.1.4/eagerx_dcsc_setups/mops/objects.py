...
class Mops(Object):
    ...
    def agnostic(spec: AgnosticSpec):
        ...
        spec.sensors.mops_output.rate = '$(config mops_rate)'
        spec.sensors.mops_output.converter = Processor.make('AngleDecomposition', angle_idx=0)
        spec.sensors.mops_output.space_converter = SpaceConverter.make('Space_Float32MultiArray', low=[-1, -1, -9], high=[1, 1, 9], dtype='float32')

        spec.sensors.action_applied.rate = '$(config mops_rate)'
        spec.sensors.action_applied.space_converter = SpaceConverter.make('Space_Float32MultiArray', low=[-3], high=[3], dtype='float32')

        spec.sensors.image.rate = 15
        spec.sensors.image.space_converter = SpaceConverter.make('Space_Image', low=0, high=1, shape='(default render_shape)', dtype='float32')

        # Set actuator properties: (space_converters, rate, etc...)
        spec.actuators.mops_input.rate = '$(config mops_rate)'
        spec.actuators.mops_input.window = 1
        spec.actuators.mops_input.space_converter = SpaceConverter.make('Space_Float32MultiArray', low=[-3], high=[3], dtype='float32')

        # Set model_state properties: (space_converters)
        spec.states.model_state.space_converter = SpaceConverter.make('Space_Float32MultiArray', low=[-3.14159265359, -9], high=[3.14159265359, 9], dtype='float32')
        ...
        spec.states.model_parameters.space_converter = SpaceConverter.make('Space_Float32MultiArray', low=low, high=high, dtype='float32')

    @staticmethod
    @register.spec('Mops', Object)
    def spec(spec: ObjectSpec, name: str, sensors=None, states=None,
             mops_rate=30, always_render=False, render_shape=None, camera_index=2):
        """Object spec of Mops"""
    ...
    def ode_bridge(cls, spec: SpecificSpec, graph: EngineGraph):
        ...
        spec.config.ode = "eagerx_dcsc_setups.mops.ode.mops_ode/mops_ode"
        # Set default params of mops ode [J, m, l, b0, K, R, c, a].
        spec.config.ode_params = [0.000189238, 0.0563641, 0.0437891, 0.000142205, 0.0502769, 9.83536, 1.49553, 0.00183742]

        # Create simstates (no agnostic states defined in this case)
        spec.states.model_state = EngineState.make('OdeSimState')
        spec.states.model_parameters = EngineState.make('OdeParameters', list(range(7)))
        ...
        image = EngineNode.make('MopsImage', 'image', shape='$(config render_shape)', rate=None, process=0)
    ...
    def real_bridge(cls, spec: SpecificSpec, graph: EngineGraph):
        ...
        # Set object arguments (nothing to set here in this case)
        spec.config.driver_launch_file = "$(find dcsc_setups)/launch/mops.launch"

        # Couple engine states
        spec.states.model_state = EngineState.make('RandomActionAndSleep', sleep_time=1.0, repeat=1)

        # Create sensor engine nodes
        # Rate=None, because we will connect them to sensors (thus uses the rate set in the agnostic specification)
        obs = EngineNode.make('MopsOutput', 'mops_output', rate=None, process=0)
        applied = EngineNode.make('ActionApplied', 'applied', rate=None, process=0)
        image = EngineNode.make('CameraRender', 'image', camera_idx='$(config opencv_cam_index)', shape='$(config render_shape)', rate=None, process=0)
