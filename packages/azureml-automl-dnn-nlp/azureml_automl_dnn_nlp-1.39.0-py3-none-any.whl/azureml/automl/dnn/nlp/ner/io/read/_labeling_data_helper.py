...
from azureml.automl.dnn.nlp.common._data_utils import (
    download_text_files_from_path_column, get_dataset_by_id
)
...
def load_dataset_for_labeling_service(
    ...
    """
    _logger.info(f"Loading {data_label.value} dataset for labeling service")
    dataset = get_dataset_by_id(dataset_id, data_label, workspace)
    download_text_files_from_path_column(dataset, data_dir, data_label)
    ...
    try:
        ...
        _logger.warning(f"Error while processing file {processed_input_file_counter} "
                        f"of {len(text_df)} for {data_label.value} dataset.")
        raise DataException._with_error(
            AzureMLError.create(
                LabelingDataConversionFailed, error_details=str(e), target=f"load_{data_label.value}_dataset")
        )

    _logger.info(f"Finished processing all {len(text_df)} files for {data_label.value} dataset.")
