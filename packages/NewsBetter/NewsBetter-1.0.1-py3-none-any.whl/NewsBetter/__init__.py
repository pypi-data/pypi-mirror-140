...
class NewsbetterApp:
    ...
    def toggle_summary(self):
        if self.summary == 0:
            self.summary = 1
            source = self.article_list.get_title()
            self.article_list.set_title(source + " (summary on)")
        elif self.summary == 1:
            self.summary = 0
            source = self.article_list.get_title()
            toggle_text = source.replace("(summary on)", "")
            self.article_list.set_title(toggle_text)
