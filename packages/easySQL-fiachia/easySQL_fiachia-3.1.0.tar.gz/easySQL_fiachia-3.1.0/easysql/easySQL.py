...
class DataSql:
        ...
        if value:
            ...
            if self.attribute:
                self.__data_list.append(
                    {self.attribute[i]: value[i] for i in range(len(self.attribute))}
                )
            else:
                self.__data_list.append(
                    {"attr_%s" % i: value[i] for i in range(len(value))}
                )
    ...
    def add_values(self, values):
        if values:
            ...
            if self.attribute:
                len_temp = len(self.attribute)
                self.__data_list.extend(
                    list(
                        map(lambda x: {self.attribute[i]: x[i] for i in range(len_temp)}, values)
                    )
                )
            else:
                len_temp = len(values[0])
                self.__data_list.extend(
                    list(
                        map(lambda x: {"attr_%s" % i: x[i] for i in range(len_temp)}, values)
                    )
                )
...
class EasySql:
    ...
    def add_with_same_attr(self, table_name, *args, values=None):
        if table_name and args and values:
            _insert_language_temp_0 = """INSERT INTO `%s` (`%s`) VALUES (""" % (
                table_name,
                "`,`".join(args)
            )
    ...
    def add_sql(self, *sql_list: str):
        if sql_list:
            self.sql_list.extend(sql_list)
            if len(self.sql_list) > 100000:
                self.thread_dic[self.thread_index] = self.commit_with_thread()
                self.thread_index += 1
    ...
    def commit_with_thread(self, max_num_each_sql=5):
        """
        使用多线程批量执行已储存事务（sql语句不冲突才可使用）
        :param max_num_each_sql: [num]每个事务最大线程数
        """
        print_warning(
            "已创建线程连接数据库，根据数据量和网络这可能花费较长时间，如需等待执行请调用返回的线程表，该模式不返回错误列表，请直接查看输出。"
        )
        result_temp_list = []
        for thread_sql in _chunks(self.sql_list, max_num_each_sql):
            thread_temp = MysqlThread(self.thread_index, self.database, thread_sql)
            thread_temp.start()
            result_temp_list.append(thread_temp)
        self.sql_list = []
...
if __name__ == "__main__":
    db = EasySql(_database_name=None, _passwd="Fiachia123+")
    db.add_with_same_attr("test", "1", "2", values=[(1,2),(3,4)])
    pass
