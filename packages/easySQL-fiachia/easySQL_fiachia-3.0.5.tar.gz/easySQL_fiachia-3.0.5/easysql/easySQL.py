...
class DataSql:
        ...
        if isinstance(data_dataSql, DataSql):
            if self.database == data_dataSql.database and self.table == data_dataSql.table:
                if self.find_info and data_dataSql.find_info:
                    ...
                    raise OverflowError("对拥有全数据的数据表无需使用add，或许您想使用的是update")
            else:
    ...
    def first(self):
        if self.value_list:
            return DataSql(
                ...
                _table_name=self.table,
                _attribute=self.attribute,
                _key_list=self.pri_key_list,
                _datas=self.value_list[0],
    ...
    def update(self, **kwargs):
        if self.find_info:
            self.find_info = _find_info_update(self.find_info, **kwargs)
            table_data_find_sql = """select * from `%s` where%s"""
            cur = self.database.connect().cursor()
            cur.execute(table_data_find_sql % (
                self.table, _find_info("AND", p1=self.find_info_pri_key)))
...
class ResultThread(threading.Thread):
    def __init__(self, threadID, thread_dict, error_list):
        threading.Thread.__init__(self)
        self.threadID = threadID
    ...
    def run(self) -> None:
        for t_key, t_thread in self.thread_dict.items():
            t_thread.join()
            self.error_list.extend(t_thread.get_error)
...
class EasySql:
    ...
    def index(self, value):
        if isinstance(value, int):
            self.__index = value % 16
        else:
            raise TypeError("index must be int.")

    def next_sql(self):
        for i in range(16):
            if self.sql_list_stats[i] == "using":
                return i
        print_warning("缓存区已满，等待语句执行")
        self.commit_with_thread(max_num_each_sql=20, model="MAX")
        num_start = _print_running(max_num=40)
        while True:
            time.sleep(3)
            print_warning("\r正在等待", next(num_start), sep="", end="")
            if self.sql_list_stats[0] == "using":
                print_warning()
                return 0

    @property
    def sql_list(self):
        return self.sql_list_dict[self.index]

    @sql_list.setter
    def sql_list(self, value):
        if value is None:
            self.sql_list_dict = {
                i: [] for i in range(16)
            }
            self.sql_list_stats = {
                i: "using" for i in range(16)
            }
            self.index = 0
        elif isinstance(value, (tuple, list)):
            if value:
                self.sql_list_dict[self.index].extend(value)
                if len(self.sql_list_dict[self.index]) > 1000000:
                    self.sql_list_stats[self.index] = "max"
                    self.index = self.next_sql()
                else:
                    self.index = self.next_sql()
                    self.sql_list = value
            else:
                self.sql_list_stats[self.index] = "using"
                self.sql_list_dict[self.index] = []
        else:
            print_error("sql_list is a list.")
    ...
    def delete(self, table_name, is_commit=True, data_dataSql=None, **kwargs):
        """
        删除数据
    ...
    def find(self, table_name, **kwargs):
        ...
        if kwargs:
            ...
            return DataSql(
                self.database, table_name, self.cursor, self.get_data(
                    _find_language), find_info=_find_info_read(find_info))
        else:
    ...
    def find_by_sql_str(self, table_name, sql_str=""):
        if sql_str:
            return DataSql(
                self.database,
                table_name,
                self.cursor,
                self.get_data("""SELECT * FROM `%s` WHERE %s""" % (table_name, sql_str)),
                find_info=sql_str
            )
        else:
            return DataSql(
    ...
    def update(self, table_name="", is_commit=True, find_info=None, **update_datas):
        """
        :param table_name:
        :param is_commit:
        :param find_info:
        ...
        if find_info is None:
            return self.add(table_name, is_commit=is_commit, **update_datas)
        else:
    ...
    def add_sql(self, *sql_list: str):
        if sql_list:
            self.sql_list_dict[self.index].extend(sql_list)
            if len(self.sql_list_dict[self.index]) > 1000000:
                self.sql_list_stats[self.index] = "max"
                self.index = self.next_sql()
    ...
    def show_sql(self, index: int = None):
        if index is None:
            for i in range(len(self.sql_list_dict[self.index])):
                print_data("%s: %s" % (i, self.sql_list_dict[self.index]))
        else:
            print_data("%s: %s" % (index, self.sql_list_dict[self.index][index]))

    def delete_sql(self, index: int = None):
        if index is None:
            self.sql_list_dict[self.index] = list()
        else:
            self.sql_list_dict[self.index].pop(index)
    ...
    def commit(self):
        ...
        """
        try:
            for sql in self.sql_list_dict[self.index]:
                self.cursor.execute(sql)
            self.db.commit()
            self.sql_list_dict[self.index] = list()
            return
    ...
    def commit_with_thread(self, max_num_each_sql=5, model="ALL"):
        """
        使用多线程批量执行已储存事务（sql语句不冲突才可使用）
        :param max_num_each_sql: [num]每个事务最大线程数
        :param model: ["ALL"]/["MAX"]/["THIS"]
        """
        print_warning("正在创建线程连接数据库，根据数据量和网络这可能花费较长时间。")
        if model == "ALL":
            thread_dict = dict()
            thread_id = 1
            for sql_index, sql_stats in self.sql_list_stats.items():
                if sql_stats == "running":
                    pass
                else:
                    self.sql_list_stats[sql_index] = "running"
                    for thread_sql in _chunks(self.sql_list_dict[sql_index], max_num_each_sql):
                        thread_dict[thread_id] = MysqlThread(thread_id, self.database, thread_sql)
                        thread_dict[thread_id].start()
                        thread_id += 1
                    self.sql_list_stats[sql_index] = "using"
                    self.sql_list_dict[sql_index] = []
            for threadId, threadRun in thread_dict.items():
                threadRun.join()
                self.error_list.extend(threadRun.get_error())
        elif model == "MAX":
            thread_dict = dict()
            thread_id = 1
            for sql_index, sql_stats in self.sql_list_stats.items():
                if sql_stats == "running":
                    ...
                    self.sql_list_stats[sql_index] = "using"
                    self.sql_list_dict[sql_index] = []
            result_thread = ResultThread(233, thread_dict, self.error_list)
            result_thread.start()
        elif model == "THIS":
            thread_dict = dict()
            thread_id = 1
            if self.sql_list_stats[self.index] != "running":
                self.sql_list_stats[self.index] = "running"
                for thread_sql in _chunks(self.sql_list_dict[self.index], max_num_each_sql):
                    thread_dict[thread_id] = MysqlThread(thread_id, self.database, thread_sql)
                    thread_dict[thread_id].start()
                    thread_id += 1
                self.sql_list_stats[self.index] = "using"
...
def _find_info_update(find_info_list=None, **find_info_kwargs):
    if find_info_list:
        if isinstance(find_info_list, (tuple, list)):
