...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            args = get_required_args(payload, args)
            kwargs = get_optional_args(payload, kwargs)
    
            res = f(self.d, *[args[e] for e in args], **kwargs)
            res = json.dumps(res)
            res = res.encode()
        except Exception as err:
            self.LOG(4, 0, label='WSGIServer', label2='Handler EXCEPTION', msg=str(err))
            res = {'error': str(err)}
