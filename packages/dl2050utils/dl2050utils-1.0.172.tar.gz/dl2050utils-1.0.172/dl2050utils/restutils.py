...
def get_optional_args(payload, args):
    for e in payload:
        if e not in args:
            raise HTTPException(400, detail=f'Invalid optional arg {e}')
    return {e:payload[e] for e in args if e in payload and payload[e] is not None}
...
def get_required(payload, args):
    ...
    for e in args:
        if e not in payload or payload[e] is None:
            raise HTTPException(400, detail=f'Missing required arg {e}')
        args.append(payload[e])
    return args

# def get_args(d, attrs):
#     return {e:d[e] for e in listify(attrs) if e in d and d[e] is not None}

# def check_valid(d, attrs):
#     invalid = []
#     for e in listify(d):
#         if e not in attrs: invalid.append(e)
#     if not len(invalid): return False
#     return invalid

# def check_required(d, attrs):
#     miss = []
#     for e in listify(attrs):
#         if e not in d or d[e] is None: miss.append(e)
#     if not len(miss): return False
#     return miss
