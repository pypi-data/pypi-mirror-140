...
def EPDiff(euler,omega,list=None):
    ...
    if (list == 'list') or (list == None):
        EPr1 = (w3 * euler[1]) + (-w2 * euler[2]) + (w1 * euler[3])
        EPr2 = (-w3 * euler[0]) + (w1 * euler[2]) + (w2 * euler[3])
        EPr3 = (w2 * euler[0]) + (-w1 * euler[1]) + (w3 * euler[3])
        EPr4 = (-w1 * euler[0]) + (-w2 * euler[1]) + (-w3 * euler[2])
