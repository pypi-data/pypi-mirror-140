...
class VQALightningModule(pl.LightningModule):
    ...
    def accuracy(self, logits, batch, dataset):
        ...
        score_per_example = {"overall": [], "yes/no": [], "number": [], "other": []}
        for ans, scores, ans_type in zip(answers, batch["scores"], batch["answer_type"]):
            score = scores.get(ans, 0.0)
            score_per_example["overall"].append(score)
            score_per_example[ans_type].append(score)

        accuracy = {
                key: mean(score_list) if len(score_list) else 0.0
                for key, score_list in score_per_example.items()
            }
