...
class BasicTokenizer:
    ...
    def from_pretrained(cls, name, dir_data=None):
        ...
        if not os.path.exists(os.path.join(dir_data, "tokenizers", name)):
            download(url, os.path.join(dir_data, "tokenizers"), basename=name)
...
class AnswerMap:
    ...
    base_url = "https://webia.lip6.fr/~dancette/multimodal/answermaps/{name}.json"

    def __init__(self, answers) -> None:
        self.answers = answers
        self.ans2id = {ans: id for (id, ans) in enumerate(self.answers)}

    def __getitem__(self, index):
        return self.answers[index]

    def get_answer_id(self, answer):
        return self.ans2id[answer]

    @classmethod
    def from_pretrained(cls, name, dir_data=None):
        dir_data = dir_data or DEFAULT_DATA_DIR
        url = cls.base_url.format(name=name)
        path = download(url, os.path.join(dir_data, "answermaps"))
        with open(path) as f:
            answers = json.load(f)
        return AnswerMap(answers=answers)
