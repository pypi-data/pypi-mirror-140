...
class VQA(AbstractVQA):
    ...
    def __init__(
        ...
        self.path_annotations_processed = {
            split: os.path.join(self.processed_dirs[split], "annotations.pickle")
        ...
        if self.features is not None and type(features) == str:
            self._load_features()
        elif self.features is not None and isinstance(self.features, object):
    ...
    def _load_processed_annotations(self, split):
        with open(self.path_annotations_processed[split], "rb") as f:
    ...
    def _process_annotations(self):
        ...
        """
        paths = [
            self.path_annotations_processed[split] for split in self.url_annotations
        ]
        ...
        if any(not os.path.exists(p) for p in paths):
            annotations = [
                self._load_original_annotations(split) for split in self.url_annotations
            ]
            ...
            for i, split in enumerate(self.url_annotations):
                print(
                    f"Saving processed annotations for split {split} at path {self.path_annotations_processed[split]}"
                )
                with open(self.path_annotations_processed[split], "wb") as f:
                    # json.dump(annotations[i], f)
                    pickle.dump(annotations[i], f)
        ...
        #####################################
        annotations = [
            self._load_processed_annotations(split) for split in self.url_annotations
        ]
        ...
        if not os.path.exists(self.path_answers):
            if "multiple_choice_answer" in annotations[0][0]:
                ...
                occ = Counter(
                    annot["multiple_choice_answer"] for annot in all_annotations
                )
                ...
                self.answers = list(
                    set(ans["answer"] for a in all_annotations for ans in a["answers"])
                )
    ...
    def _load(self):
        ...
        if self.has_annotations:
            with open(self.path_annotations_processed[self.split], "rb") as f:
    ...
    def evaluate(self, predictions, agg=True):
        """
...
class VQACE(VQA2):
    ...
    """
    VQA-CE (VQA-Counterexamples) from paper 
    Beyond Question-Based Biases: Assessing Multimodal Shortcut Learning in Visual Question Answering
    at https://arxiv.org/abs/2104.03149v3.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        print("loading VQACE")
        # self.vqa2 = VQA2(*args, **kwargs)
        # download counterexamples
        self.url_counterexamples = "https://github.com/cdancette/detect-shortcuts/releases/download/v1.0/counterexamples.json"

        os.makedirs(os.path.join(self.dir_data, "datasets", "VQACE"), exist_ok=True)

        # download counterexamples
        if not os.path.exists(
            os.path.join(self.dir_data, "datasets", "VQACE", "counterexamples.json")
        ):
            print("downloading counterexamples")
            download_file(
                self.url_counterexamples,
                os.path.join(self.dir_data, "datasets", "VQACE"),
                filename="counterexamples.json",
            )
        ...
        with open(
            ...
            self.counterexamples = set(json.load(f))
        print(self.split)
        if self.split != "val":
            raise ValueError("VQACE only available for val split")

        self.questions = [
            q for q in self.questions if q["question_id"] in self.counterexamples
        ]
        self.annotations = [
            q for q in self.annotations if q["question_id"] in self.counterexamples
        ]

        # self.collate_fn = self.vqa2.collate_fn

    # def __len__(self):
    #     return len(self.vqa2)

    # def __getitem__(self, index):
    #     return self.vqa2[index]

    # def get_all_tokens(self):
    #     return self.vqa2.get_all_tokens()

    # def evaluate(self, predictions):
    #     return self.vqa2.evaluate(predictions)

    # def download_and_process(self):
    #     return self.vqa2.download_and_process()
