...
class COCOBottomUpFeatures:
    ...
    def __init__(self, features: str, dir_data: str = None, in_memory: bool = False):
        ...
        if self.in_memory:
            print("Opening features and boxes in memory")
            db = tb.open_file(self.featspath)
            self.features = db.root.features[:]
            self.boxes = db.root.boxes[:]
            db.close()
    ...
    def download_and_process(cls, name, dir_data):
        cls(features=name, dir_data=dir_data)
    ...
    def __getitem__(self, image_id: int):
        ...
        if self.in_memory:
            features = self.features[start_position : start_position + num_boxes]
            boxes = self.boxes[start_position : start_position + num_boxes]
        else:
