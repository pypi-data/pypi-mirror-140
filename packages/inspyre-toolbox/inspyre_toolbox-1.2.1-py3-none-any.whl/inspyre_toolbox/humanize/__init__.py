...
from typing import Any, Callable, List, Optional, Union, ContextManager

from inflect import engine

from inspyre_toolbox.errors import ArgumentConflictError
...
class Numerical(object):
    ...
    Note:
        When performing mathematical operations on a Numerical object the principle number of the class (the number
        contained within the Numerical.number property) will change to the resulting number of said mathematical
        operation any time the numerical object would sit on the left of an operator. You can change this behavior (
        on a by-call basis) by
        calling the
        operation method directly and passing a value of bool(False) to the

    """
    
    def __init__(self, number, noun: typing.Optional[str] = None, store_as_float=False, never_replace=False,
        ...
        # Let's first make sure that we don't suffer any argument conflicts
        if never_replace and always_replace:
            raise ArgumentConflictError('never_replace', 'always_replace')
        
        # Check whether 'number' is an integer or a float or not. If not; we raise an error
