...
class Atdf:
    def __init__(self, values: dict[str, List[List[str]]], sanity_check=True):
        if sanity_check:
    ...
    def update_row_value(self, record_name, index, new_value: List[str]) -> None:
        records_to_update: AtdfRecords = self.atdf_records[record_name]
        records_to_update[index, :] = new_value
