...
def get_backends(
    ...
    bs = BackendService(strangeworks.rest_client)
    return bs.get_backends(
...
def select_backend(selected_backend: Union[str, Backend] = None) -> Backend:
    bs = BackendService(strangeworks.rest_client)
    return bs.select_backend(selected_backend)


def get_coupling_map_from_backend(backend: Backend) -> dict:
    return strangeworks.rest_client.get(
...
def run(
    ...
    response = strangeworks.rest_client.post(
    ...
    return Job.from_json(
        job=response,
        backend=backend,
        rest_client=strangeworks.rest_client,
    )
