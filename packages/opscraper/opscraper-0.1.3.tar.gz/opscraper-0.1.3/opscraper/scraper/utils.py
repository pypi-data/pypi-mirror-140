...
def competition_metadata(competition_id: str, season_id: int, fixture_logger):
    competition = CompetitionModel.objects(competition_id=competition_id,
                                           season_id=season_id)
    if not competition:
        fixture_logger.error(f"competitionId: {competition_id},"
        " message: competition not found,"
        " function: competition_metadata")
...
def map_single_game(game: FixtureModel, fixture_logger):
    ...
    if home_team_document and away_team_document:
        ...
        fixture_logger.error(f"gameId: {game.game_oddsportal_id},"
        f" teamId:{game.home_team_id},"
        f" message: could not map home team document,"
        f" function: map_single_game")
        return False
    elif not away_team_document:
        fixture_logger.error(f"gameId: {game.game_oddsportal_id},"
        f" teamId:{game.away_team_id},"
        f" message: could not map away team document,"
        f" function: map_single_game")
        return False
    ...
    if mapped_game:
        ...
        fixture_logger.error(f"gameId: {game.game_oddsportal_id},"
        f" message: could not map game document,"
        f" function: map_single_game")
        return False

    game.save()
    fixture_logger.error(f"gameId: {game.game_oddsportal_id},"
    f" RaGameId: {game.game_id},"
    f" message: Successfully mapped game document,"
    f" function: map_single_game")
    return game
