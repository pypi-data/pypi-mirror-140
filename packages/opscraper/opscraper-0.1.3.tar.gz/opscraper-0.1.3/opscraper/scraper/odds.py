...
class Odds:
    ...
    def get_hash_uid(self):
        ...
        odds_logger.info(f"game_id: {self.game_oddsportal_id}, hashkey : {hash_uid}, message: decode hash key, function: get_hash_uid")
        return hash_uid
    ...
    def insert_double_odds_df(self, one_x_two_odds_df, bookmaker_id, bookmaker_name, line_id, market):
        for index, row in one_x_two_odds_df.iterrows():
            ...
            try:
                odd_document.save()
                odds_logger.info(f"gameId: {self.game.game_id}, database: oddsportal, bookmaker: {bookmaker_name}, message: Successfully inserted, function: insert_double_odds_df")
            except:
                odds_logger.error(f"gameId: {self.game.game_id}, database: oddsportal, bookmaker: {bookmaker_name}, message: Failed to insert, function: insert_double_odds_df")
            ...
            try:
                prod_odd_document.save()
                odds_logger.info(f"gameId: {self.game.game_id}, database: prod, bookmaker: {bookmaker_name}, message: successfully inserted, function: insert_double_odds_df")
            except:
                odds_logger.error(f"gameId: {self.game.game_id}, database: prod, bookmaker: {bookmaker_name}, message: failed to insert, function: insert_double_odds_df")
    ...
    def get_odds_json(self, market_code: str):
        ...
        while True:
            ...
            if "E" not in odds_data["d"]:
                odds_logger.info(f"game_id: {self.game_oddsportal_id}, hashkey:{self.hash_uid}, message: Successfully decode json odds, function: get_odds_json")
                return odds_data
            self.hash_uid = self.get_hash_uid()
            odds_logger.info(f"game_id: {self.game_oddsportal_id}, hashkey:{self.hash_uid}, message: Failed decode json odds, function: get_odds_json")
            time.sleep(2)
