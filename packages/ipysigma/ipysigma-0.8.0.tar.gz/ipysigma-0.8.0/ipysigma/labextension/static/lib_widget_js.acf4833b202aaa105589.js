...
class SigmaView extends base_1.DOMWidgetView {
    ...
    selectItem(type, key) {
        ...
        let innerHTML = `<b>${type === 'node' ? 'Node' : 'Edge'}</b> <i>${escapeHtml(key)}</i>`;
        const kwargInfo = [];
        const vizInfo = [];
        const info = [];
        ...
        if (info.length !== 0)
            innerHTML +=
                `<hr>${type === 'node' ? 'Node' : 'Edge'} attributes:<br>` +
                    info.join('<br>');
        if (vizInfo.length !== 0)
...
//# sourceMappingURL=lib_widget_js.acf4833b202aaa105589.js.map