...
class SigmaView extends base_1.DOMWidgetView {
    ...
    selectItem(type, key) {
        ...
        if (type === 'node') {
            ...
            innerHTML += `<b>Edge</b><br>from ${renderTypedValue(source)} to ${renderTypedValue(target)}`;
        }
        const kwargInfo = [];
        const vizInfo = [];
        const info = [];
