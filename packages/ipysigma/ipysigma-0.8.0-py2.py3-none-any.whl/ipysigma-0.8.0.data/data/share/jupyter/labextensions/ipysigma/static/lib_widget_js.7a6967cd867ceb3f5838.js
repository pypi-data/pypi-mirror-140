...
class SigmaView extends base_1.DOMWidgetView {
    ...
    selectItem(type, key) {
        ...
        let innerHTML = '';
        if (type === 'node') {
            innerHTML += `<b>Node</b> <i>${escapeHtml(key)}</i>`;
        }
        else {
            const [source, target] = this.graph.extremities(key);
            innerHTML += `<b>Edge</b><br>from ${renderAttributeValue(source)} to ${renderAttributeValue(target)}`;
        }
        const kwargInfo = [];
        const vizInfo = [];
        const info = [];
...
//# sourceMappingURL=lib_widget_js.7a6967cd867ceb3f5838.js.map