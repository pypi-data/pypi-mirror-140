...
class TemplateTreeControl(TreeControl[Template]):
    ...
    def render_node(self, node: TreeNode[Template]):
        ...
        """
        if not hasattr(node, "_my_render"):
            meta = {
                "@click": f"click_label({node.id})",
                "tree_node": node.id,
                "cursor": node.is_cursor,
            }
            paths = self.tree.get_mult(_get_template_key(self.tree, node.data)).data.flatten()
            existing = [p for p in paths if os.path.exists(p)]
            color = 'green' if len(existing) == len(paths) else 'yellow'
            if len(existing) == 0:
                color = 'red'
...
class TemplateRenderer:
    ...
    def __rich_console__(self, console, options):
        ...
        xr = self.tree.get_mult(_get_template_key(self.tree, self.template))
        ...
        for values in itertools.product(*[xr.coords[c].data for c in coords]):
            ...
            style = Style(
                bgcolor=None if os.path.exists(path) else 'red'
            )
