...
@LastEditTime: 2022-02-17 17:55:48
...
class CheckGmPowerHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        """
        is_power = False
        store_user_nick = self.get_user_nick().split(':')[0]
        if not store_user_nick:
            return self.response_json_success(is_power)
        if store_user_nick == config.get_value("test_user_nick"):
            is_power = True
        return self.response_json_success(is_power)
...
class GetAppidByGmHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if store_user_nick == config.get_value("test_user_nick"):
            ...
            app_base_model = AppBaseModel(context=self)
            app_id = app_base_model.get_app_id(store_name)
        return self.response_json_success(app_id)
...
class GetHighPowerListHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if len(custom_function_list) == 0:
            ...
            config_data["skin_config_list"] = query(custom_function_list[0]["skin_ids_second_list"]).select(lambda x: {"name": x["name"], "theme_id": x["theme_id"]}).to_list()
