    ...
    for column in data_frame:
        ...
        if np.any(_condition_1):
            _condition_2 = np.any(np.array([_v.unit != u.dimensionless_unscaled
                                            for _v in data_frame[column].to_numpy()]))
        if _condition_2:
            data_frame[column + '_unit'] = [_v.unit.to_string() for _v in data_frame[column].to_numpy()]
        if _condition_1:
            data_frame[column] = [_v.value for _v in data_frame[column].to_numpy()]
    return data_frame
...
def store_data(data_base_path='',
    ...
    _id_stimuli = upsert_db(table_name='stimuli', new_df=_row_stimuli, db=db,
    ...
    _id_recording = upsert_db(table_name='recording', new_df=_row_recording, db=db,
...
def upsert_db(table_name='',
              ...
              db=sqlite3.Connection, replace=True):
    if not new_df.size:
        return None
    statement = "SELECT name FROM sqlite_master WHERE type='table';"
