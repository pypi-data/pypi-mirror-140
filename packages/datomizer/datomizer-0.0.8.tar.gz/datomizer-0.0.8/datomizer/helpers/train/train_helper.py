...
def create_train_request(should_evaluate=True):
    return {"sampleInputData": 1, "privacyLevels": [-1], "shouldEvaluate": should_evaluate}


def train(dato_mapper: DatoMapper, should_evaluate=True):
    dato_mapper.wait()
...
def get_train_iteration(dato_mapper: DatoMapper, train_id):
    flow = dato_mapper.get_flow()
    iteration = [t for t in flow[general.TRAIN_ITERATIONS] if t[general.ID] == train_id][0]
    return iteration


def get_train_generated_zip(dato_mapper: DatoMapper, train_id: int):
