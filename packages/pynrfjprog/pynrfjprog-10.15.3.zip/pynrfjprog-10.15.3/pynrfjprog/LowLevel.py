...
class API(object):
    ...
    def open(self):
        ...
        self._logger = Parameters.LoggerAdapter(_logger, None, log=self._log, log_str_cb=self._log_str_cb,
    ...
    def enum_emu_con_info(self, list_usb_emus=True, list_ip_emus=True):
        ...
        while attempt > 0:
            ...
            result = self._lib.NRFJPROG_enum_emu_con_info_inst(self._handle,  ctypes.byref(emu_infos), emu_infos_len,
                                   ctypes.byref(num_available), list_usb_emus, list_ip_emus)
            if result != NrfjprogdllErr.SUCCESS:
                raise APIError(result, error_data=self.get_errors())
    ...
    def connect_to_emu_with_ip(self, hostname="", port=0, serial_number=0, jlink_speed_khz=_DEFAULT_JLINK_SPEED_KHZ):
        """
        Connects to a given emulator/debugger.
        ...
        result = self._lib.NRFJPROG_connect_to_emu_with_ip_inst(self._handle, hostname, port, serial_number, jlink_speed_khz)
        if result != NrfjprogdllErr.SUCCESS:
            self._logger.set_id(None)
            raise APIError(result, error_data=self.get_errors())
