...
class Run():
    ...
    def _setup_run(self):
        ...
        self.metrics_manager = MetricsManager(run_id=self.run_view.get("id"), push_method=self._push_metrics)
    ...
    @action_method
    def set_metrics_cache_settings(self, aggr: str = 'mean', 
                                    max_cache_timeout: int = None, 
        ...
        aggr [str]: The aggregate method. One of ['none', 'sum', 'max', 'mean']  (default is mean)
                    If None, the cache is fully sent to the lab (all metrics). Otherwise, only the aggregation is
                    made on the cache. If you plan having a lot of metrics (>500), it is best to use an aggregation method
                    to limit the load on the lab.
        """
    ...
    def start_recording_logs(self, logger_name=None):
        ...
        if self.dry_mode or self.read_only:
            print(f'A logger is returned but the logs are not saved because you are in a dry (offline) mode.')
