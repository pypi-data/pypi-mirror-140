...
def NXOS_show_ip_ospf_neighbors_detail_vrf(hostname, username, password, ip):
    ...
    # Create Testbed
        ...
        # For Each VRF
            for vrf in show_vrf['vrfs']:
                ...
                try:
                    show_ip_ospf_neighbor_detail_vrf = device.parse(f"show ip ospf neighbor detail vrf { vrf} ")
                except:
                    show_ip_ospf_neighbor_detail_vrf = f"{ hostname } Can't Parse"

        # Pass to template 

                if show_ip_ospf_neighbor_detail_vrf != f"{ hostname } Can't Parse":
                    NXOS_show_ip_ospf_neighbor_detail_vrf_template = env.get_template('NXOS_show_ip_ospf_neighbor_detail_vrf.j2')
                    loop_counter = 0
        # Render Templates
                    for filetype in filetype_loop:
                        parsed_output = NXOS_show_ip_ospf_neighbor_detail_vrf_template.render(to_parse_ip_ospf_neighbor=show_ip_ospf_neighbor_detail_vrf['vrf'],filetype_loop=loop_counter)
                        loop_counter = loop_counter + 1
    ...
    # -------------------------
                        if loop_counter <= 3:
                            with open(f"{ vrf }/{ filename }_Show IP OSPF Neighbor Detail VRF {vrf }.{ filetype }", "w") as fh:
                                fh.write(parsed_output)               
                                fh.close()
                        else:
                            with open(f"{ vrf }/{ filename }_Show IP OSPF Neighbor Detail VRF {vrf } Mind Map.md", "w") as fh:
                                fh.write(parsed_output)               
                                fh.close()
                        with open(f"{ vrf }/{ filename }_Show IP OSPF Neighbor Detail VRF {vrf }.json", "w") as fh:
                            json.dump(show_ip_ospf_neighbor_detail_vrf, fh, indent=4, sort_keys=True)
                            fh.close()
        return(show_ip_ospf_neighbor_detail_vrf)
    except Exception as e:
        logging.exception(e)
