...
def IOS_show_wlan_client_stats(hostname, username, password, ip):
    ...
    # Create Testbed
        ...
        if show_wlan_summary != f"{ hostname } Can't Parse":
            for wlan in show_wlan_summary['wlan_summary']['wlan_id'][wlan]:
                try:
                ...
                # create folders to hold files
                # -------------------------
                    if not os.path.exists(f"WLAN_ID { show_wlan_summary['wlan_summary']['wlan_id'] }"):
                        os.mkdir(f"WLAN_ID { show_wlan_summary['wlan_summary']['wlan_id'] }")
                    else:
                        print("Directory already exists")                    
                ...
                # Render Templates
                    ...
                    # -------------------------
                        if loop_counter <= 3:
                            with open(f"WLAN_ID { show_wlan_summary['wlan_summary']['wlan_id'][wlan] }/{ filename }_Show WLAN { wlan } Client Stats.{ filetype }", "w") as fh:
                                fh.write(parsed_output)               
                                fh.close()
                        else:
                            with open(f"WLAN_ID { show_wlan_summary['wlan_summary']['wlan_id'][wlan] }/{ filename }_Show WLAN { wlan } Client Stats Mind Map.md", "w") as fh:
                                fh.write(parsed_output)               
                                fh.close()
                        with open(f"WLAN_ID { show_wlan_summary['wlan_summary']['wlan_id'][wlan] }/{ filename }_Show WLAN { wlan } Client Stats.json", "w") as fh:
