...
def NXOS_show_all(hostname, username, password, ip):
    ...
    NXOS_show_cdp_neighbor_details(hostname, username, password, ip)
    return("Parsed All Show Commands")
...
def NXOS_show_cdp_neighbor_details(hostname, username, password, ip):
    try:
    # Create Testbed
        filename = hostname
        first_testbed = Testbed('dynamicallyCreatedTestbed')
        ...
        # Show CDP Neighbor Details to JSON
            ...
            try:
                show_cdp_neighbor_details = device.parse("show cdp neighbor details")
            except:
                show_cdp_neighbor_details = f"{ hostname } Can't Parse"

        # Pass to template 

        if show_cdp_neighbor_details != f"{ hostname } Can't Parse":
            NXOS_show_cdp_neighbor_details_template = env.get_template('NXOS_show_cdp_neighbor_details.j2')
            loop_counter = 0
        # Render Templates
            for filetype in filetype_loop:
                parsed_output = NXOS_show_cdp_neighbor_details_template.render(to_parse_cdp_neighbors=show_cdp_neighbor_details['index'],filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
    ...
    # -------------------------
                ...
                with open(f"{ filename }_Show CDP Neighbor Details.json", "w") as fh:
                    json.dump(show_cdp_neighbor_details, fh, indent=4, sort_keys=True)
                    fh.close()                                 
        return(show_cdp_neighbor_details)
    except Exception as e:
        logging.exception(e)