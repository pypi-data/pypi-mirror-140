...
def IOS_show_wlan_all(hostname, username, password, ip):
    ...
    # Create Testbed
        ...
        # Render Templates
            for filetype in filetype_loop:
                parsed_output = IOS_show_wlan_summary_template.render(to_parse_wlan=show_wlan_all,filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
...
def IOS_show_wlan_client_stats(hostname, username, password, ip):
    ...
    # Create Testbed
        ...
        if show_wlan_summary != f"{ hostname } Can't Parse":
            for wlan in show_wlan_summary['wlan_id']:
                try:
                ...
                # Render Templates
                    for filetype in filetype_loop:
                        parsed_output = IOS_show_wlan_client_stats_template.render(to_parse_wlan=show_wlan_client_stats,filetype_loop=loop_counter)
                        loop_counter = loop_counter + 1
                    ...
                    # -------------------------
                        if loop_counter <= 3:
                            with open(f"{ filename }_Show WLAN { wlan } Client Stats.{ filetype }", "w") as fh:
                                fh.write(parsed_output)               
                                fh.close()
                        else:
                            with open(f"{ filename }_Show WLAN { wlan } Client Stats Mind Map.md", "w") as fh:
                                fh.write(parsed_output)               
                                fh.close()
                        with open(f"{ filename }_Show WLAN { wlan } Client Stats.json", "w") as fh:
