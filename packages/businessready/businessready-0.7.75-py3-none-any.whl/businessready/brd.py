...
def ISE_allowed_protocols(url, username, password):
    try:
        ...
        allowedProtocols = requests.request("GET", f"https://{ url }/ers/config/allowedprotocols", headers=headers, auth=(username, password), verify=False)
    ...
    # -------------------------
                    ...
                    with open(f"ISE Allowed Protocols.json", "w") as fh:
                        json.dump(allowedProtocolHrefJSON, fh, indent=4, sort_keys=True)
                        fh.close()                            
