    ...
    # Create Testbed
        ...
        # Pass to template 
            ...
            for policy in show_wireless_profile_policy_summary['policy_name']:
                policy_name.append(policy)
                # Show WLAN all to JSON
                for hit in policy_name:
                    try:
                        print(f"{hit}")
                        show_wireless_profile_policy_detail = device.parse(f"show wireless profile policy detailed { hit }")
                    except:
                ...
                # -------------------------
                            if loop_counter <= 3:
                                with open(f"{ hit }/{ filename }_Show Wireless Profile Policy { hit } Detailed.{ filetype }", "w") as fh:
                                    fh.write(parsed_output)               
                                    fh.close()
                            else:
                                with open(f"{ hit }/{ filename }_Show Wireless Profile Policy { hit } Detailed Mind Map.md", "w") as fh:
                                    fh.write(parsed_output)               
                                    fh.close()
                            with open(f"{ hit }/{ filename }_Show Wireless Profile Policy { hit } Detailed.json", "w") as fh:
