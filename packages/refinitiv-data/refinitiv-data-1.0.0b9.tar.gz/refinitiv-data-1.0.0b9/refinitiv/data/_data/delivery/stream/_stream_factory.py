...
from ._stream_cxn_config import StreamCxnConfig, get_cxn_config_async
...
async def load_config(details: StreamDetails, session: "Session") -> "StreamCxnConfig":
    ...
    config: "StreamCxnConfig" = await get_cxn_config_async(api_config_key, session)
    logger.debug(f"Loaded config for {api_type}, {config}")
    return config


async def create_stream_cxn(
    details: StreamDetails, session: "Session"
) -> "StreamConnection":
    content_type = details.content_type
    protocol_type = details.protocol_type
    config = await load_config(details, session)
