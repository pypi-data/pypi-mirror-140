...
def run_init(*args):
    ...
    """
    if len(args) != 1:
        alert_and_exit("请输入命令完成项目初始化：arms init [name] （可先使用arms search [keyword]搜索模版名）")
    try:
...
def run_patch(*args):
    ...
    """
    if len(args) != 1:
        alert_and_exit("请输入命令完成项目补丁：arms patch [name]")
    try:
...
def run_config(*args):
    ...
    """
    if len(args) > 1:
        alert_and_exit("请输入文档访问链接：arms config [git_url]")
    elif len(args) == 1:
        git_url = args[0]
        pull_config(git_url)
    else:
...
def run_search(*args):
    ...
    """
    if len(args) != 1:
        alert_and_exit("请输入搜索关键词：arms search [keyword]")
