...
class TimeSeriesTransformer(AzureMLTransformer, FeaturizationInfoProvider):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._target_imputation_marker_column_name = MissingDummiesTransformer.get_column_name(self.target_column_name)
...
def _get_columns_by_type(
    ...
    columns = [
        x
        for x in df.select_dtypes(selected_types).columns
        if (x not in (drop_column_names or []) and x not in exclude_cols and x not in include_cols)
    ]
