...
class WorkerThread(threading.Thread):
    def __init__(self, compute_state):
        try:
            ...
            self._job_uuid = self.compute_state['job_id']
    ...
    def terminate(self) -> None:
        ...
        if self.compute_state['progress'] == 95:
            ...
            if self.compute_state['progress'] != 100:
                logger_no_user_data.debug(
                    f'Job "{self._job_uuid}" was not fetched within {seconds_to_sleep}s, cleaning up...'
                )

        webserver_utils.finalize_and_clean_up_compute_job(self._job_uuid)
