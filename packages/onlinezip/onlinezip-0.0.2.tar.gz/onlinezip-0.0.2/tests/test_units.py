...
def test_raise_when_no_ranges():
    with pytest.raises(HTTPRangeRequestUnsupported, match=r'range request is not supported'):
        OnlineZip(
            'https://files.pythonhosted.org/packages/74/4e/c533c3136427be62c38cc0e038cabf167bb54489c2ced2f6df903c456861/conda-4.3.16.tar.gz')


@pytest.mark.skip(reason="file is deleted, replacement needed")
def test_list_filenames_zip64():
    zip = OnlineZip('https://public-onlinezip.s3.eu-west-1.amazonaws.com/big_zip.zip')
    assert len(zip.namelist()) is 127


@pytest.mark.skip(reason="file is deleted, replacement needed")
def test_extract_zip64():
    zip = OnlineZip('https://public-onlinezip.s3.eu-west-1.amazonaws.com/big_zip.zip')
    zip.extract('Cyberpunk 2077/bin/x64/Cyberpunk2077.exe')
