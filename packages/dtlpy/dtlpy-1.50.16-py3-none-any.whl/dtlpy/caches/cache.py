...
class CacheManger:
    ...
    def get(self, key: CacheKey):
        ...
        for i in range(1, self._max_level + 1):
            res = self.cache_levels[i].get(key=key.get())
            if res:
                success = True
                break
        return success, res
    ...
    def set(self, key: CacheKey, value):
        ...
        """
        self.cache_levels[1].set(key.get(), json.dumps(value))
    ...
    def delete(self, key: CacheKey):
        ...
        """
        for i in range(1, self._max_level + 1):
            self.cache_levels[i].delete(key.get())
    ...
    def read(self, request_path: str):
        ...
        key = CacheKey(entity_id=entity_id)
        hit, response = self.get(key=key)
        if hit:
            return hit, response
        return False, None
    ...
    def write(self, list_entities_json):
        ...
        for entity_json in list_entities_json:
            ...
            self.set(key=key, value=entity_json)

    def invalidate(self, list_entities_json):
        """
