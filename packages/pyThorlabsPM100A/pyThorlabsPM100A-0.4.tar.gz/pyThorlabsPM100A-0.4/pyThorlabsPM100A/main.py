...
import sys
import argparse
from pyThorlabsPM100A.driver import ThorlabsPM100A
from pyThorlabsPM100A.plots import PlotObject
...
class interface():
    output = {'Power':0} 
    currentPowerUnits = 'W'
    continuous_read = False #When this is set to True, the data from device are acquired continuosly at the rate set by self.refresh_time
    refresh_time = 0.2 #default refresh rate, in seconds
    stored_data = [] # List used to store data acquired by device
    plot_window = None # QWidget object of the widget (i.e. floating window) that will contain the plot
    plot_object = None # PlotObject object of the plot where self.store_powers is plotted
    current_power_string = " " # Last power read from powermeter, as a string
    ...
    def __init__(self, app, mainwindow, parent, name_logger=__package__):
        ...
        self.instrument = ThorlabsPM100A() 
...
### GUI Events Functions
    ...
    def click_button_refresh_list_devices(self):
        ...
        (list_valid_devices,list_IDN) = self.instrument.list_devices()
        if(len(list_valid_devices)>0):
            list_IDNs_and_devices = [i + " -->  " + j for i, j in zip(list_IDN, list_valid_devices)] 
    ...
    def click_button_connect_disconnect(self):
        ''' 
        This function establishes the connection to the powermeter selected in the combobox list, or disconnects the powermeter, depending on the value of the variable self.instrument.connected .   
        If self.instrument.connected == False, then the powermeter is not connected, and we attempt connection
        If self.instrument.connected == True, then the powermeter is connected, and we attempt disconnection
        '''
        
        if(self.instrument.connected == False): # We attempt connection       
            self.connect_device()
        elif(self.instrument.connected == True): # We attempt disconnection
    ...
    def click_box_PowerRangeAuto(self, state):
        if state == QtCore.Qt.Checked:
            ...
            status = 'OFF'
            status_bool = False
        self.logger.info(f"Setting the auto-ranging function to {status} for the device {self.connected_device_name}...")    
        try:
            self.instrument.auto_power_range  = status_bool
    ...
    def press_enter_wavelength(self):
        ...
        try: 
            ...
            self.edit_Wavelength.setText(str(self.instrument.wavelength))
            return False
...
### END GUI Events Functions
    ...
    def read_wavelength(self):
        ...
        if self.wavelength == None:
            ...
            return
        self.edit_Wavelength.setText(str(int(self.wavelength)))
    ...
    def read_power_range(self):
        ...
        if self.power_range == None:
            ...
            return
        self.edit_PowerRange.setText(f"{self.power_range:.2e}")
    ...
    def read_status_power_autorange(self):
        ...
        try:
            ...
            self.box_PowerRangeAuto.setChecked(auto_power_range)
            if auto_power_range:
                status = 'ON'
    ...
    def start_reading(self):
        ...
        if not(self.press_enter_refresh_time()): #read the current value in the refresh_time textbox, and validates it. The function returns True/False if refresh_time was valid
            return
        if not(self.press_enter_wavelength()): #read the current value in the refresh_time textbox, and validates it. The function returns True/False if refresh_time was valid
            return
    ...
    def Update(self):
        ...
        '''
        if self.plot_object:
            self.plot_object.data.setData(list(range(1, len(self.stored_data)+1)), self.stored_data) #This line is executed even when self.continuous_read == False, to make sure that plot gets cleared when user press the stop button
        if(self.continuous_read == True):
            (currentPower,currentPowerUnits) = self.instrument.power
            self.output['Power'] = currentPower
            #self.output['PowerUnits'] = currentPowerUnits
            ...
            #super().Update()
            self.current_power_string = f"{currentPower:.2e}" + ' ' +  currentPowerUnits
            self.edit_Power.setText(self.current_power_string)
