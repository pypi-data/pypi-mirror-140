    ...
    def create_gui(self,parent,plot=False): 
        '''
            parent, QWidget        
                ...
                When this script is ran on its own (i.e. not within another gui) self.mainwindow and self.parent are actually the same object
            plot, boolean
...
### Functions to talk with low-level driver
        ...
        if(self.continuous_read == True):
            ...
            if hasattr(self,'trigger'):
                if not(self.trigger==None):
                    ...
                    QtCore.QTimer.singleShot(int(self.trigger[1]), self.trigger[0])    
    ...
    def set_trigger(self,external_function,delay=0):
        ...
        The optional parameter delay sets a delay (in ms) between the call to the function update and the call to the function external_function
        '''
...
def main():
    ...
    window = MainWindow()
    Interface = interface(app,window) #In this case window is both the MainWindow and the parent of the gui
