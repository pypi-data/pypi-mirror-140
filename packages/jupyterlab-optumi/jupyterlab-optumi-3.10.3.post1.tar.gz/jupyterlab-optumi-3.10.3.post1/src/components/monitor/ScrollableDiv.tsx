...
export class ScrollableDiv extends React.Component<IProps, IState> {
    ...
    private getFormattedContents = (): JSX.Element => {
		...
		if (source.length == 0) {
			return (<SPAN sx={{padding: '20px', display: 'inline-block', color: 'var(--jp-ui-font-color1)'}}>No output to display.</SPAN>);
		} else {
            const lines: string[] = [];
            ...
            return (
                <AnsiLink sx={{padding: '20px', display: 'inline-block', color: 'var(--jp-ui-font-color1)'}} key={lines[0]} text={ansiText} />
            );
        }
        return (<></>);
		// return content;
    ...
    public render = (): JSX.Element => {
        ...
        return (
            <DIV sx={{overflow: 'auto', width: '100%', height: '100%', whiteSpace: 'pre'}} innerRef={this.element}>
