		...
		return (
			<DIV sx={{
				...
				<DIV sx={{paddingBottom: "5px", textAlign: 'center', verticalAlign: 'middle'}}>
                    <DIV sx={{display: 'inline-flex', width: '100%'}}>
						...
						<UserDialog
							...
							getOpenTo={(openTo: (page: number) => Promise<void>) => {
						...
						<Tab
							...
							icon={
								<Badge
									...
									color={"primary"}>
									<SvgIcon viewBox="0, 0, 400,446" sx={{width: '28px', height: '28px'}}>
						...
						<Tab 
							...
							icon={
								<Badge
									...
									anchorOrigin={{ vertical: 'top', horizontal: 'left',}}
									color={"primary"}>
										<SvgIcon viewBox="0, 0, 400,400" sx={{width: '28px', height: '28px'}}>
							...
							} /*label="MONITOR"*/ sx={{minWidth: "72px"}} />
						<Tab 
							disableRipple
							icon={
								(() => {
									...
									if (Global.user.billingType == BillingType.CREDIT_BUCKET && dur.asHours() < 25) {
										return (<Badge
											badgeContent='!'
											anchorOrigin={{ vertical: 'top', horizontal: 'left',}}
											color={dur.asHours() <= 2 ? "error" : "warning"}>
												<SvgIcon viewBox="0, 0, 400,200" sx={{width: '28px', height: '28px'}}>
									...
									if (Global.user.billingType == BillingType.METERED_BILLING && Global.user.subscriptionActive && !Global.user.lastBillPaid) {
										return (<Badge
											badgeContent='!'
											anchorOrigin={{ vertical: 'top', horizontal: 'left',}}
											color={"error"}>
							...
							} /*label="MACHINES"*/ sx={{minWidth: "72px"}} />
					</Tabs>
				</DIV>
				<ShadowedDivider />
				...
				{this.state.page == Page.DEPLOY ? (
					<DeployPage sx={{flexGrow: 1}} openUserDialogTo={this.openUserDialogTo} />
				) : this.state.page == Page.MONITOR ? (
					<MonitorPage sx={{flexGrow: 1}} openUserDialogTo={this.openUserDialogTo}/>
