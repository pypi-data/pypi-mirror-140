...
class experiment():
    ...
    def __init__(self, app, mainwindow, parent, config_file, name_logger=__package__):
        ...
        self._refresh_time = 0.2
        self.continous_acquisition = False #Boolean variable that keeps track of wheter we are performing a continuous acquisition
    ...
    def update(self):
        if self.continous_acquisition:
            ...
            if self.external_trigger == False:
    ...
    def trigger(self):
        ...
        if self.continous_acquisition:
            ...
            self.logger.info(f"Received trigger from {self.trigger_instrument}, but experiment is not currently acquiring data.")

    def set_trigger_instrument(self,instrument_name):
        if self.trigger_instrument == instrument_name:
            return
        self.trigger_instrument = ''
        self.logger.info(f"Setting the instrument {instrument_name} as trigger...")
        for instrument in self.instruments:        
            if instrument['fullname'] == instrument_name:
                try:
                    instrument['interface'].set_trigger(self.trigger,delay=0)
