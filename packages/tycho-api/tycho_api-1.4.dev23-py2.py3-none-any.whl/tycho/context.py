    ...
    def get_definition(self, app_id):
        ...
        if not app_definition:
            try:
                ...
                # After yaml.safe_load, spec_template is a dict(!)
                template_dict = yaml.safe_load(response.text)
                logger.debug (f"-----> TEMPLATE_DICT:\n{template_dict}\nTYPE={type(template_dict)}")
                context = self.registry["settings"]
                logger.debug (f"-----> CONTEXT:\n{context}\nTYPE={type(context)}")
                # Convert spec_template to a string before passing into jinja2Template
                spec_template = yaml.safe_dump(template_dict)
                logger.debug (f"-----> SPEC_TEMPLATE:\n{spec_template}\nTYPE={type(spec_template)}")
                template = jinja2Template(spec_template)
                try:
                    app_def_str = template.render(**context)
                    logger.debug (f"-----> APP_DEFINITION:\n{app_def_str}")
                    app_def_dict = yaml.safe_load(app_def_str)
                    #app_def_dict = ast.literal_eval(app_def_str)
                    self.apps[app_id]['definition'] = app_def_dict
    ...
    def get_spec (self, app_id):
        ...
        if not spec:
            ...
            try:
                ...
                if response.status_code != 200:
                    raise ValueError(f"-- app {app_id}. failed to parse spec. code:{response.status_code}")
                # After yaml.safe_load, template_dict is a dict(!)
                template_dict = yaml.safe_load(response.text)
                logger.debug (f"-----> TEMPLATE_DICT: {template_dict}\nTYPE={type(template_dict)}")
                context = self.registry["settings"]
                logger.debug (f"-----> CONTEXT: {context}\nTYPE={type(context)}")
                # Convert spec_template back to a string before passing into jinja2Template
                ...
                try:
                    ...
                    # SPEC MUST BE STORED AS A DICT, NOT A STRING!!!
                    spec_as_dict = yaml.safe_load(spec_str)
                    #spec_as_dict = ast.literal_eval(spec_str)
                    logger.debug (f"-----> RENDERED SPEC_AS_DICT:\n{spec_as_dict}\nTYPE:{type(spec_as_dict)}")
                    #spec = spec_as_dict
                    self.apps[app_id]['spec_obj'] = spec_as_dict
                    logger.debug("f-----> TYPE of self.apps[app_id]['spec_obj'] IS {type(self.apps[app_id]['spec_obj'])}")
                except Exception as e:
                    traceback.print_exc ()
