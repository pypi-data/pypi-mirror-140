...
class PipelineClient:
    ...
    def create(
        self, name: str, pipeline_def: PipelineDef, is_resilient: bool = False
        ...
        res = self.http_client.make_authed_request(
            self.service_uri + "/streams", HttpMethod.POST, payload
        )
    ...
    def list(
        ...
        res = self.http_client.make_authed_request(
            f"{self.service_uri}/streams", HttpMethod.GET, params=params
        )
    ...
    def orchestrate(
        self, pipeline_id: Union[str, UUID], wait_on: List[Union[str, UUID]] = []
    ) -> Union[DtlError, Job]:
        """
    ...
    def schedule(
        ...
        if isinstance(res, DtlError):
            ...
            if isinstance(jobs, DtlError):
                ...
                return DtlError(
                    "Encountered an issue! There should be at least one job!"
                )
            else:
                return jobs[0]
        elif res.status_code == 400:
    ...
    def preview(
        ...
        target_ds = Datastore(
            "Dummy target datasource object", VoidDef(), datastore_id=uuid4()
        )
    ...
    def templatize(
        self, pipeline_id: Union[str, UUID]
    ...
    def update_schedule(
        ...
        if not payload:
            return DtlError(
                "Please specify one of these params: 'run_date', 'frequency' or 'abort_if_overlong'"
            )
        ...
        if isinstance(rsp, DtlError):
            return DtlError(
                f"Encountered an error while updating pipeline schedule: {rsp.message}"
            )
...
class _TemplateClient:
    ...
    def create(
        self, pipeline_template: PipelineTemplate, validate: bool = True
    ) -> Union[DtlError, PipelineTemplate]:
        """
        ...
        if validate is True and pipeline_template.dummy_input_map is None:
            return DtlError(
                "dummy_input_map must have key -> value if 'validate' argument is True"
            )
    ...
    def update(
        ...
        res = self.http_client.make_authed_request(
            f"{self.service_uri}/templates/{str(id)}", HttpMethod.PUT, req_body
        )
    ...
    def list(
        self, page: int = 1, item_per_page: int = 25
    ) -> Union[DtlError, List[PipelineTemplate]]:
        """
        List all templates.
        ...
        res = self.http_client.make_authed_request(
            f"{self.service_uri}/templates", HttpMethod.GET, params=req_params
        )
        if isinstance(res, DtlError):
            return res
    ...
    def get(self, id: Union[str, UUID]) -> Union[DtlError, PipelineTemplate]:
        ...
        """
        res = self.http_client.make_authed_request(
            f"{self.service_uri}/templates/{str(id)}", HttpMethod.GET
        )
        if isinstance(res, DtlError):
            return res
    ...
    def delete(self, id: Union[str, UUID]) -> Union[DtlError, bool]:
        ...
        res = self.http_client.execute_authed_request(
            f"{self.service_uri}/templates/{str(id)}", HttpMethod.DELETE
        )
    ...
    def share(
        ...
        if isinstance(rsp, DtlError):
            ...
            return SharePermission(
                object_type=ObjectType.PipelineTemplate
            ).from_payload(rsp.json())
        else:
    ...
    def validate(
        self, id: UUID, dummy_input_map: Dict[str, str]
    ) -> Union[bool, DtlError]:
        """
        ...
        rsp = self.http_client.execute_authed_request(
            url, HttpMethod.POST, dummy_input_map
        )
    ...
    def generate_pipeline(
        ...
        if isinstance(rsp, DtlError):
            return DtlError(
                f"Encountered an error while generating pipeline from template: {rsp.message}"
            )
    ...
    def run(
        ...
        if isinstance(rsp, DtlError):
            return DtlError(
                f"Encountered an error while running pipeline from template: {rsp.message}"
            )
