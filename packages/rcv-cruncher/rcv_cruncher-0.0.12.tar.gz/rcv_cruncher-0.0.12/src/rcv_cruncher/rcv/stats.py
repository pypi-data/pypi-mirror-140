"""Contains RCV_stats class which is added into RCV.
"""
...
class RCV_stats:
    """
    Extra methods that are part of RCV
    """
    ...
    def _exhaustion_categories(self, *, tabulation_num=1):
        ...
        final_ranks_list = self.get_final_ranks(tabulation_num=tabulation_num, disaggregate=False)
        ballot_marks_list = self.get_cvr_dict(self._contest_rule_set_name, disaggregate=False)["ballot_marks"]
        ...
        for used_last_rank, inital_ranks, final_ranks, ballot_marks in zip(
            ...
            # then it wasnt exhausted
                ...
                why_exhaust.append(BallotMarks.POSTTALLY_EXHAUSTED_BY_RANK_LIMIT)
            else:
                why_exhaust.append(BallotMarks.POSTTALLY_EXHAUSTED_BY_ABSTENTION)
    ...
    def _tabulation_winner(self, tabulation_num=1):
        ...
        """
        elected_candidates = [
            d for d in self.get_candidate_outcomes(tabulation_num=tabulation_num) if d["round_elected"] is not None
        ]
        ...
        return [
            ...
            for d in sorted(
                elected_candidates,
                key=lambda x: (x["round_elected"], -x["elected_vote"], x["name"]),
            )
        ]
    ...
    def _first_round_winner_place(self, tabulation_num=1):
        ...
        """
        winner = self._tabulation_winner(tabulation_num=tabulation_num)[0]
        tally_tuple = self.get_round_tally_tuple(1, tabulation_num=tabulation_num, only_round_active_candidates=True)
    ...
    def _compute_contest_stat_table(self):
        ...
        cvr = self.get_cvr_dict(self._contest_rule_set_name, disaggregate=False)
        ...
        for iTab in range(1, self._tab_num + 1):
            df[f"final_weight{iTab}"] = self.get_final_weights(tabulation_num=iTab, disaggregate=False)
        ...
        for iTab in range(1, self._tab_num + 1):
            ...
            df[f"pretally_exhausted{iTab}"] = df[exhaust_type_str].eq(BallotMarks.PRETALLY_EXHAUST)
            ...
            exh_by_rank_limit_fully_ranked = self._cvr_stat_table["fully_ranked_incl_overvotes"] & df[
                exhaust_type_str
            ].eq(BallotMarks.POSTTALLY_EXHAUSTED_BY_RANK_LIMIT)
    ...
    def _compute_summary_contest_stat_tables(self) -> None:
        ...
        for iTab in range(1, self._tab_num + 1):
            ...
            treat_writeins = self._rule_sets[self._contest_rule_set_name][
            ...
            posttally_duplicate = sum(
                self._contest_stat_table[f"posttally_exhausted_by_duplicate_rankings{iTab}"] * weight
            )
            ...
            posttally_rank_limit_full = sum(
                self._contest_stat_table[f"posttally_exhausted_by_rank_limit_fully_ranked{iTab}"] * weight
            )
            s["total_posttally_exhausted_by_rank_limit_fully_ranked"] = posttally_rank_limit_full
            s["total_posttally_exhausted_by_rank_limit_partially_ranked"] = (
                posttally_rank_limit - posttally_rank_limit_full
            )
    ...
    def _compute_contest_split_stats(self, split_filter: List[bool]) -> pd.DataFrame:
        ...
        for iTab in range(1, self._tab_num + 1):
            ...
            posttally_rank_limit_full = sum(
                filtered_stat_table[f"posttally_exhausted_by_rank_limit_fully_ranked{iTab}"] * weight
            )
            s["split_total_posttally_exhausted_by_rank_limit_fully_ranked"] = posttally_rank_limit_full
            s["split_total_posttally_exhausted_by_rank_limit_partially_ranked"] = (
                posttally_rank_limit - posttally_rank_limit_full
            )
    ...
    def _compute_summary_contest_split_stat_tables(self) -> None:
        ...
        for field in self.split_fields:
            ...
            if field.lower() in field_name_lower_dict:
                ...
                for unique_val in set(cvr_field):
                    ...
                    split_stat_df_list = self._compute_contest_split_stats(field_filter)
        ...
        summary_contest_split_stat_tables = [
            pd.concat(split_stat_list, axis=0, ignore_index=True, sort=False)
