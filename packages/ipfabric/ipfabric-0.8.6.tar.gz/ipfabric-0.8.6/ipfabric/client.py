...
class IPFClient(IPFabricAPI):
    def __init__(
        ...
        self.graphs = (
            Diagram(self)
            if parse_version(self.os_version) < parse_version("4.3")
            else ImportError("v4.3 digrams has been moved to ipfabrc-diagrams python package")
        )
        self.security = (
            Security(client=self)
            if parse_version(self.os_version) < parse_version("4.3")
            else ImportError("Security Policy tables have changed and need updated")
        )
