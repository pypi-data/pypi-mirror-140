...
class SemiDFA(Drawable):
    ...
    def dotFormat(self, size="20,20", filename=None, direction="LR", strict=False, maxlblsz=6, sep="\n") -> str:
        ...
        for si in range(len(self.States)):
            ...
            s += "\"dummy{0:s}\" -> \"{1:s}\";{2:s}".format(sn, graphvizTranslate(self.dotLabel(self.States[si])), sep)
        for si in range(len(self.States)):
...
class FA(Drawable):
    ...
    def dotFormat(self, size="20,20", filename=None, direction="LR", strict=False, maxlblsz=6, sep="\n") -> str:
        ...
        s += "dummy -> \"{0:s}\"{1:s}".format(self.dotLabel(o.States[o.Initial]), sep)
...
class OFA(FA):
    ...
    def dotDrawTransition(self, st1, label, st2, sep="\n"):
        ...
        Returns:
            str:"""
        return "\"{0:s}\" -> \"{1:s}\" [label = \"{2:s}\"];{3:s} ".format(graphvizTranslate(self.dotLabel(st1)),
                                                                          graphvizTranslate(self.dotLabel(st2)),
                                                                          label, sep)
...
class NFA(OFA):
    ...
    def dotFormat(self, size="20,20", filename=None, direction="LR", strict=False, maxlblsz=6, sep="\n") -> str:
        ...
        s += "size=\"{0:s}\";{1:s}".format(size, sep)
        for si in o.Initial:
            sn = self.dotLabel(o.States[si])
            s += "node [shape = point]; \"dummy{0:s}\"{1:s}".format(sn, sep)
            s += o.dotDrawState(si)
            s += "\"dummy{0:s}\" -> \"{1:s}\";{2:s}".format(sn, graphvizTranslate(self.dotLabel(o.States[si])), sep)
    ...
    def HKeqP(self, other, strict=True):
        ...
        :param other: NFA
        :param strict: if True checks for same alphabets
        :return: Boolean
...
class NFAr(NFA):
    ...
    def homogenousP(self, inplace=False):
        ...
        :arg bool inplace: if True performs CEpsilon transitions elimination
    ...
    def elimEpsilonO(self):
        """Eliminate CEpsilon-transitions from this automaton, with reduction of states through elimination of
        CEpsilon-cycles, and single CEpsilon-transition cases.
    ...
    def unlinkSoleIncoming(self, state):
        """If given state has only one incoming transition (indegree is one), and it's through CEpsilon,
    ...
    def unlinkSoleOutgoing(self, state):
        """If given state has only one outgoing transition (outdegree is one), and it's through CEpsilon,
...
class DFA(OFA):
    ...
    def addTransition(self, sti1, sym, sti2):
        ...
        if sym == Epsilon:
            raise DFAnotNFA("Invalid CEpsilon transition from {0:>s} to {1:>s}.".format(str(sti1), str(sti2)))
        self.Sigma.add(sym)
        if sti1 not in self.delta:
            self.delta[sti1] = {sym: sti2}
        else:
    ...
    def make_sufix_free(self):
        """ Turns a DFA in a prefix-free automaton deleting all incoming transitions to the initial state"""
        new = self.minimalMooreSq()
        for i in new.delta:
            l = [k for k in new.delta[i] if new.delta[i][k] == new.Initial]
            for k in l:
                del(new.delta[i][k])
        return new
