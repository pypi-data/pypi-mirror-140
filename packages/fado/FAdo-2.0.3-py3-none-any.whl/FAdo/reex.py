...
class RegExp(RegularExpression):
    ...
    def nfaPosition(self, lstar=True):
        ...
        :arg lstar: if not None followlists are computed as disjunct
        :type lstar: boolean
        :return: Position NFA
        :rtype: NFA
    ...
    def nfaFollowEpsilon(self, trim=True):
        ...
        :param trim:
        :return: NFA possibly with CEpsilon transitions
        :rtype: NFAe
...
class CAtom(RegExp):
    ...
    def reduced(self, has_epsilon=False):
        ...
        :param has_epsilon: used internally to indicate that the language of which this term is a subterm has the empty
            word.
...
class COption(RegExp):
    """Class for COption operation  on regular expressions.
    ...
    def __repr__(self):
        return "COption(%s)" % (repr(self.arg))

    def __copy__(self):
        return COption(copy.copy(self.arg), copy.copy(self.Sigma))
    ...
    def reversal(self):
        ...
        :rtype: reex.RegExp"""
        return COption(self.arg.reversal(), self.Sigma)

    def mark(self):
        return COption(self.arg.mark(), self.Sigma)
    ...
    def unmark(self):
        return COption(self.arg.unmark(), self.Sigma)
    ...
    def unmarked(self):
        return COption(self.arg.unmarked())

    def _marked(self, pos):
        (r1, pos1) = self.arg._marked(pos)
        return COption(r1), pos1
...
class CStar(RegExp):
    ...
    def mark(self):
        return CStar(self.arg.mark(), self.Sigma)

    def __init__(self, arg, sigma=None):
        super(CStar, self).__init__()
        self.arg = arg
        self.Sigma = sigma
    ...
    def __repr__(self):
        return "CStar(%s)" % repr(self.arg)

    def __copy__(self):
        return CStar(copy.copy(self.arg), copy.copy(self.Sigma))

    def unmark(self):
        """ Conversion back to RegExp

        :rtype: reex.CStar"""
        return CStar(self.arg.unmark(), self.Sigma)
    ...
    def reversal(self):
        ...
        :rtype: reex.RegExp"""
        return CStar(self.arg.reversal(), sigma=self.Sigma)
...
def _ifconcat(re0, re1, dd=True, both=False, sigma=None):
    """
    Concatenation of two res if not CEpsilon
    :param re0: RegExp
    :param re1: RegExp
    :param dd: Boolean if True re0.re1 else r21.re0
    :param both: Boolean if True test both
    :return:
    """
    if re0.epsilonP():
        return re1
...
class BuildRegexp(lark.Transformer):
    ...
    def shuffle(self, s):
        (arg1, arg2) = s
        return CShuffle(arg1, arg2, self.sigma)

    def conj(self, s):
        (arg1, arg2) = s
        return CConj(arg1, arg2, self.sigma)

    def option(self, s):
        return COption(s[0], self.sigma)

    def notn(self, s):
        return Compl(s[0], self.sigma)
