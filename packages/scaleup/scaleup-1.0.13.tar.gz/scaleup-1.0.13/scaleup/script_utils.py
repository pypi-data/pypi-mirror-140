...
__writers = { 
	"SIMULATION" : (lambda writer, res : __write_simulation_step_result__(writer, res)),
	"FITTING" : (lambda writer, res : __write_report_step_result__(writer, res)),
	"VERIFICATION" : (lambda writer, res : __write_report_step_result__(writer, res)),
	"OPTIMIZATION" : (lambda writer, res : __write_report_step_result__(writer, res)),
	}
...
def create_script_option_string(showInUi : bool = True, altReader : bool = False):
	...
	:param showInUi: Show the Automation UI when running, defaults to True
	:type showInUi: bool, optional
	:param altReader: Use the alternate file reader for Excel models (i.e. not Excel Interop), defaults to False
	:type altReader: bool, optional
...
def create_fitting_parameter(parameter_name : str, unit : str, initial_value : float, max_value : float, min_value : float, fit_to_log : bool = False):
	...
	:rtype: str
	"""
	ret = __string_delim.join([parameter_name, unit, str(initial_value), str(max_value), str(min_value), str(fit_to_log)])
	return ret
...
def __write_simulation_profile(writer, profile):
	writer('Scenario name\t{}'.format(profile.scenario_name))
	writer('Solver steps\t{}'.format(profile.solver_steps))
	writer('Total time\t{}'.format(profile.total_time))
	writer('Matrix size\t{}'.format(profile.matrix_size))
	writer('Plotted points\t{}'.format(profile.plotted_points))
	...
	writer('\n')
	writer(profile.results)
	writer('\n')


def __write_simulation_step_result__(writer, step_result):
	writer(step_result.endpoints)
	writer('\n\n')
	for profile in step_result.profiles:
		__write_simulation_profile(writer, profile)
...
def write_script_result(result, writer = None):
	"""
	Write the contents of a script result.
	...
	if(result.ErrorMessage != None and len(result.ErrorMessage) > 0):
		...
		for step_result in result.steps:
			...
			__writers[step_result.type](writer, step_result)
			step_number += 1



def write_script_result_to_file(result, file_name : str, append : bool = True):
	"""
	Write the contents of a script result.
	A script result contain have an error message or an array of step results.
	If it contains an error message, then this is written
	...
	try:
		...
		mode = "a" if append else "w"
		f=open(file_path, mode)
		write_script_result(result, f.write)
	finally:
		f.flush()
		f.close()
