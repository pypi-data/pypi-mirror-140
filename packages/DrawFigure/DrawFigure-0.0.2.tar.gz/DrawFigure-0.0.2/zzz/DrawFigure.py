...
class Figs():
    ...
    def show_menu(self,event):
        ...
        self.config["text"]["x"]=0.0129*event.x-3.0351
        self.config["text"]["y"]=(-1/140)*event.y+2.45714285
    ...
    def Layout(self):
        ...
        ttk.Label(frame2,text="对数坐标:").grid(row=5,column=2,sticky=W)
    ...
    def draw(self,post_data):
        ...
        if self.config["text"]["creat_text"]:
            self.ax.text(self.config["text"]["x"],self.config["text"]["y"],self.config["text"]["text"])
