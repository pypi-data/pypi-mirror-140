...
def gen_independent_tables(tables):
    for table in tables:
        has_dependent_cols = any(gen_dependent_cols(table))
        has_relationships = any(gen_relationships(table))

        if not (has_dependent_cols or has_relationships):
            yield table


def gen_dependent_tables(tables, dependency, seen=None):
    seen = set(seen or [])
    ...
    for table in tables:
        ...
        dependent_cols = set(gen_dependent_cols(table))

        if len(dependent_cols) == 2 and not table.id:
            # it's an association proxy table
            continue

        relationship_cols = {p.singular_noun(r) for r in gen_relationships(table)}
        ...
        if is_dependent and all_dependencies_met:
            yield table
            yield from gen_dependent_tables(tables, table, seen)


def gen_association_tables(tables):
    for table in tables:
        dependent_cols = list(gen_dependent_cols(table))

        if len(dependent_cols) == 2:
            yield table


def gen_cols(table, *args):
    excluded_cols = set(args)
...
def gen_independent_cols(table, *args):
    excluded_cols = set(args)
...
def gen_dependent_cols(table, *args):
    excluded_cols = set(args)
...
def gen_relationships(table):
    mapper = sqlalchemy_inspect(table)
    columns = set(gen_cols(table))
...
def gen_from_independent_tables(tables, independent_tables, seen=None):
    for dependency in independent_tables:
        yield from gen_dependent_tables(tables, dependency, seen=seen)


def get_all_tables(tables):
    _independent_tables = gen_independent_tables(tables)
    association_tables = list(gen_association_tables(tables))
