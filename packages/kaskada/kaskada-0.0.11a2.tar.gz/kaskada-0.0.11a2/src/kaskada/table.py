...
def upload_from_s3(table: Union[table_pb.Table, table_pb.CreateTableResponse, table_pb.GetTableResponse, str], access_key: str, secret: str, virtual_hosted: str = None, region: str = None, bucket: str = None, key: str = None, client: Client = None):
    """
    Uploads a file from an external S3 source to a table. Credentials are required and are not persisted in any way.

    Args:
        table (Union[table_pb.Table, table_pb.CreateTableResponse, table_pb.GetTableResponse, str]): The target table object
        access_key (str): Amazon Web Services Access Key. Used only for authentication and is not persisted.
        secret (str): Amazon Web Services Secret Key. Used only for authentication and is not persisted.
        ...
        region (str, optional): [description]. The target region of the S3 bucket. Only used if virtual_hosted is not provided.
        bucket (str, optional): [description]. The target S3 bucket. Only used if virtual_hosted is not provided.
        key (str, optional): [description]. The target key to the object in the S3 bucket. Only used if virtual_hosted is not provided.
        client (Client, optional): The Kaskada Client. Defaults to kaskada.KASKADA_DEFAULT_CLIENT.
