...
class DecryptionHandler:
 decryption_key:xRSoJ
 def __init__(self,decryption_key:xRSoJ):
  self.decryption_key=decryption_key
 def decrypt(self,content)->xRSoJ:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=xRSoc):
  if path and not xRSoE(path,xRSoi):
   path=xRSoi(xRSoB(path,"_path",[]))
  if not path:
   return xRSoc
...
def patch_traceback_lines():
 if xRSoB(traceback.FrameSummary,"_ls_patch_applied",xRSoc):
  return
 @xRSoF
 def line(self):
  try:
 ...
 xRSov(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=xRSoQ
def patch_inspect_findsource():
 if xRSoB(inspect,"_ls_patch_applied",xRSoc):
  return
 ...
 inspect._ls_patch_applied=xRSoQ
