...
CAPTURE_METAMODEL_SEPARATELY=rUasQ
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=rUasb(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=rUasb(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->rUasz:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[rUasz,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,rUasq)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(rUasq,state_files_to)
...
class CommitMetamodelUtils:
 @rUasR
 def create_metadata_archive(cls,version:Version,delete_reference:bool=rUasQ,overwrite:bool=rUasQ,metamodels_file:rUasz=rUasq):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @rUasR
 def create_delta_metamodel_file(cls,version:rUasm,revision:rUasm,store_to_zip=rUasQ)->Tuple[Dict,Dict]:
 ...
 @rUasR
 def reconstruct_metamodel(cls,version:rUasm,revision:rUasm=rUasq)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in rUasl(1,revision+1):
  ...
  return result
 @rUasR
 def get_version_metamodel(cls,version:rUasm,revision:rUasm=rUasq)->Dict:
 ...
 @rUasR
 def get_commit_diff(cls,version_no:rUasm,commit_no:rUasm)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->rUasW:
 if rUasu(clazz,rUaso):
  return{}
 constructor=rUasE(clazz,"__init__",rUasq)
...
def _create_metamodel_helper(obj,width=25,visited:set=rUasq):
 if obj is rUasq:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not rUasq else obj
 if rUasu(obj,rUaso):
  result=rUaso(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif rUasu(obj,rUase):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if rUasX(result)>width:
   result={"size":rUasX(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:rUasz)->rUasm:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
