...
SENT_EMAILS=[]
EMAIL_BLACKLIST=qIpKF()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",qIpKx(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=qIpKJ,from_name=qIpKJ,smtp_host=qIpKJ,smtp_user=qIpKJ,smtp_pass=qIpKJ,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if qIpKS(recipients,qIpKn)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=qIpKJ):
 ...
 result["From"]=formataddr((qIpKx(Header(from_name,"utf-8")),from_email))
 return result
