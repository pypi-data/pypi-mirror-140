...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return FnbSO if e else FnbSR
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->FnbSC:
...
@click.option("--revert",is_flag=FnbSO,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:FnbSm):
...
def _cpvcs_initialized(pod_name:FnbSP)->FnbSm:
...
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_init(name:FnbSP):
...
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=FnbSO,default=FnbSR)
def cmd_cpvcs_delete(name:FnbSP,remote:FnbSm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_register_remote(name:FnbSP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=FnbSO)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=FnbSO)
def cmd_cpvcs_rename(name:FnbSP,new_name:FnbSP):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_commit(message:FnbSP,name:FnbSP):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=FnbSO,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=FnbSO,default=FnbSR,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_push(squash:FnbSm,message:FnbSP,name:FnbSP,three_way:FnbSm):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
@click.option("-v","--version",type=FnbSf)
@click.option("-m","--message",required=FnbSR)
def cmd_cpvcs_push_overwrite(version:FnbSf,message:FnbSP,name:FnbSP):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=FnbSf,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=FnbSO,default=FnbSR,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_inject(version:FnbSf,reset:FnbSm,name:FnbSP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=FnbSO,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=FnbSO,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
@click.option("--lazy/--eager",default=FnbSO,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:FnbSP,inject:FnbSm,reset:FnbSm,lazy:FnbSm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=FnbSO,default=FnbSR)
def cmd_cpvcs_list_pods(remote:FnbSm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_versions(name:FnbSP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=FnbSO,type=FnbSf)
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_version_info(version:FnbSf,name:FnbSP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=FnbSf,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_version_metamodel(version:FnbSf,name:FnbSP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=FnbSO,type=FnbSf,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=FnbSO,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=FnbSO,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=FnbSR,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_set_version(version:FnbSf,inject:FnbSm,reset:FnbSm,commit_before:FnbSm,name:FnbSP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_commits(version:FnbSf,name:FnbSP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=FnbSO)
@click.option("--commit","-c",required=FnbSO)
@click.option("-n","--name",help="Name of the cloud pod",required=FnbSO)
def cmd_cpvcs_commit_diff(version:FnbSf,commit:FnbSf,name:FnbSP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(FnbSC)
  status.stop()
...
def print_table(columns:Dict[FnbSP,FnbSP],rows:List[Dict[FnbSP,Any]],formatters:Dict[FnbSP,Callable[[Any],FnbSP]]=FnbSC):
 from rich.table import Table
 if formatters is FnbSC:
  formatters=FnbSH()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  FnbSo(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->FnbSC:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->FnbSC:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->FnbSC:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->FnbSC:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
