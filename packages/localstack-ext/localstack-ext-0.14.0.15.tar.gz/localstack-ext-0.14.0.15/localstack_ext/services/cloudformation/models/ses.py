...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[JKHgN])[0]
 def get_physical_resource_id(self,attribute=JKHgN,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if JKHgs(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return JKHgB(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @JKHgj
 def add_defaults(resource,stack_name:JKHgu):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return JKHgB(SESReceiptRule,self).get_cfn_attribute(attribute)
 @JKHgX
