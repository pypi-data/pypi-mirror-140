...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=VIGrK,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if VIGrk(perm["IpProtocol"])!=VIGrk(rp["IpProtocol"]):
    continue
 ...
 @VIGrA
 def get_vpc_and_name_for_security_group(cls,group_id:VIGrk)->Tuple[VIGrk,VIGrk]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or VIGrK
  group_name=groups and groups[0]["GroupName"]or VIGrK
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",VIGrK)
   vpc_id=VIGrK
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",VIGrK)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=VIGrk(result.get("IpProtocol"))
   description=result.pop("Description",VIGrK)
   cidr_ipv6=result.pop("CidrIpv6",VIGrK)
