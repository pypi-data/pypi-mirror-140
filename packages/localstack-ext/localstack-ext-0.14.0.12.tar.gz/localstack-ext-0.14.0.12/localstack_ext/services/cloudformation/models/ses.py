...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[dEskB])[0]
 def get_physical_resource_id(self,attribute=dEskB,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if dEskA(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return dEskJ(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @dEskS
 def add_defaults(resource,stack_name:dEskq):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return dEskJ(SESReceiptRule,self).get_cfn_attribute(attribute)
 @dEske
