...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or cqpOt
 @cqpOY
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return cqpOk(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @cqpOV
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if cqpOz(origins,cqpOo)else origins.get("Items")
  origins_to_create=cqpOy([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if cqpOz(aliases,cqpOo)else aliases.get("Items")
  aliases_to_create=cqpOy(aliases)
  for distr in distrs:
   origins=cqpOy([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=cqpOy(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",cqpOi)
   config["Enabled"]=cqpOh(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if cqpOz(custom_errors,cqpOn)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=cqpOh(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":cqpOi}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=cqpOh(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=cqpOt):
    parent=config if parent is cqpOt else parent
    items=parent.get(key)or[]
    if cqpOz(items,cqpOn)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":cqpOR(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if cqpOz(cache_behavs,cqpOn)else cache_behavs)
