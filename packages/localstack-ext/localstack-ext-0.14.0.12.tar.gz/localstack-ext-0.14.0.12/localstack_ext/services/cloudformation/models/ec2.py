...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=ysPIW,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if ysPIw(perm["IpProtocol"])!=ysPIw(rp["IpProtocol"]):
    continue
 ...
 @ysPIo
 def get_vpc_and_name_for_security_group(cls,group_id:ysPIw)->Tuple[ysPIw,ysPIw]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or ysPIW
  group_name=groups and groups[0]["GroupName"]or ysPIW
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",ysPIW)
   vpc_id=ysPIW
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",ysPIW)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=ysPIw(result.get("IpProtocol"))
   description=result.pop("Description",ysPIW)
   cidr_ipv6=result.pop("CidrIpv6",ysPIW)
