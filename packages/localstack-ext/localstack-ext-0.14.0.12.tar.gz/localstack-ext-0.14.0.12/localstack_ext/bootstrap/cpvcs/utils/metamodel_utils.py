...
CAPTURE_METAMODEL_SEPARATELY=EOBUt
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=EOBUL(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=EOBUL(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->EOBUT:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[EOBUT,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,EOBUQ)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(EOBUQ,state_files_to)
...
class CommitMetamodelUtils:
 @EOBUF
 def create_metadata_archive(cls,version:Version,delete_reference:bool=EOBUt,overwrite:bool=EOBUt,metamodels_file:EOBUT=EOBUQ):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @EOBUF
 def create_delta_metamodel_file(cls,version:EOBUd,revision:EOBUd,store_to_zip=EOBUt)->Tuple[Dict,Dict]:
 ...
 @EOBUF
 def reconstruct_metamodel(cls,version:EOBUd,revision:EOBUd=EOBUQ)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in EOBUl(1,revision+1):
  ...
  return result
 @EOBUF
 def get_version_metamodel(cls,version:EOBUd,revision:EOBUd=EOBUQ)->Dict:
 ...
 @EOBUF
 def get_commit_diff(cls,version_no:EOBUd,commit_no:EOBUd)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->EOBUe:
 if EOBUA(clazz,EOBUX):
  return{}
 constructor=EOBUK(clazz,"__init__",EOBUQ)
...
def _create_metamodel_helper(obj,width=25,visited:set=EOBUQ):
 if obj is EOBUQ:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not EOBUQ else obj
 if EOBUA(obj,EOBUX):
  result=EOBUX(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif EOBUA(obj,EOBUj):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if EOBUg(result)>width:
   result={"size":EOBUg(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:EOBUT)->EOBUd:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
