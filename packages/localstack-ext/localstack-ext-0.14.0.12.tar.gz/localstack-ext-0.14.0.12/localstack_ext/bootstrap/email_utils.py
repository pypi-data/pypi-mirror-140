...
SENT_EMAILS=[]
EMAIL_BLACKLIST=MDwtO()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",MDwtL(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=MDwtv,from_name=MDwtv,smtp_host=MDwtv,smtp_user=MDwtv,smtp_pass=MDwtv,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if MDwtT(recipients,MDwto)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=MDwtv):
 ...
 result["From"]=formataddr((MDwtL(Header(from_name,"utf-8")),from_email))
 return result
