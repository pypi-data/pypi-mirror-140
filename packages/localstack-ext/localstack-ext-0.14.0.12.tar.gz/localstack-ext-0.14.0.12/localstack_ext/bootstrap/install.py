...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=ASYRF)
 timescaledb_installer.install(raise_on_error=ASYRF)
 redis_installer.install(raise_on_error=ASYRF)
 mosquitto_installer.install(raise_on_error=ASYRF)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:ASYRC,target_file:ASYRC,timeout:int=ASYRv):
 def do_download(url,print_error=ASYRF):
  try:
