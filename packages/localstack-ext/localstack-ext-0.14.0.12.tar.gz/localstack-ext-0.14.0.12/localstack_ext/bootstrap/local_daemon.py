...
class RequestHandler(BaseHTTPRequestHandler):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if EPqQj(result,EPqQh)else result
  return result
 def read_content(self):
  if EPqQb(self,"data_bytes"):
   return
  content_length=self.headers.get("Content-Length")
  self.data_bytes=self.rfile.read(EPqQY(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get("file_name")or "s3file.%s"%EPqQg(uuid.uuid4()))
...
def next_available_bind_address(port):
 start_id=EPqQO(USED_BIND_ADDRESSES)+2
 for idx in EPqQV(start_id,start_id+30):
  host=LOCAL_BIND_ADDRESS_PATTERN.replace("*",EPqQg(idx))
  ...
  return host
 raise EPqQi(f"Unable to determine free bind address for port {port}")
def create_network_interface_alias(address,interface=EPqQR):
 sudo_cmd="sudo"
 if is_mac_os():
 ...
 raise EPqQi("Unable to create network interface")
def run_shell_cmd(command):
 try:
  return{"result":run_cmd(command)}
 except EPqQi as e:
  error_string=EPqQg(e)
  if EPqQj(e,subprocess.CalledProcessError):
...
def run_cmd(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=EPqQX))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output("uname -a",shell=EPqQX))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode("utf-8")if EPqQj(obj,EPqQg)else obj
def to_str(obj):
 return obj.decode("utf-8")if EPqQj(obj,EPqQv)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if EPqQO(sys.argv)>1 else "main"
...
def start_in_background():
 ...
 run(daemon_cmd,outfile=log_file,asynchronous=EPqQX)
 ...
 run("sudo -v",stdin=EPqQX)
 def start_root_daemon(*args,asynchronous=EPqQR):
  sudo_cmd=f"sudo {daemon_cmd} root >> {log_file}"
  run(sudo_cmd,outfile=log_file,stdin=EPqQX,asynchronous=asynchronous)
