  ...
  try:
   if not self._check_if_available():
    try:
     ...
     if JSMvU(e,PackageInstallationException):
      raise
     else:
      suffix=(f" ({self.log_package_name})." if self.log_package_name is not JSMvB else ".")
   ...
   if raise_on_error:
    ...
    if self.logger.isEnabledFor(logging.DEBUG):
     ...
     self.logger.error(e)
 def _check_if_available(self)->JSMvG:
  raise JSMvL
 def _prepare_installation(self)->JSMvB:
  pass
 def _install_package(self)->JSMvB:
  raise JSMvL
 def _post_process(self)->JSMvB:
  pass
 def _verify_installation(self)->JSMvB:
  if not self._check_if_available():
   package_info=(f" of {self.log_package_name}" if self.log_package_name is not JSMvB else "")
...
class MultiPackageInstaller(PackageInstaller):
 def __init__(self,log_package_name:Optional[JSMvI],package_installers:Union[PackageInstaller,List[PackageInstaller]]):
  JSMvN(MultiPackageInstaller,self).__init__(log_package_name=log_package_name)
  self.package_installers=(package_installers if JSMvU(package_installers,List)else[package_installers])
 def install(self,raise_on_error:JSMvG=JSMvu):
...
class OSSpecificPackageInstaller(MultiPackageInstaller):
 def __init__(self,debian_installers:PackageInstaller,redhat_installers:PackageInstaller):
  ...
  JSMvN(OSSpecificPackageInstaller,self).__init__(log_package_name=self.__class__.__name__,package_installers=package_installers)
 def install(self,raise_on_error:JSMvG=JSMvu):
  try:
   if not in_docker():
    ...
    JSMvN(OSSpecificPackageInstaller,self).install(raise_on_error=raise_on_error)
  except PackageInstallationException as e:
   if raise_on_error:
    raise
   else:
...
class DebianPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:JSMvI):
  JSMvN(DebianPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[JSMvI,List[JSMvI]]):
  packages=packages if JSMvU(packages,List)else[packages]
  with INSTALL_LOCK:
   run(["apt-get","update"])
   run(["apt-get","install","-y","--no-install-recommends"]+packages)
class RedHatPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:JSMvI):
  JSMvN(RedHatPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[JSMvI,List[JSMvI]]):
  packages=packages if JSMvU(packages,List)else[packages]
  with INSTALL_LOCK:
...
class DebianPostgres11Installer(DebianPackageInstaller):
 def __init__(self):
  JSMvN(DebianPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->JSMvG:
...
class RedHatPostgres11Installer(RedHatPackageInstaller):
 def __init__(self):
  JSMvN(RedHatPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->JSMvG:
  return is_command_available("psql")
 def _prepare_installation(self)->JSMvB:
...
class DebianPlPythonInstaller(DebianPackageInstaller):
 def __init__(self):
  JSMvN(DebianPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->JSMvG:
...
class RedHatPlPythonInstaller(RedHatPackageInstaller):
 def __init__(self):
  JSMvN(RedHatPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->JSMvG:
...
class DebianMariaDBInstaller(DebianPackageInstaller):
 def __init__(self):
  JSMvN(DebianMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->JSMvG:
...
class RedHatMariaDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  JSMvN(RedHatMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->JSMvG:
  return is_command_available("mysqld")
 def _install_package(self)->JSMvB:
...
class DebianTimescaleDBInstaller(DebianPackageInstaller):
 def __init__(self):
  JSMvN(DebianTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->JSMvG:
...
class RedHatTimescaleDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  JSMvN(RedHatTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->JSMvG:
...
class DebianRedisInstaller(DebianPackageInstaller):
 def __init__(self):
  JSMvN(DebianRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->JSMvG:
...
class RedHatRedisInstaller(RedHatPackageInstaller):
 def __init__(self):
  JSMvN(RedHatRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->JSMvG:
  return is_command_available("redis-server")
 def _install_package(self)->JSMvB:
...
class DebianMosquittoInstaller(DebianPackageInstaller):
 def __init__(self):
  JSMvN(DebianMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->JSMvG:
...
class RedHatMosquittoInstaller(RedHatPackageInstaller):
 def __init__(self):
  JSMvN(RedHatMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->JSMvG:
  return is_command_available("mosquitto")
 def _install_package(self)->JSMvB:
