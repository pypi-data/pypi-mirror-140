...
CAPTURE_METAMODEL_SEPARATELY=SPFHR
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=SPFHe(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=SPFHe(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->SPFHk:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[SPFHk,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,SPFHN)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(SPFHN,state_files_to)
...
class CommitMetamodelUtils:
 @SPFHQ
 def create_metadata_archive(cls,version:Version,delete_reference:bool=SPFHR,overwrite:bool=SPFHR,metamodels_file:SPFHk=SPFHN):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @SPFHQ
 def create_delta_metamodel_file(cls,version:SPFHU,revision:SPFHU,store_to_zip=SPFHR)->Tuple[Dict,Dict]:
 ...
 @SPFHQ
 def reconstruct_metamodel(cls,version:SPFHU,revision:SPFHU=SPFHN)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in SPFHY(1,revision+1):
  ...
  return result
 @SPFHQ
 def get_version_metamodel(cls,version:SPFHU,revision:SPFHU=SPFHN)->Dict:
 ...
 @SPFHQ
 def get_commit_diff(cls,version_no:SPFHU,commit_no:SPFHU)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->SPFHE:
 if SPFHK(clazz,SPFHy):
  return{}
 constructor=SPFHG(clazz,"__init__",SPFHN)
...
def _create_metamodel_helper(obj,width=25,visited:set=SPFHN):
 if obj is SPFHN:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not SPFHN else obj
 if SPFHK(obj,SPFHy):
  result=SPFHy(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif SPFHK(obj,SPFHm):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if SPFHo(result)>width:
   result={"size":SPFHo(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:SPFHk)->SPFHU:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
