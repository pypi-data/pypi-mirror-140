...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=HbknQ
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:HbknG)->HbknG:
  pass
 @Hbknh
 def get(cls,requested_file_locator:HbknG):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=HbknQ
 @Hbknh
 def get(cls,state_file_locator:HbknG,requested_storage:HbknG,serializers:Dict[HbknG,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[HbknG,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:HbknG)->HbknG:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:HbknG)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:HbknG)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:HbknG)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:HbknG)->HbknG:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[HbknG]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:HbknG,new_key:HbknG,referenced_by_version:HbknG=HbknQ):
  pass
 @abc.abstractmethod
 def version_exists(self,key:HbknG)->HbknU:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:HbknG,key:HbknG):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:HbknG,new_key:HbknG,base_path:HbknG)->HbknU:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[HbknG]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[HbknG,HbknG],remote_location:HbknG=HbknQ)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[HbknG]:
  ...
  for cpvcs_obj in args:
   if HbknL(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif HbknL(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[HbknG,HbknG],remote_location:HbknG=HbknQ)->List[Union[Revision,Version]]:
