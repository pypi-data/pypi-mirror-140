...
class RequestHandler(BaseHTTPRequestHandler):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if EopIn(result,EopIX)else result
  return result
 def read_content(self):
  if EopIJ(self,"data_bytes"):
   return
  content_length=self.headers.get("Content-Length")
  self.data_bytes=self.rfile.read(EopIw(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get("file_name")or "s3file.%s"%EopIb(uuid.uuid4()))
...
def next_available_bind_address(port):
 start_id=EopIe(USED_BIND_ADDRESSES)+2
 for idx in EopIl(start_id,start_id+30):
  host=LOCAL_BIND_ADDRESS_PATTERN.replace("*",EopIb(idx))
  ...
  return host
 raise EopIP(f"Unable to determine free bind address for port {port}")
def create_network_interface_alias(address,interface=EopIG):
 sudo_cmd="sudo"
 if is_mac_os():
 ...
 raise EopIP("Unable to create network interface")
def run_shell_cmd(command):
 try:
  return{"result":run_cmd(command)}
 except EopIP as e:
  error_string=EopIb(e)
  if EopIn(e,subprocess.CalledProcessError):
...
def run_cmd(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=EopIv))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output("uname -a",shell=EopIv))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode("utf-8")if EopIn(obj,EopIb)else obj
def to_str(obj):
 return obj.decode("utf-8")if EopIn(obj,EopIt)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if EopIe(sys.argv)>1 else "main"
...
def start_in_background():
 ...
 run(daemon_cmd,outfile=log_file,asynchronous=EopIv)
 ...
 run("sudo -v",stdin=EopIv)
 def start_root_daemon(*args,asynchronous=EopIG):
  sudo_cmd=f"sudo {daemon_cmd} root >> {log_file}"
  run(sudo_cmd,outfile=log_file,stdin=EopIv,asynchronous=asynchronous)
