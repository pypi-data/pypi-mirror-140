...
class KeyActivationError(lQxmn):
 pass
...
class InvalidDecryptionKeyError(KeyActivationError):
 pass
def is_enterprise():
 return read_api_key(raise_if_missing=lQxmM)=="enterprise"
def read_api_key(raise_if_missing=lQxmh):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise lQxmn("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def truncate_api_key(api_key:lQxmN):
 return '"%s..."(%s)'%(api_key[:3],lQxmX(api_key))
def fetch_key()->lQxmP:
 ...
 if api_key=="test":
  ...
  key_base64=load_cached_key(api_key=api_key,check_timeout=lQxmM)
  decoded_key=base64.b64decode(key_base64)
  return decoded_key
 proxies=get_proxies()or lQxmo
 data={"api_key":api_key,"version":__version__}
 try:
  logging.getLogger("py.warnings").setLevel(logging.ERROR)
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=lQxmM,proxies=proxies)
  ...
  if log_license_issues():
   api_key=lQxmN(api_key_configured()or "")
...
def cache_key_locally(api_key,key_b64):
 timestamp=lQxmN(lQxmS(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in lQxmw(lQxmX(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 cache=get_key_cache()
 cache.update({"timestamp":lQxmS(timestamp),"key_hash":md5(api_key),"key":key_b64})
 cache.save()
def load_cached_key(api_key:lQxmN,check_timeout:bool=lQxmh)->lQxmN:
 cache=get_key_cache()
 ...
 timestamp=lQxmN(cache["timestamp"])
 ...
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 return key_b64
def enable_file_decryption(key:lQxmP):
 ...
 try:
  ...
  if "import" not in to_str(file_content):
   raise lQxmG("Decryption resulted in invalid python file!")
 except lQxmn:
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise lQxmn("Unable to fetch and validate API key from environment")
   if to_str(key)!="test":
...
def api_key_configured():
 return read_api_key(raise_if_missing=lQxmM)
def is_logged_in():
 return lQxmh if get_auth_cache().get("token")else lQxmM
def get_auth_headers()->Dict:
 ...
 api_key=read_api_key(raise_if_missing=lQxmM)
 if api_key:
  return{"ls-api-key":api_key,"ls-version":__version__}
 raise lQxmn("Please log in first")
