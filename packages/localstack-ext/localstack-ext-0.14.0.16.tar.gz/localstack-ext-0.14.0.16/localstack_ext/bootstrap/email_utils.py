...
SENT_EMAILS=[]
EMAIL_BLACKLIST=geOFN()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",geOFP(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=geOFx,from_name=geOFx,smtp_host=geOFx,smtp_user=geOFx,smtp_pass=geOFx,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if geOFA(recipients,geOFY)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=geOFx):
 ...
 result["From"]=formataddr((geOFP(Header(from_name,"utf-8")),from_email))
 return result
