...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=nTYNv)
 timescaledb_installer.install(raise_on_error=nTYNv)
 redis_installer.install(raise_on_error=nTYNv)
 mosquitto_installer.install(raise_on_error=nTYNv)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:nTYNX,target_file:nTYNX,timeout:int=nTYNB):
 def do_download(url,print_error=nTYNv):
  try:
