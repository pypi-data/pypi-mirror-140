...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return MFjpS if e else MFjpB
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->MFjpC:
...
@click.option("--revert",is_flag=MFjpS,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:MFjpe):
...
def _cpvcs_initialized(pod_name:MFjpQ)->MFjpe:
...
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_init(name:MFjpQ):
...
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=MFjpS,default=MFjpB)
def cmd_cpvcs_delete(name:MFjpQ,remote:MFjpe):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_register_remote(name:MFjpQ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=MFjpS)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=MFjpS)
def cmd_cpvcs_rename(name:MFjpQ,new_name:MFjpQ):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_commit(message:MFjpQ,name:MFjpQ):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=MFjpS,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=MFjpS,default=MFjpB,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_push(squash:MFjpe,message:MFjpQ,name:MFjpQ,three_way:MFjpe):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
@click.option("-v","--version",type=MFjpG)
@click.option("-m","--message",required=MFjpB)
def cmd_cpvcs_push_overwrite(version:MFjpG,message:MFjpQ,name:MFjpQ):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=MFjpG,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=MFjpS,default=MFjpB,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_inject(version:MFjpG,reset:MFjpe,name:MFjpQ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=MFjpS,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=MFjpS,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
@click.option("--lazy/--eager",default=MFjpS,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:MFjpQ,inject:MFjpe,reset:MFjpe,lazy:MFjpe):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=MFjpS,default=MFjpB)
def cmd_cpvcs_list_pods(remote:MFjpe):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_versions(name:MFjpQ):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=MFjpS,type=MFjpG)
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_version_info(version:MFjpG,name:MFjpQ):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=MFjpG,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_version_metamodel(version:MFjpG,name:MFjpQ):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=MFjpS,type=MFjpG,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=MFjpS,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=MFjpS,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=MFjpB,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_set_version(version:MFjpG,inject:MFjpe,reset:MFjpe,commit_before:MFjpe,name:MFjpQ):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_commits(version:MFjpG,name:MFjpQ):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=MFjpS)
@click.option("--commit","-c",required=MFjpS)
@click.option("-n","--name",help="Name of the cloud pod",required=MFjpS)
def cmd_cpvcs_commit_diff(version:MFjpG,commit:MFjpG,name:MFjpQ):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(MFjpC)
  status.stop()
...
def print_table(columns:Dict[MFjpQ,MFjpQ],rows:List[Dict[MFjpQ,Any]],formatters:Dict[MFjpQ,Callable[[Any],MFjpQ]]=MFjpC):
 from rich.table import Table
 if formatters is MFjpC:
  formatters=MFjpb()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  MFjpi(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->MFjpC:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->MFjpC:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->MFjpC:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->MFjpC:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
