...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or CwJLt
 @CwJLs
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return CwJLN(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @CwJLa
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if CwJLE(origins,CwJLT)else origins.get("Items")
  origins_to_create=CwJLy([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if CwJLE(aliases,CwJLT)else aliases.get("Items")
  aliases_to_create=CwJLy(aliases)
  for distr in distrs:
   origins=CwJLy([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=CwJLy(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",CwJLO)
   config["Enabled"]=CwJLf(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if CwJLE(custom_errors,CwJLV)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=CwJLf(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":CwJLO}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=CwJLf(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=CwJLt):
    parent=config if parent is CwJLt else parent
    items=parent.get(key)or[]
    if CwJLE(items,CwJLV)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":CwJLA(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if CwJLE(cache_behavs,CwJLV)else cache_behavs)
