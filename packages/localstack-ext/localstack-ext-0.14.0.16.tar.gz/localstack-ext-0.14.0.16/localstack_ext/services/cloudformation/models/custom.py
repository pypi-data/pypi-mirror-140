...
class CustomResource(GenericBaseModel):
 ...
 def get_deploy_templates():
  def create_custom_resource(resource_id,resources,resource_type,func,stack_name,*args):
   ...
   def _convert(obj,**kwargs):
    if zkcAE(obj,zkcAS):
     for key,value in obj.items():
      if zkcAE(value,zkcAu):
       obj[key]=zkcAa(value).lower()
    return obj
   ...
   if zkcAa(service_token).startswith("arn:aws:lambda"):
   ...
   result=zkcAn
   try:
    result=retry(fetch_result,retries=zkcAy(CUSTOM_RESOURCES_RESULT_POLL_TIMEOUT/2),sleep=2)
    result=json.loads(to_str(result))
   except zkcAH:
    bucket_objects=map_all_s3_objects(buckets=[CUSTOM_RESOURCES_RESULTS_BUCKET])
    LOG.info("Unable to fetch CF custom resource result from s3://%s/%s . Existing keys: %s"%(CUSTOM_RESOURCES_RESULTS_BUCKET,result_key,zkcAI(bucket_objects.keys())))
    raise
