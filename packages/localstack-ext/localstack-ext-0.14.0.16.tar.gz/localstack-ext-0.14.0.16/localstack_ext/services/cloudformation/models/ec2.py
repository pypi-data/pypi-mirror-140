...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=Jxfen,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if Jxfer(perm["IpProtocol"])!=Jxfer(rp["IpProtocol"]):
    continue
 ...
 @JxfeF
 def get_vpc_and_name_for_security_group(cls,group_id:Jxfer)->Tuple[Jxfer,Jxfer]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or Jxfen
  group_name=groups and groups[0]["GroupName"]or Jxfen
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",Jxfen)
   vpc_id=Jxfen
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",Jxfen)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=Jxfer(result.get("IpProtocol"))
   description=result.pop("Description",Jxfen)
   cidr_ipv6=result.pop("CidrIpv6",Jxfen)
