...
class AppSyncResolver(GenericBaseModel):
 ...
 def fetch_details(cls,api_id,type_name,field_name,data_source_name):
  ...
  return(result or[mGQUh])[0]
 @mGQUd
 def get_deploy_templates():
  def get_resolver_params(params,**kwargs):
   params=keys_to_lower(clone(params))
   req_template=params.pop("requestMappingTemplateS3Location",mGQUh)
   res_template=params.pop("responseMappingTemplateS3Location",mGQUh)
   if req_template:
    tmpl=aws_utils.download_s3_object(req_template,as_str=mGQUv)
    if tmpl:
     params["requestMappingTemplate"]=tmpl
   if res_template:
    tmpl=aws_utils.download_s3_object(res_template,as_str=mGQUv)
    if tmpl:
...
class GraphQLSchema(GenericBaseModel):
 ...
 @mGQUy
 def fetch_details(cls,api_id,definition=mGQUh,s3_location=mGQUh):
  client=aws_stack.connect_to_service("appsync")
  try:
   schema=client.get_introspection_schema(apiId=api_id,format="SDL")["schema"]
   schema=schema.read()if mGQUL(schema,"read")else schema
...
class AppSyncApiKey(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return mGQUB(AppSyncApiKey,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute=mGQUh,**kwargs):
  props=self.props
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=[k for k in keys if k.get("description")==description and expires in[mGQUh,k.get("expires")]]
  return(result or[mGQUh])[0]
 @mGQUy
 def get_deploy_templates(cls):
  def _create_params(_,resources,resource_id,**kwargs):
   ...
   if res_props.get("Expires"):
    res_props["Expires"]=mGQUj(res_props.get("Expires"))
...
class AppSyncDataSource(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return mGQUB(AppSyncDataSource,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute=mGQUh,**kwargs):
  return self.props.get("dataSourceArn")
 def fetch_state(self,stack_name,resources):
  props=self.props
  ...
  return([s for s in sources if s["name"]==ds_name]or[mGQUh])[0]
class AppSyncFunctionConfig(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return mGQUB(AppSyncFunctionConfig,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute,**kwargs):
...
class GraphQLAPI(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  if not props.get("apiId"):
   ...
   if not details:
    raise mGQUk('Unable to find GraphQL API named "%s"'%props.get("Name"))
  ...
  if attribute_name=="GraphQLUrl":
   return appsync_api.get_graphql_url(props["apiId"],use_domain_name=mGQUv)
  return mGQUB(GraphQLAPI,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute,**kwargs):
  props=self.props
 ...
 def fetch_details(cls,api_name):
  ...
  return([a for a in apis if a["name"]==api_name]or[mGQUh])[0]
