...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return etdao if e else etdaz
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->etdau:
...
@click.option("--revert",is_flag=etdao,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:etdaE):
...
def _cpvcs_initialized(pod_name:etdaF)->etdaE:
...
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_init(name:etdaF):
...
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=etdao,default=etdaz)
def cmd_cpvcs_delete(name:etdaF,remote:etdaE):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_register_remote(name:etdaF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=etdao)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=etdao)
def cmd_cpvcs_rename(name:etdaF,new_name:etdaF):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_commit(message:etdaF,name:etdaF):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=etdao,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=etdao,default=etdaz,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_push(squash:etdaE,message:etdaF,name:etdaF,three_way:etdaE):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
@click.option("-v","--version",type=etdav)
@click.option("-m","--message",required=etdaz)
def cmd_cpvcs_push_overwrite(version:etdav,message:etdaF,name:etdaF):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=etdav,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=etdao,default=etdaz,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_inject(version:etdav,reset:etdaE,name:etdaF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=etdao,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=etdao,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
@click.option("--lazy/--eager",default=etdao,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:etdaF,inject:etdaE,reset:etdaE,lazy:etdaE):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=etdao,default=etdaz)
def cmd_cpvcs_list_pods(remote:etdaE):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_versions(name:etdaF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=etdao,type=etdav)
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_version_info(version:etdav,name:etdaF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=etdav,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_version_metamodel(version:etdav,name:etdaF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=etdao,type=etdav,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=etdao,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=etdao,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=etdaz,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_set_version(version:etdav,inject:etdaE,reset:etdaE,commit_before:etdaE,name:etdaF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_commits(version:etdav,name:etdaF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=etdao)
@click.option("--commit","-c",required=etdao)
@click.option("-n","--name",help="Name of the cloud pod",required=etdao)
def cmd_cpvcs_commit_diff(version:etdav,commit:etdav,name:etdaF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(etdau)
  status.stop()
...
def print_table(columns:Dict[etdaF,etdaF],rows:List[Dict[etdaF,Any]],formatters:Dict[etdaF,Callable[[Any],etdaF]]=etdau):
 from rich.table import Table
 if formatters is etdau:
  formatters=etdai()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  etdag(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->etdau:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->etdau:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->etdau:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->etdau:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
