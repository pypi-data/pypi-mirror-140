...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or swKrq
 @swKrU
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return swKrY(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @swKrS
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if swKrE(origins,swKrX)else origins.get("Items")
  origins_to_create=swKrp([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if swKrE(aliases,swKrX)else aliases.get("Items")
  aliases_to_create=swKrp(aliases)
  for distr in distrs:
   origins=swKrp([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=swKrp(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",swKrM)
   config["Enabled"]=swKrd(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if swKrE(custom_errors,swKrk)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=swKrd(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":swKrM}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=swKrd(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=swKrq):
    parent=config if parent is swKrq else parent
    items=parent.get(key)or[]
    if swKrE(items,swKrk)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":swKrR(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if swKrE(cache_behavs,swKrk)else cache_behavs)
