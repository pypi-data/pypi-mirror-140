...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=WdUyJ,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if WdUye(perm["IpProtocol"])!=WdUye(rp["IpProtocol"]):
    continue
 ...
 @WdUyb
 def get_vpc_and_name_for_security_group(cls,group_id:WdUye)->Tuple[WdUye,WdUye]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or WdUyJ
  group_name=groups and groups[0]["GroupName"]or WdUyJ
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",WdUyJ)
   vpc_id=WdUyJ
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",WdUyJ)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=WdUye(result.get("IpProtocol"))
   description=result.pop("Description",WdUyJ)
   cidr_ipv6=result.pop("CidrIpv6",WdUyJ)
