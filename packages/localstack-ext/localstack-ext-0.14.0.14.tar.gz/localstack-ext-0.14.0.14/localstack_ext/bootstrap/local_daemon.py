...
class RequestHandler(BaseHTTPRequestHandler):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if tezig(result,teziG)else result
  return result
 def read_content(self):
  if tezio(self,"data_bytes"):
   return
  content_length=self.headers.get("Content-Length")
  self.data_bytes=self.rfile.read(teziY(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get("file_name")or "s3file.%s"%teziQ(uuid.uuid4()))
...
def next_available_bind_address(port):
 start_id=tezif(USED_BIND_ADDRESSES)+2
 for idx in teziM(start_id,start_id+30):
  host=LOCAL_BIND_ADDRESS_PATTERN.replace("*",teziQ(idx))
  ...
  return host
 raise tezip(f"Unable to determine free bind address for port {port}")
def create_network_interface_alias(address,interface=teziU):
 sudo_cmd="sudo"
 if is_mac_os():
 ...
 raise tezip("Unable to create network interface")
def run_shell_cmd(command):
 try:
  return{"result":run_cmd(command)}
 except tezip as e:
  error_string=teziQ(e)
  if tezig(e,subprocess.CalledProcessError):
...
def run_cmd(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=tezib))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output("uname -a",shell=tezib))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode("utf-8")if tezig(obj,teziQ)else obj
def to_str(obj):
 return obj.decode("utf-8")if tezig(obj,teziI)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if tezif(sys.argv)>1 else "main"
...
def start_in_background():
 ...
 run(daemon_cmd,outfile=log_file,asynchronous=tezib)
 ...
 run("sudo -v",stdin=tezib)
 def start_root_daemon(*args,asynchronous=teziU):
  sudo_cmd=f"sudo {daemon_cmd} root >> {log_file}"
  run(sudo_cmd,outfile=log_file,stdin=tezib,asynchronous=asynchronous)
