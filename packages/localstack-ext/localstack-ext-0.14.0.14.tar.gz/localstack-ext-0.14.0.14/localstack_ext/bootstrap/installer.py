  ...
  try:
   if not self._check_if_available():
    try:
     ...
     if IzOEk(e,PackageInstallationException):
      raise
     else:
      suffix=(f" ({self.log_package_name})." if self.log_package_name is not IzOED else ".")
   ...
   if raise_on_error:
    ...
    if self.logger.isEnabledFor(logging.DEBUG):
     ...
     self.logger.error(e)
 def _check_if_available(self)->IzOEB:
  raise IzOEJ
 def _prepare_installation(self)->IzOED:
  pass
 def _install_package(self)->IzOED:
  raise IzOEJ
 def _post_process(self)->IzOED:
  pass
 def _verify_installation(self)->IzOED:
  if not self._check_if_available():
   package_info=(f" of {self.log_package_name}" if self.log_package_name is not IzOED else "")
...
class MultiPackageInstaller(PackageInstaller):
 def __init__(self,log_package_name:Optional[IzOEf],package_installers:Union[PackageInstaller,List[PackageInstaller]]):
  IzOEL(MultiPackageInstaller,self).__init__(log_package_name=log_package_name)
  self.package_installers=(package_installers if IzOEk(package_installers,List)else[package_installers])
 def install(self,raise_on_error:IzOEB=IzOEC):
...
class OSSpecificPackageInstaller(MultiPackageInstaller):
 def __init__(self,debian_installers:PackageInstaller,redhat_installers:PackageInstaller):
  ...
  IzOEL(OSSpecificPackageInstaller,self).__init__(log_package_name=self.__class__.__name__,package_installers=package_installers)
 def install(self,raise_on_error:IzOEB=IzOEC):
  try:
   if not in_docker():
    ...
    IzOEL(OSSpecificPackageInstaller,self).install(raise_on_error=raise_on_error)
  except PackageInstallationException as e:
   if raise_on_error:
    raise
   else:
...
class DebianPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:IzOEf):
  IzOEL(DebianPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[IzOEf,List[IzOEf]]):
  packages=packages if IzOEk(packages,List)else[packages]
  with INSTALL_LOCK:
   run(["apt-get","update"])
   run(["apt-get","install","-y","--no-install-recommends"]+packages)
class RedHatPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:IzOEf):
  IzOEL(RedHatPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[IzOEf,List[IzOEf]]):
  packages=packages if IzOEk(packages,List)else[packages]
  with INSTALL_LOCK:
...
class DebianPostgres11Installer(DebianPackageInstaller):
 def __init__(self):
  IzOEL(DebianPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->IzOEB:
...
class RedHatPostgres11Installer(RedHatPackageInstaller):
 def __init__(self):
  IzOEL(RedHatPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->IzOEB:
  return is_command_available("psql")
 def _prepare_installation(self)->IzOED:
...
class DebianPlPythonInstaller(DebianPackageInstaller):
 def __init__(self):
  IzOEL(DebianPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->IzOEB:
...
class RedHatPlPythonInstaller(RedHatPackageInstaller):
 def __init__(self):
  IzOEL(RedHatPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->IzOEB:
...
class DebianMariaDBInstaller(DebianPackageInstaller):
 def __init__(self):
  IzOEL(DebianMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->IzOEB:
...
class RedHatMariaDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  IzOEL(RedHatMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->IzOEB:
  return is_command_available("mysqld")
 def _install_package(self)->IzOED:
...
class DebianTimescaleDBInstaller(DebianPackageInstaller):
 def __init__(self):
  IzOEL(DebianTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->IzOEB:
...
class RedHatTimescaleDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  IzOEL(RedHatTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->IzOEB:
...
class DebianRedisInstaller(DebianPackageInstaller):
 def __init__(self):
  IzOEL(DebianRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->IzOEB:
...
class RedHatRedisInstaller(RedHatPackageInstaller):
 def __init__(self):
  IzOEL(RedHatRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->IzOEB:
  return is_command_available("redis-server")
 def _install_package(self)->IzOED:
...
class DebianMosquittoInstaller(DebianPackageInstaller):
 def __init__(self):
  IzOEL(DebianMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->IzOEB:
...
class RedHatMosquittoInstaller(RedHatPackageInstaller):
 def __init__(self):
  IzOEL(RedHatMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->IzOEB:
  return is_command_available("mosquitto")
 def _install_package(self)->IzOED:
