...
SENT_EMAILS=[]
EMAIL_BLACKLIST=MxOzC()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",MxOzg(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=MxOze,from_name=MxOze,smtp_host=MxOze,smtp_user=MxOze,smtp_pass=MxOze,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if MxOzQ(recipients,MxOzo)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=MxOze):
 ...
 result["From"]=formataddr((MxOzg(Header(from_name,"utf-8")),from_email))
 return result
