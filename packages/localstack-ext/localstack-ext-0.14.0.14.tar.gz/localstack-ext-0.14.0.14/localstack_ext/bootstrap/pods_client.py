...
class PodInfo:
 def __init__(self,name=YKnFq,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(YKnFd):
 BACKEND="_none_"
 def __init__(self,pod_name=YKnFq,config=YKnFq):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise YKnFH("Not implemented")
 def delete(self,remote:YKnFa)->YKnFa:
  raise YKnFH("Not implemented")
 def push(self,comment:YKnFR=YKnFq,three_way:YKnFa=YKnFr)->PodInfo:
  raise YKnFH("Not implemented")
 def push_overwrite(self,version:YKnFU,comment:YKnFR=YKnFq):
  pass
 def pull(self,inject_version_state:YKnFa=YKnFr,reset_state_before:YKnFa=YKnFr,lazy:YKnFa=YKnFr):
  raise YKnFH("Not implemented")
 def commit(self,message:YKnFR=YKnFq):
  raise YKnFH("Not implemented")
 def inject(self,version:YKnFU,reset_state:YKnFa)->YKnFa:
  raise YKnFH("Not implemented")
 def list_versions(self)->List[YKnFR]:
  raise YKnFH("Not implemented")
 def version_info(self,version:YKnFU):
  raise YKnFH("Not implemented")
 def version_metamodel(self,version:YKnFU)->Dict:
  raise YKnFH("Not implemented")
 def set_version(self,version:YKnFU,inject_version_state:YKnFa,reset_state:YKnFa,commit_before:YKnFa):
  raise YKnFH("Not implemented")
 def list_version_commits(self,version:YKnFU)->List[YKnFR]:
  raise YKnFH("Not implemented")
 def get_commit_diff(self,version:YKnFU,commit:YKnFU)->Dict:
  raise YKnFH("Not implemented")
 def register_remote(self,pod_name:YKnFR,ci_pod:YKnFa)->YKnFa:
  raise YKnFH("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->YKnFa:
  raise YKnFH("Not implemented")
 def list_pods(self,fetch_remote:YKnFa)->Set[YKnFR]:
  raise YKnFH("Not implemented")
 ...
 def get(cls,pod_name,pre_config=YKnFq):
  ...
  raise YKnFH('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=YKnFr
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=YKnFC)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=YKnFC
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise YKnFH("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @YKnFo
 def get_state_zip_from_instance(get_content=YKnFr):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise YKnFH("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:YKnFR=YKnFq):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @YKnFo
 def parse_pod_name_from_qualifying_name(qualifying_name:YKnFR)->YKnFR:
 ...
 @YKnFo
 def _get_max_version_for_pod_from_platform(pod_name:YKnFR,auth_headers):
  ...
  remote_max_ver=YKnFU(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with YKnFL(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with YKnFL(meta_archive,"rb")as meta_archive_content:
   ...
   with YKnFL(state_archive,"rb")as state_archive_content:
    self.upload_content(state_presigned_url,state_archive_content)
 @YKnFo
 def _add_state_files_from_directory(service:YKnFR,path:YKnFR,root_dir:YKnFR,serialization:Serialization):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _add_state_to_cpvcs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=YKnFq)
  ...
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:YKnFR):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _clone_pod(self,auth_headers,lazy:YKnFa=YKnFr):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:YKnFa)->YKnFa:
  ...
  if remote:
   pass
  return YKnFr
 def _push_to_remote(self,url:YKnFR):
 ...
 def pull(self,inject_version_state:YKnFa=YKnFr,reset_state_before:YKnFa=YKnFr,lazy:YKnFa=YKnFr):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=YKnFr):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if not lazy:
    required_versions=",".join(YKnFM(lambda ver:YKnFR(ver),YKnFQ(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def _download_version_product(self,version:YKnFU,retain:YKnFa=YKnFr)->Dict[YKnFR,YKnFR]:
 ...
 def _get_presigned_url_for_version_product(self,version:YKnFU)->Dict:
 ...
 def _inject_from_remote(self,version:YKnFU,retain:YKnFa=YKnFr)->YKnFa:
  from localstack_ext.bootstrap.cpvcs.utils.remote_utils import(extract_meta_and_state_archives)
  content=self._get_presigned_url_for_version_product(version=version)
  state_url=content.get("presigned_url_state")
  tmp_state_archive=new_tmp_file()
 ...
 def inject(self,version:YKnFU,reset_state:YKnFa)->YKnFa:
  ...
  if not cpvcs_api.config_context.pod_exists_locally(self.pod_name):
   ...
   if not state_archive_path and cpvcs_api.is_remotely_managed():
    ...
    product_archive_path=self._download_version_product(version=version,retain=YKnFC)
    if not product_archive_path:
     return YKnFr
    state_archive_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if reset_state:
   reset_local_state(reset_data_dir=YKnFC)
 ...
 def version_metamodel(self,version:YKnFU)->Dict:
  ...
  if not result and cpvcs_api.is_remotely_managed():
   self._download_version_product(version=version,retain=YKnFC)
 ...
 def get_commit_diff(self,version:YKnFU,commit:YKnFU)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:YKnFR,ci_pod:YKnFa=YKnFr)->YKnFa:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def list_pods(self,fetch_remote:YKnFa)->Set[YKnFR]:
  ...
  return result
 @YKnFf
 def upload_content(cls,presigned_url:YKnFR,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise YKnFH("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @YKnFo
 def create_platform_url(request:YKnFR)->YKnFR:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:YKnFR=YKnFq,three_way:YKnFa=YKnFr)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=YKnFC)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(YKnFe(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=YKnFe(zip_data_content)
  return result
 def pull(self,inject_version_state:YKnFa=YKnFr,reset_state_before:YKnFa=YKnFr,lazy:YKnFa=YKnFr):
  ...
  rm_rf(zip_path)
 @YKnFo
 def presigned_url(pod_name:YKnFR,mode:YKnFR)->YKnFR:
  ...
  if response.status_code>=400:
   raise YKnFH("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise YKnFH("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:YKnFR=YKnFq,three_way:YKnFa=YKnFr):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=YKnFC)
 ...
 def pull(self,inject_version_state:YKnFa=YKnFr,reset_state_before:YKnFa=YKnFr,lazy:YKnFa=YKnFr):
  ...
  if remote_ref not in remote_refs:
   raise YKnFH('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=YKnFS(self,"clone_dir",YKnFq)
...
class PodConfigManagerMeta(YKnFm):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=YKnFq
   for manager in cls.CHAIN:
    try:
     tmp=YKnFS(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif YKnFD(tmp,YKnFi)and YKnFD(result,YKnFi):
       result.extend(tmp)
    except YKnFH:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not YKnFq:
    return result
   raise YKnFH('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(YKnFd,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @YKnFf
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise YKnFH('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(YKnFd):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=YKnFC).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=YKnFj([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=YKnFr):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except YKnFH:
   if safe:
    return{}
   raise YKnFH('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(YKnFd):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise YKnFH("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise YKnFH("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:YKnFR,pre_config:Dict[YKnFR,YKnFR],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:YKnFR,remote:YKnFa,pre_config:Dict[YKnFR,YKnFR])->YKnFa:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:YKnFR,pre_config:Dict[YKnFR,YKnFR],**kwargs)->YKnFa:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",YKnFr))
 return result
def rename_pod(current_pod_name:YKnFR,new_pod_name:YKnFR,pre_config:Dict[YKnFR,YKnFR],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:YKnFa,pre_config:Dict[YKnFR,YKnFR],**kwargs)->List[YKnFR]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:YKnFR,pre_config:Dict[YKnFR,YKnFR],message:YKnFR=YKnFq,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:YKnFR,version:YKnFU,reset_state:YKnFa,pre_config:Dict[YKnFR,YKnFR],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.inject(version=version,reset_state=reset_state)
 return result
def list_versions(pod_name:YKnFR,pre_config:Dict[YKnFR,YKnFR],**kwargs)->List[YKnFR]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:YKnFU,pod_name:YKnFR,pre_config:Dict[YKnFR,YKnFR],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:YKnFU,pod_name:YKnFR,pre_config:Dict[YKnFR,YKnFR],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:YKnFU,inject_version_state:YKnFa,reset_state:YKnFa,commit_before:YKnFa,pod_name:YKnFR,pre_config:Dict[YKnFR,YKnFR],**kwargs)->YKnFa:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:YKnFU,pod_name:YKnFR,pre_config:Dict[YKnFR,YKnFR])->List[YKnFR]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:YKnFU,commit:YKnFU,pod_name:YKnFR,pre_config:Dict[YKnFR,YKnFR])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:YKnFU,pod_name:YKnFR,comment:YKnFR,pre_config:Dict[YKnFR,YKnFR]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=YKnFq,squash_commits=YKnFr,comment=YKnFq,three_way=YKnFr,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=YKnFr,reset_state_before=YKnFr,lazy=YKnFr,**kwargs):
 pre_config=kwargs.get("pre_config",YKnFq)
 if not pod_name:
  raise YKnFH("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before,lazy=lazy)
 print("Done.")
def reset_local_state(reset_data_dir=YKnFr,exclude_from_reset:List[YKnFR]=YKnFq):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise YKnFH("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->YKnFR:
 try:
...
def get_persisted_resource_names(data_dir)->List[YKnFR]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and YKnFe(os.listdir(entry.path))>0:
    names.append(entry.name)
