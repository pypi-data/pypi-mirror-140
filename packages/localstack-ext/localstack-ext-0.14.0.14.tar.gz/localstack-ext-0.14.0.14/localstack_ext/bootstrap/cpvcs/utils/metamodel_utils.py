...
CAPTURE_METAMODEL_SEPARATELY=CWlIx
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=CWlIY(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=CWlIY(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->CWlIz:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[CWlIz,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,CWlIO)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(CWlIO,state_files_to)
...
class CommitMetamodelUtils:
 @CWlIP
 def create_metadata_archive(cls,version:Version,delete_reference:bool=CWlIx,overwrite:bool=CWlIx,metamodels_file:CWlIz=CWlIO):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @CWlIP
 def create_delta_metamodel_file(cls,version:CWlIJ,revision:CWlIJ,store_to_zip=CWlIx)->Tuple[Dict,Dict]:
 ...
 @CWlIP
 def reconstruct_metamodel(cls,version:CWlIJ,revision:CWlIJ=CWlIO)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in CWlIm(1,revision+1):
  ...
  return result
 @CWlIP
 def get_version_metamodel(cls,version:CWlIJ,revision:CWlIJ=CWlIO)->Dict:
 ...
 @CWlIP
 def get_commit_diff(cls,version_no:CWlIJ,commit_no:CWlIJ)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->CWlIU:
 if CWlIX(clazz,CWlIw):
  return{}
 constructor=CWlIj(clazz,"__init__",CWlIO)
...
def _create_metamodel_helper(obj,width=25,visited:set=CWlIO):
 if obj is CWlIO:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not CWlIO else obj
 if CWlIX(obj,CWlIw):
  result=CWlIw(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif CWlIX(obj,CWlIS):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if CWlIK(result)>width:
   result={"size":CWlIK(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:CWlIz)->CWlIJ:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
