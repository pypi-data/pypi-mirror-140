...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:IUOQL=IUOQy)->IUOQn:
 ...
 def update_ver_log(self,author:IUOQn,ver_no:IUOQs,rev_id:IUOQn,rev_no:IUOQs):
  with IUOQX(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:IUOQn,key:IUOQn=IUOQN)->IUOQn:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:IUOQn,key:IUOQn=IUOQN)->IUOQn:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->IUOQL:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:IUOQn,key:IUOQn,path:IUOQn)->IUOQn:
...
def zip_directories(zip_dest:IUOQn,directories:List[IUOQn])->IUOQn:
 ...
 return zip_dest
def add_file_to_archive(archive:IUOQn,entry_name:IUOQn,content:Union[IUOQn,IUOQf]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:IUOQn,file_name:IUOQn)->IUOQn:
 try:
