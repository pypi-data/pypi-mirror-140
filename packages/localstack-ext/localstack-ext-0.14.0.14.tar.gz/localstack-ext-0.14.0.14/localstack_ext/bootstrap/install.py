...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=IcQKE)
 timescaledb_installer.install(raise_on_error=IcQKE)
 redis_installer.install(raise_on_error=IcQKE)
 mosquitto_installer.install(raise_on_error=IcQKE)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:IcQKg,target_file:IcQKg,timeout:int=IcQKq):
 def do_download(url,print_error=IcQKE):
  try:
