...
class DecryptionHandler:
 decryption_key:OnFqE
 def __init__(self,decryption_key:OnFqE):
  self.decryption_key=decryption_key
 def decrypt(self,content)->OnFqE:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=OnFqs):
  if path and not OnFqD(path,OnFqG):
   path=OnFqG(OnFqM(path,"_path",[]))
  if not path:
   return OnFqs
...
def patch_traceback_lines():
 if OnFqM(traceback.FrameSummary,"_ls_patch_applied",OnFqs):
  return
 @OnFqd
 def line(self):
  try:
 ...
 OnFqj(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=OnFqV
def patch_inspect_findsource():
 if OnFqM(inspect,"_ls_patch_applied",OnFqs):
  return
 ...
 inspect._ls_patch_applied=OnFqV
