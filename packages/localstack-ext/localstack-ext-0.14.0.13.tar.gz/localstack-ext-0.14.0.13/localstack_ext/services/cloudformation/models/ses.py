...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[zEcJw])[0]
 def get_physical_resource_id(self,attribute=zEcJw,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if zEcJL(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return zEcJI(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @zEcJV
 def add_defaults(resource,stack_name:zEcJp):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return zEcJI(SESReceiptRule,self).get_cfn_attribute(attribute)
 @zEcJP
