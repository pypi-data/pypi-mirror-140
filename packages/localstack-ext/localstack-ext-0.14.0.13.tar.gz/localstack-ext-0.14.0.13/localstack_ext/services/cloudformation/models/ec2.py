...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=NhSxb,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if NhSxU(perm["IpProtocol"])!=NhSxU(rp["IpProtocol"]):
    continue
 ...
 @NhSxc
 def get_vpc_and_name_for_security_group(cls,group_id:NhSxU)->Tuple[NhSxU,NhSxU]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or NhSxb
  group_name=groups and groups[0]["GroupName"]or NhSxb
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",NhSxb)
   vpc_id=NhSxb
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",NhSxb)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=NhSxU(result.get("IpProtocol"))
   description=result.pop("Description",NhSxb)
   cidr_ipv6=result.pop("CidrIpv6",NhSxb)
