...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or wghSv
 @wghSx
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return wghSO(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @wghSt
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if wghSU(origins,wghSc)else origins.get("Items")
  origins_to_create=wghSX([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if wghSU(aliases,wghSc)else aliases.get("Items")
  aliases_to_create=wghSX(aliases)
  for distr in distrs:
   origins=wghSX([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=wghSX(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",wghSf)
   config["Enabled"]=wghSa(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if wghSU(custom_errors,wghSM)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=wghSa(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":wghSf}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=wghSa(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=wghSv):
    parent=config if parent is wghSv else parent
    items=parent.get(key)or[]
    if wghSU(items,wghSM)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":wghSL(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if wghSU(cache_behavs,wghSM)else cache_behavs)
