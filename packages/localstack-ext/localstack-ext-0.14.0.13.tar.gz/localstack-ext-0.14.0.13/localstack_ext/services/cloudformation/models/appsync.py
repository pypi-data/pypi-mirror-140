...
class AppSyncResolver(GenericBaseModel):
 ...
 def fetch_details(cls,api_id,type_name,field_name,data_source_name):
  ...
  return(result or[tPRxB])[0]
 @tPRxy
 def get_deploy_templates():
  def get_resolver_params(params,**kwargs):
   params=keys_to_lower(clone(params))
   req_template=params.pop("requestMappingTemplateS3Location",tPRxB)
   res_template=params.pop("responseMappingTemplateS3Location",tPRxB)
   if req_template:
    tmpl=aws_utils.download_s3_object(req_template,as_str=tPRxc)
    if tmpl:
     params["requestMappingTemplate"]=tmpl
   if res_template:
    tmpl=aws_utils.download_s3_object(res_template,as_str=tPRxc)
    if tmpl:
...
class GraphQLSchema(GenericBaseModel):
 ...
 @tPRxu
 def fetch_details(cls,api_id,definition=tPRxB,s3_location=tPRxB):
  client=aws_stack.connect_to_service("appsync")
  try:
   schema=client.get_introspection_schema(apiId=api_id,format="SDL")["schema"]
   schema=schema.read()if tPRxS(schema,"read")else schema
...
class AppSyncApiKey(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return tPRxY(AppSyncApiKey,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute=tPRxB,**kwargs):
  props=self.props
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=[k for k in keys if k.get("description")==description and expires in[tPRxB,k.get("expires")]]
  return(result or[tPRxB])[0]
 @tPRxu
 def get_deploy_templates(cls):
  def _create_params(_,resources,resource_id,**kwargs):
   ...
   if res_props.get("Expires"):
    res_props["Expires"]=tPRxU(res_props.get("Expires"))
...
class AppSyncDataSource(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return tPRxY(AppSyncDataSource,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute=tPRxB,**kwargs):
  return self.props.get("dataSourceArn")
 def fetch_state(self,stack_name,resources):
  props=self.props
  ...
  return([s for s in sources if s["name"]==ds_name]or[tPRxB])[0]
class AppSyncFunctionConfig(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return tPRxY(AppSyncFunctionConfig,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute,**kwargs):
...
class GraphQLAPI(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  if not props.get("apiId"):
   ...
   if not details:
    raise tPRxq('Unable to find GraphQL API named "%s"'%props.get("Name"))
  ...
  if attribute_name=="GraphQLUrl":
   return appsync_api.get_graphql_url(props["apiId"],use_domain_name=tPRxc)
  return tPRxY(GraphQLAPI,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute,**kwargs):
  props=self.props
 ...
 def fetch_details(cls,api_name):
  ...
  return([a for a in apis if a["name"]==api_name]or[tPRxB])[0]
