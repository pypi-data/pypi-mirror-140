...
SENT_EMAILS=[]
EMAIL_BLACKLIST=jQUMs()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",jQUMO(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=jQUMl,from_name=jQUMl,smtp_host=jQUMl,smtp_user=jQUMl,smtp_pass=jQUMl,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if jQUMg(recipients,jQUML)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=jQUMl):
 ...
 result["From"]=formataddr((jQUMO(Header(from_name,"utf-8")),from_email))
 return result
