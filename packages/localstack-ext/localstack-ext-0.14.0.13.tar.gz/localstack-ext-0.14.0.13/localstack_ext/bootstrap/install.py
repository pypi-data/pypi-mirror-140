...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=mnuTj)
 timescaledb_installer.install(raise_on_error=mnuTj)
 redis_installer.install(raise_on_error=mnuTj)
 mosquitto_installer.install(raise_on_error=mnuTj)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:mnuTA,target_file:mnuTA,timeout:int=mnuTE):
 def do_download(url,print_error=mnuTj):
  try:
