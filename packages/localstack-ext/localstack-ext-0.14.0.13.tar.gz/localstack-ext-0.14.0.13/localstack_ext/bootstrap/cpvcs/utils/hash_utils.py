...
def random_hash()->WpOaB:
 return WpOaH(random.getrandbits(160))
def compute_file_hash(file_path:WpOaB,accum=WpOaE)->Optional[WpOaB]:
 try:
  with WpOaj(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:WpOaG,state_archive:WpOaB)->WpOaB:
 ...
 m.update(WpOaB(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->WpOaB:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=WpOaF(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with WpOaj(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except WpOat as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if WpOag(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(WpOaB(cpvcs_node.revision_number).encode("utf-8"))
 elif WpOag(cpvcs_node,Version):
  m.update(WpOaB(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
