...
CAPTURE_METAMODEL_SEPARATELY=YCWae
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=YCWaA(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=YCWaA(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->YCWaE:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[YCWaE,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,YCWad)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(YCWad,state_files_to)
...
class CommitMetamodelUtils:
 @YCWaG
 def create_metadata_archive(cls,version:Version,delete_reference:bool=YCWae,overwrite:bool=YCWae,metamodels_file:YCWaE=YCWad):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @YCWaG
 def create_delta_metamodel_file(cls,version:YCWaj,revision:YCWaj,store_to_zip=YCWae)->Tuple[Dict,Dict]:
 ...
 @YCWaG
 def reconstruct_metamodel(cls,version:YCWaj,revision:YCWaj=YCWad)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in YCWal(1,revision+1):
  ...
  return result
 @YCWaG
 def get_version_metamodel(cls,version:YCWaj,revision:YCWaj=YCWad)->Dict:
 ...
 @YCWaG
 def get_commit_diff(cls,version_no:YCWaj,commit_no:YCWaj)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->YCWam:
 if YCWaR(clazz,YCWaQ):
  return{}
 constructor=YCWaI(clazz,"__init__",YCWad)
...
def _create_metamodel_helper(obj,width=25,visited:set=YCWad):
 if obj is YCWad:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not YCWad else obj
 if YCWaR(obj,YCWaQ):
  result=YCWaQ(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif YCWaR(obj,YCWaD):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if YCWav(result)>width:
   result={"size":YCWav(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:YCWaE)->YCWaj:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
