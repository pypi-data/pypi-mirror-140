...
ROOT_DIR_LOOKUP={aHcPL(Serialization.KINESIS):KINESIS_DIR,aHcPL(Serialization.DDB):DYNAMODB_DIR,aHcPL(Serialization.MAIN):API_STATES_DIR}
def init(pod_name="My-Pod"):
 ...
 r0_hash=random_hash()
 v0_hash=random_hash()
 r0=Revision(hash_ref=r0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0,state_files=aHcPF())
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=aHcPV,state_files=aHcPF(),version_number=0)
 ...
 with aHcPr(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with aHcPr(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with aHcPr(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:aHcPL,meta_archives:Dict[aHcPL,aHcPL],state_archives:Dict[aHcPL,aHcPL],remote_info:Dict[aHcPL,aHcPL],pod_name:aHcPL):
...
def merge_from_remote(version_space_archive:aHcPL,meta_archives:Dict[aHcPL,aHcPL],state_archives:Dict[aHcPL,aHcPL]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->aHcPs:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:aHcPL):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:aHcPL)->aHcPs:
...
def list_locally_available_pods(show_remote_or_local:aHcPs=aHcPd)->Set[aHcPL]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:aHcPL,file_name:aHcPL,service:aHcPL,region:aHcPL,root:aHcPL,serialization:Serialization,account_id=TEST_AWS_ACCOUNT_ID)->aHcPL:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->aHcPL:
 ...
 return key
def _get_state_file_path(key:aHcPL)->aHcPL:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=aHcPF(aHcPS(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:aHcPL)->Optional[aHcPL]:
...
def get_version_info(version_no:aHcPU)->Dict[aHcPL,aHcPL]:
 ...
 return result
def commit(message:aHcPL=aHcPV)->Revision:
 ...
 curr_expansion_point_parent_state_files=aHcPF()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=aHcPV
 ...
 new_expansion_point=Revision(hash_ref=random_hash(),state_files=aHcPF(),parent_ptr=curr_expansion_point_hash,creator=curr_expansion_point.creator,rid=short_uid(),revision_number=curr_expansion_point.revision_number+1)
...
def create_version_space_archive()->aHcPL:
...
def get_max_version_no()->aHcPU:
 with aHcPr(config_context.get_max_ver_path())as fp:
  return aHcPU(os.path.basename(fp.readline()))
...
def push_overwrite(version:aHcPU,comment:aHcPL)->aHcPs:
 ...
 CommitMetamodelUtils.create_metadata_archive(version_node,overwrite=aHcPd,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return aHcPd
def push(comment:aHcPL=aHcPV,three_way:aHcPs=aHcPN)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=aHcPF(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,PodFilePaths.get_version_state_archive(new_max_version_no)),state_files=aHcPF(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 if expansion_point.parent_ptr!=NIL_PTR:
  ...
  delta_log_ptr=create_delta_log(expansion_point.state_files,aHcPF())
 ...
 return new_version
def _get_dst_path_for_state_file(version_state_dir:aHcPL,state_file:StateFileRef):
 if state_file.serialization in[aHcPL(Serialization.KINESIS),aHcPL(Serialization.DDB)]:
 ...
 mkdir(dst_path)
 return dst_path
def _create_state_directory(version_number:aHcPU,state_file_refs:Set[StateFileRef],delete_files=aHcPN,archive=aHcPd):
...
def set_active_version(version_no:aHcPU,commit_before=aHcPN)->aHcPs:
 ...
 return aHcPN
def _set_active_version(key:aHcPL):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=aHcPF(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:aHcPU)->Version:
 versions=load_version_references()
 version_ref=aHcPf((version[1]for version in versions if version[0]==version_no),aHcPV)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[aHcPU,aHcPL]]:
 result={}
 with aHcPr(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with aHcPr(symlink,"r")as sp:
    result[aHcPU(os.path.basename(symlink))]=sp.readline()
 return aHcPB(result.items(),key=lambda x:x[0],reverse=aHcPd)
def list_versions()->List[aHcPL]:
 ...
 return result
def list_version_commits(version_no:aHcPU)->List[aHcPL]:
 if version_no==-1:
 ...
 return result
def _update_head(new_head_ver_no,new_head_key)->aHcPL:
 with aHcPr(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->aHcPL:
 with aHcPr(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->aHcPL:
 with aHcPr(config_context.get_known_ver_path(),"a")as fp:
...
def merge_expansion_point_with_max(three_way=aHcPN):
 ...
 for serialization_mechanism,new_version_state_files_path in new_version_tmp_dirs.items():
  try:
   merge_manager=CPVCSMergeManager.get(serialization_mechanism,raise_if_missing=aHcPd)
...
def create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef],diff_method:MetamodelDeltaMethod=MetamodelDeltaMethod.SIMPLE)->aHcPL:
 try:
