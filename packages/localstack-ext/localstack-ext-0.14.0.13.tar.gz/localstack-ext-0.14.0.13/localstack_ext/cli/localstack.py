...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return NiSKk if e else NiSKO
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->NiSKJ:
...
@click.option("--revert",is_flag=NiSKk,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:NiSKs):
...
def _cpvcs_initialized(pod_name:NiSKV)->NiSKs:
...
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_init(name:NiSKV):
...
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=NiSKk,default=NiSKO)
def cmd_cpvcs_delete(name:NiSKV,remote:NiSKs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_register_remote(name:NiSKV):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=NiSKk)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=NiSKk)
def cmd_cpvcs_rename(name:NiSKV,new_name:NiSKV):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_commit(message:NiSKV,name:NiSKV):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=NiSKk,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=NiSKk,default=NiSKO,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_push(squash:NiSKs,message:NiSKV,name:NiSKV,three_way:NiSKs):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
@click.option("-v","--version",type=NiSKa)
@click.option("-m","--message",required=NiSKO)
def cmd_cpvcs_push_overwrite(version:NiSKa,message:NiSKV,name:NiSKV):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=NiSKa,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=NiSKk,default=NiSKO,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_inject(version:NiSKa,reset:NiSKs,name:NiSKV):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=NiSKk,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=NiSKk,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
@click.option("--lazy/--eager",default=NiSKk,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:NiSKV,inject:NiSKs,reset:NiSKs,lazy:NiSKs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=NiSKk,default=NiSKO)
def cmd_cpvcs_list_pods(remote:NiSKs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_versions(name:NiSKV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=NiSKk,type=NiSKa)
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_version_info(version:NiSKa,name:NiSKV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=NiSKa,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_version_metamodel(version:NiSKa,name:NiSKV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=NiSKk,type=NiSKa,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=NiSKk,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=NiSKk,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=NiSKO,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_set_version(version:NiSKa,inject:NiSKs,reset:NiSKs,commit_before:NiSKs,name:NiSKV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_commits(version:NiSKa,name:NiSKV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=NiSKk)
@click.option("--commit","-c",required=NiSKk)
@click.option("-n","--name",help="Name of the cloud pod",required=NiSKk)
def cmd_cpvcs_commit_diff(version:NiSKa,commit:NiSKa,name:NiSKV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(NiSKJ)
  status.stop()
...
def print_table(columns:Dict[NiSKV,NiSKV],rows:List[Dict[NiSKV,Any]],formatters:Dict[NiSKV,Callable[[Any],NiSKV]]=NiSKJ):
 from rich.table import Table
 if formatters is NiSKJ:
  formatters=NiSKz()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  NiSKo(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->NiSKJ:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->NiSKJ:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->NiSKJ:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->NiSKJ:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
