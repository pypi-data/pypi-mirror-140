...
delete_service_parser.add_argument('service_id', help='Service ID to delete', type=int)
...
async def handle_delete(args):
    if args.delete_type == 'service':
        # delete the service without confirmation if force is set
        if args.force:
            await client.delete_service(args.service_id)
            print(f"Service {args.service_id} has been deleted")
            return
        # fetch the service to get the name
        service = await client.fetch_service(args.service_id)
        # ask for confirmation
        print(f"Deleting service {service['name']}. Are you sure? (y/N)", end='')
        if input().lower() == 'y':
            await client.delete_service(service['id'])
            print(f"Service {service['name']} has been deleted")
    else:
        raise Exception(f"Unknown type: {args.delete_type}")
