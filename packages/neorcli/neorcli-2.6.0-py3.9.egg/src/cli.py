...
list_service_parser = list_subparsers.add_parser('service', help='List services')
list_service_parser.add_argument('project_id', help='Project ID to list services in', type=int)
...
async def handle_list(args):
    if args.list_type == 'service':
        services = await client.list_services(args.project_id)
        services = services['results']
        for service in services:
            print(f"{service['name']} service has ID {service['id']}")
