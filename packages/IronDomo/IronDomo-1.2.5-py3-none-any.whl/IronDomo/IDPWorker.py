...
class IronDomoWorker(object):
    ...
    def __init__(self, broker, service, verbose=False, credentials=None, workload = None, idle_timeout = None, identity = None, unique=False):
        self.alive = True
        self.last_message = time.time() 
        self.broker = broker
        self.service = service
        self.unique = unique
        self.identity = '{0}_{1}'.format(self.service.decode(), 0 if unique else b64encode(os.urandom(5)).decode()) if identity == None else identity
