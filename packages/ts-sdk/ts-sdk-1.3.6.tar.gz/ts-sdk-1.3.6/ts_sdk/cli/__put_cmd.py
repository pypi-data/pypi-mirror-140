...
def put_cmd_args(parser: argparse.ArgumentParser):
    parser.add_argument('type', type=str, choices=[
        'ids', 'protocol', 'master-script', 'task-script'],
                        help='artifact type')
    ...
    parser.add_argument('folder', type=__folder_type,
    ...
    parser.add_argument(
        '--ignore-ssl', '-i', action='store_true',
    ...
    parser.add_argument(
        '--config', '-c', help='JSON file with configuration',
...
def __namespace_type(arg_value: str,
                     ns_pattern=re.compile("^private-[0-9a-zA-Z-]+$")) -> str:
    """
    ...
    # validate namespace
    if (not ns_pattern.match(arg_value) or
            arg_value.endswith("-") or
            re.search(r"--", arg_value)):
...
def __cmd(args):
    ...
    config_to_show = {
        key_to_show: args.__dict__[key_to_show]
        for key_to_show in keys_to_show
    }
    if isinstance(config_to_show.get('auth_token'), str):
        config_to_show.update(
            {'auth_token': f'{config_to_show["auth_token"][0:7]}...'}
        )
    ...
    if args.type == 'task-script':
        ...
        print("Note: A local script interruption doesn't stop a remote build!",
              flush=True)
