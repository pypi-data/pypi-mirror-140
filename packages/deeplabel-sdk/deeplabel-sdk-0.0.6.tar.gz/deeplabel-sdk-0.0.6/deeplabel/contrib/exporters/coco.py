    ...
    def run(
        ...
        for image in gallery.images:
            if not image.detections:
                logger.debug(f"No detections for image {image.image_id}. Skipping")
...
class CocoExporter:
    ...
    client: "deeplabel.client.BaseClient"
    append:bool = True
    existing_dataset:CocoDataset = dataclasses.field(init=False)

    def __post_init__(self):
        dataset_path = os.path.join(self.root_dir, "dataset.json")
        if os.path.exists(dataset_path):
            with open(dataset_path, 'r') as f:
                existing_dataset = json.load(f)
            self.existing_dataset = CocoDataset(**existing_dataset)
        else:
            self.existing_dataset = CocoDataset(categories=[],annotations=[],images=[])
    ...
    def export(self, folder: deeplabel.label.folders.RootFolder):
        ...
        if self.append:
            dataset = CocoDataset(
                categories=self.merge_docs_by_id(self.existing_dataset.categories, dataset.categories),
                annotations=self.merge_docs_by_id(self.existing_dataset.annotations, dataset.annotations),
                images=self.merge_docs_by_id(self.existing_dataset.images, dataset.images)
            )
        
        dataset_path = os.path.join(self.root_dir, "annotations.json")
