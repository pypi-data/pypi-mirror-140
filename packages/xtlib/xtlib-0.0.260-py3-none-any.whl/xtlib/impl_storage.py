...
class ImplStorage(ImplBase):
    ...
    def plot(self, runs, col_list, x, layout, break_on, title, show_legend, plot_titles, legend_titles, 
        ...
        if group_by and (first or last):
            ...
            run_names_list = self.store.database.get_group_query_run_names(workspace, job_id, group_by, agg_name, sort_col, first, last)
    ...
    @option(name="hp-set", default="hidden", help="controls how hp_set columns are displayed", values=["hidden", "simple", "changed", "full", "columns"])
