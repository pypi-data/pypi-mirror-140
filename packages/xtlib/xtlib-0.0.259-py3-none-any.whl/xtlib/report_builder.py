...
class ReportBuilder():
    ...
    def build_report(self, records, user_columns, report_type, args):
        ...
        self.build_hp_set_names(records)

        if True:       # hp_set_display == "hidden":
        ...
        if hp_set_display == "compressed":
            user_columns.append("hp_set:$hpset")

        elif hp_set_display == "full":
            user_columns.append("hp_set")

        elif hp_set_display == "columns":
            if self.hp_common_set:
                # for each run record, convert hp_set column into new columns in the record 
                for record in records:
                    hp_set_str = record["hp_set"]
                    hp_set = self.format_hpset(hp_set_str)
    ...
    def format_hpset(self, value):
        ...
        new_value = self.hp_set_names[value]
        return new_value
    ...
    def build_hp_set_names(self, records):
        ...
        self.hp_set_names = hp_set_names
    ...
    def xt_custom_format(self, fmt, value):
        ...
        elif fmt in [hpset, hpset_cols]:
            value = str(self.format_hpset(value))
            align = "<"
