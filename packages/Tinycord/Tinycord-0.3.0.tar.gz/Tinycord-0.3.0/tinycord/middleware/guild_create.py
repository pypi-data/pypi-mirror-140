...
async def guild_create(client: "Client", gateway: "Gateway", event: "GatewayDispatch") -> typing.List[typing.Awaitable]:
    ...
    for id, thread in guild.threads.items():
        client.threads[id] = thread
        """ The threads that were should be cached. """

    for id, role in guild.roles.items():
        client.roles[id] = role
        """ The roles that were should be cached. """
    ...
    if guild.id in client.unavailable_guilds: # Seeing if the guild id in a speaicl array where cached guilds id saved
        ...
        if len(client.unavailable_guilds) == 0:
            ...
            await client.dispatch('on_ready', gateway)
            """ Calling the on_ready event. """

        return "on_guild_cache", [
            guild
        ]
        """ The event that will dispatched. """
