...
class ThinkDataQuery(object):
    ...
    def get_data_raw_pyhive(self, ctx, sql, block_size=100000, fetch_size=10000, retry_count=5, read_timeout=60):
        '''
        ...
        for i in range(retry_count):
            try:
                result = self.get_data_raw_pyhive_i(
                    ctx, unique_path, sql, block_size, fetch_size, read_timeout)
                return result
            except Exception as e:
                gol_e = e
    ...
    def get_data_raw_pyhive_i(self, ctx, unique_path, sql, block_size=100000, fetch_size=10000, read_timeout=60):
        '''
        @des: 内部调用
        '''
