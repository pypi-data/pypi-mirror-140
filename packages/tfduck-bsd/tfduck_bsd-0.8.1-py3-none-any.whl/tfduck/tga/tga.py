...
class ThinkDataQuery(object):
        ...
        for i in range(5):
            try:
                result = self.get_data_raw_pyhive_i(
                    ctx, unique_path, sql, block_size, fetch_size)
                return result
            except Exception as e:
                gol_e = e
    ...
    def get_data_raw_pyhive_i(self, ctx, unique_path, sql, block_size=100000, fetch_size=10000):
        '''
        @des: 内部调用
        '''
        ...
        try:
            conn = presto.connect(host=self.hive_conn_info[0],
                                  ...
                                  requests_kwargs={"timeout": (
                                      30, 30), "stream": True, "verify": False}
                                  )
            cursor = conn.cursor()
            cursor.execute(sql)
            BMOBJ.clog(ctx, "文件大小")
