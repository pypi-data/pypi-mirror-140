...
class QualityMixin:
    ...
    def quality_build_report(self) -> List[dict]:
        ...
        report = self.task_dependent_metrics(report)
    ...
    def task_dependent_metrics(self, report: List[dict]) -> List[dict]:
        """
        Split task dependent metrics into a separate method so it can be overwritten by instruments
        if desired
        """
