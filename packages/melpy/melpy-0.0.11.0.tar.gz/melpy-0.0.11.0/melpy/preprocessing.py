...
class BFFImputer():
    def evaluate(self, pos):
        ...
        for i in range(len(self.X[0,:])):
            self.diffs[pos[0],i] = self.X[pos[0], i]
            
        for i in range(len(self.X[:,0])):
            for j in range(len(self.X[0,:])):
                if j != pos[1]: 
                    self.diffs[i,j] = np.absolute(self.X[pos[0], j] - self.X[i,j])
                    
        for i in range(self.scores.shape[0]):
            self.scores[i] = np.sum(self.diffs[i])
        
        return [np.argsort(self.scores, axis=0)[i] for i in range(self.scores.shape[0]//2) if np.isnan(self.X[np.argsort(self.scores, axis=0)[i][0], self.col]) == False]
    ...
    def transform(self, X, column):
        ...
        for i in range(len(self.nans)):
            self.friends = self.evaluate(self.nans[i])
            self.X[self.nans[i][0], self.nans[i][1]] = self.X[self.friends[0][0], self.col]
