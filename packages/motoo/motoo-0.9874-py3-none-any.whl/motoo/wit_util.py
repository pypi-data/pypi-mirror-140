...
def get_std_type(wit_data_list):
    ...
    for a_wit in wit_data_list:
        if isinstance(a_wit, WitData):
            for i in range(2):
                index = len(WIT_DATA_TYPE_LINK[i])
                try:
                    index = WIT_DATA_TYPE_LINK[i].index(a_wit.get_type)
                except: pass
                std_types[i] = std_types[i] if WIT_DATA_TYPE_LINK[i].index(std_types[i]) < index else a_wit.get_type
    if std_types[0] and std_types[1]:
        return WitDataType.DAY_TYPE
    elif std_types[0]:
        return std_types[0]
    elif std_types[1]:
        return std_types[1]
...
class WitData:
    ...
    def get_type(self):
        ...
        if day_regex.match(self.dates[0]): return WitDataType.DAY_TYPE
        elif week_regex.match(self.dates[0]): return WitDataType.WEEK_TYPE
        elif month_regex.match(self.dates[0]): return WitDataType.MONTH_TYPE
        elif quarter_regex.match(self.dates[0]): return WitDataType.QUARTER_TYPE
        elif year_regex.match(self.dates[0]): return WitDataType.YEAR_TYPE
        else: return 0
    ...
    def spread(self, target_type):
        ...
        if self_type == WitDataType.YEAR_TYPE:
            for index, a_date in enumerate(self.dates):
                start_date = a_date+'0101'
                end_date = a_date+'1231'
            ...
            for index, a_date in enumerate(self.dates):
                start_date = pd.Period(year=int(a_date[:4]), quarter=int(a_date[4:5]), freq='D').strftime('%Y%m%d')
                end_date = pd.Period(year=int(a_date[:4]), quarter=int(a_date[4:5]), freq='Q').strftime('%Y%m%d')
            ...
            for index, a_date in enumerate(self.dates):
                start_date = pd.Period(year=int(a_date[:4]), month=int(a_date[4:6]), freq='D').strftime('%Y%m%d')
                end_date = pd.Period(year=int(a_date[:4]), month=int(a_date[4:6]), freq='M').strftime('%Y%m%d')
            ...
            for index, a_date in enumerate(self.dates):
                start_date = datetime.strptime(a_date[:6]+'1', '%Y%W%w').strftime('%Y%m%d')
                end_date = datetime.strptime(a_date[:6]+'0', '%Y%W%w').strftime('%Y%m%d')
                if target_type == WitDataType.DAY_TYPE:
                    for yyyymmdd in pd.date_range(start=start_date, end=end_date).strftime("%Y%m%d").tolist():
                        new_dates = np.append(new_dates, yyyymmdd)
                        new_data = np.append(new_data, self.data[index])
