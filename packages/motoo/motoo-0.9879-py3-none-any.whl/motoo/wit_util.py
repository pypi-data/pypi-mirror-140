...
def reshape_wit_data_list(wit_data_list, default_value=None):
    ...
    for a_wit in wit_data_list:
        if isinstance(a_wit, WitData): a_wit = a_wit.spread(std_type)
...
def get_std_type(wit_data_list):
    ...
    for a_wit in wit_data_list:
        if isinstance(a_wit, WitData):
            for i in range(2):
                std_index = len(WIT_DATA_TYPE_LINK[i]) + 1
                try:
                    std_index = WIT_DATA_TYPE_LINK[i].index(std_types[i])
                except: pass

                a_wit_index = len(WIT_DATA_TYPE_LINK[i])
                try:
    ...
    if std_types[0] and std_types[1] and std_types[0] != std_types[1]:
