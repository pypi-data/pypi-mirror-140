...
class Params:
    def __init__(
        ...
        Args:
            ...
            synthesizer_path (Path, optional): Synthesizer模型，不填写时使用默认目录的第一个. Defaults to None.
        ...
        if vocoder.lower() == "hifigan":
            ...
            self.vocoder = None
        self.text = text
        self.synt_path = synthesizer_path
        self.min_stop_token: int = accuracy
...
class MockingBird:
    ...
    @classmethod
    def _vocoder(cls, params: Params):
        global _synthesizers_cache
    ...
    @classmethod
    def genrator_voice(cls, params: Params) -> Union[Path, io.BytesIO]:
        """根据提供的参数生成语音
        ...
        Returns:
            Union[Path, io.bytesIO]: 如果提供了语音文件保存位置，则返回指向保存位置的Path,否则返回音频内容bytesIO
        """
