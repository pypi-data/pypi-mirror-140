...
def find_model_version(model_name, env, metadata):
    ...
    if (n_models := file_list.shape[0]) == 0:
        message = f"No model artifacts found for model {model_name=} in {env=} with metadata {metadata}. Upload/promote artifacts or specify the correct model name before redeploying"
