...
def deploy_model(model_settings, platform_methods=platform_methods):
    ...
    try:            
        ...
        logging.info(f"Deploying function {human_readable_function_name} with external id {function_name} to {c.platform}")
        deploy_function, test_function = platform_methods[c.platform]
        deploy_function(human_readable_function_name, function_name, info=c.info[service_name])
