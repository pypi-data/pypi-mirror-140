...
- Run this command from your repo's root folder:
  ...
  python -m akerbp.mlops.deployment.setup
  ```
  This will update the pipeline and validate your settings. Once the settings are validated, commit changes and
  you're ready to go!
...
Note: 
  The metadata field under input is passed to the description argument when creating the function (when deploying to CDF). There will however be support for a metadata field in cognite functions shortly. Until then, we will use the description as metadata to our functions in CDF. 
...
This section describes how you can called deployed models and obtain predictions for doing inference.
We have two options for calling a function in CDF, either using the MLOps framework directly or by using the Cognite SDK. Independent of how you call your model, you have to pass the data as a dictionary with field "data" containing a dictionary with your data, where the keys of the inner dictionary specifies to columns, and the values are list of samples for the correspondsing columns. 
...
1. set up cognite client with suffient access rights
...
If the input data dictionary contains a key "to_file" with value True, the predictions are uploaded to cognite Files, and the 'prediction_file' field in the reponse will contain a reference to the predictions. If "to_file" is set to False, or if the input dictionary does not contain such a key-value pair, the predictions are directly available through the function call response. 
