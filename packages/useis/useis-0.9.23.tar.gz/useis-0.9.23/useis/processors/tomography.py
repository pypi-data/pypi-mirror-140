...
class Tomography(ProjectManager):
    def __init__(self, base_projects_path, project_name, network_code,
        ...
        if not self.paths.current_epoch.exists():
    ...
    def event_table_file(self):
        return self.paths.current_epoch / 'event_table.pickle'

    @property
    def site_table_file(self):
        return self.paths.current_epoch / 'site_table.pickle'
    ...
    def travel_time_table(self, site_id: int, phase: Phase):
        ...
        tt_table = estuaire_data.EKTTTable(ttt, site_id,
                                           evnfile=self.event_table_file,
                                           stafile=self.site_table_file)
    ...
    def __write_travel_time_grids__(self):
        for travel_time_grid in self.travel_times:
            ...
            with open(file_name, 'wb') as output_file:
                pickle.dump(travel_time_grid, output_file)
    ...
    def run_sensitivity(self):
        ...
        for site_name in self.travel_times.seed_labels:
            for phase in Phase:
                cmd = f'sensitivity ' \
                      f'--output ' \
                      f'/tmp/{self.sensitivity_file(site_name, phase)} ' \
                      f'--arrival ' \
                      f'/tmp/{self.travel_time_grid_file(site_name, phase)} ' \
