...
def get_outputs_parallel(
    ...
    # TODO: Error catching issue here

    # TODO: Can we make kwargs shared memory? Possibly only if it could be written to byte array.
    ...
    for args in enumerate(input_args):
        p = Process(target=q_wrap, args=([Q, args, func_kwargs]))
        procs.append(p)
        p.start()
    for p in procs:
        i, res = Q.get()
