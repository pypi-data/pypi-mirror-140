...
class BaseLabelme(object):
                        ...
                        for key, value in shape.items():
                            if key == 'label':
                                if value not in self.label_shape_flags.keys():
                                    self.label_shape_flags[value] = flags_shape.copy()
                            if key == 'shape_type':
                                # if shape[key] not in shape_type_key:
                                shape_type_key.append(value)
                                flags_shape[key] = shape_type_key
                                self.label_shape_flags[shape['label']].update(flags_shape)
                            if key == 'flags':
                                for flag in list(value):
                                    if shape['flags'][flag] is True:
                                        if flag not in flags_key:
                                            flags_key.append(flag)
                                flags_shape[key] = flags_key.copy()
                                self.label_shape_flags[shape['label']].update(flags_shape)
                else:
    ...
    def __repr__(self):
        ...
        tb.add_row(
            [self.num_images, self.num_shape_type, self.num_label, self.num_labelme, self.num_background,
             self.num_flags])
        tb2 = pt.PrettyTable()
    ...
    def __init__(self, labelme_dir, images_dir, only_annt=False, is_labelme=False):
        ...
        self.name_classes = list()  # coco转换labelme时所用
        self.shape_type = list()  # coco转换labelme时所用
        self.background = list()  # coco转换labelme时所用
