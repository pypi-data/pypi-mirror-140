...
def agrega_categoria_entropia_aproximada(diccionario_entropia, categoria, datos, tamanio_vectorial, traslape):
    print("Agrego categoria de entropia: ")
    print("--------------------------------------------------------------")
    print("Diccionario:", diccionario_entropia)
    print("Categoria", categoria)
    ...
    nuevos_datos = np.array(datos)
    if categoria not in diccionario_entropia:
        diccionario_entropia[categoria] = calcula_entropia_aproximada(nuevos_datos,
                                                                      tamanio_vectorial,
                                                                      traslape)
    print("Nuevo diccionario: ", diccionario_entropia)
...
def calcula_entropia_aproximada(arreglo, m, r) -> float:

    def _maxdist(x_i, x_j):
        return max([abs(ua - va) for ua, va in zip(x_i, x_j)])

    def _phi(m):
        x = [[arreglo[j] for j in range(i, i + m - 1 + 1)] for i in range(n - m + 1)]
        c = [
