...
class PipelineController(object):
    ...
    def __init__(
            ...
            add_run_number=False,  # type: bool
    ):
        # type: (...) -> None
        """
        ...
        :param add_run_number: If True (default: False), add the run number of the pipeline to the pipeline name.
        ...
        # TODO: replace to API version 2.17
    ...
    def _add_pipeline_name_run_number(self):
        ...
        prev_pipelines_ids = self._task.query_tasks(
            ...
            task_filter=dict(project=[self._task.project], system_tags=[self._tag])
        )
        if prev_pipelines_ids and len(prev_pipelines_ids) > 1:
            self._task.set_name(task_name + " #{}".format(len(prev_pipelines_ids)))
