...
PUNCTUATION = set('''！，。？、；：“”‘’《》%（）<>{}「」【】*～`,.?:;'"!%()''')
re_text = re.compile(r'[^0-9^\u4e00-\u9fa5]', flags=re.S)
...
def text(element: Element):
    ...
    if element.tag == 'a':
        if len(text) > 100:
            h_text = ''.join(element.xpath('.//h1//text() | .//h2//text() | .//h3//text() | .//h4//text()'))
            if h_text:
                text = h_text
            else:
                t_text = element.xpath('string(.//@title)')
                text = t_text if t_text else text
...
def number_of_a_char(element: Element):
    ...
    text = re.sub(r'\s*', '', text, flags=re.S)
    # text = re.sub(r'[^0-9^\u4e00-\u9fa5]', '', text, flags=re.S)
...
def number_of_clusters(element: Element, tags=None):
    ...
    :return:
    """
    from auto_parse.gae.deal.list import LIST_MIN_NUMBER, LIST_MAX_LENGTH, LIST_MIN_LENGTH, \
