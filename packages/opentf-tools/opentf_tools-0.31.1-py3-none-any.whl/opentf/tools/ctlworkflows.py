...
GET_QUALITYGATE_HELP = '''Get qualitigate status for a workflow
...
Examples:
  ...
  opentf-ctl get qualitygate 9ea3be45-ee90-4135-b47f-e66e4f793383 --mode=full
...
def list_workflows():
    ...
    try:
        ...
        _error('Cound not get columns specification: %s', err)
...
def get_qualitygate(workflow_id, mode):
    ...
    Abort with an error code of 20 if the qualitygate failed.

    Abord with an error code of 10 if the workflows is still running.
    """
    _ensure_uuid(workflow_id)
    result = _get(_qualitygate(), f'/workflows/{workflow_id}/qualitygate?mode={mode}')
    if 'details' not in result or 'status' not in result['details']:
        _error(
