...
def df_from_audata(rec,
    ...
    logging.info(f"df_from_audata(rec={rec}, columns={columns}, time_ref={time_ref}, datetimes={datetimes})")

    # If datetimes requested, warn if there is not a time column
    if datetimes:
        tf = False
        for col in columns:
            col_meta = columns[col]
            if 'type' in col_meta and col_meta['type'] == 'time':
                tf = True
                break
        if not tf:
            logging.warning(f"Datetimes requested, but dataframe has no time column! Columns are:\n{columns}")
...
def audata_from_df(data: pd.DataFrame,
    ...
    logging.info(f"audata_from_df(data={data}, time_ref={time_ref}, time_cols={time_cols}, timedelta_cols={timedelta_cols})")
...
def audata_from_arr(arr: Union[np.ndarray, np.recarray],
    ...
    logging.info(f"audata_from_arr(arr={arr}, time_ref={time_ref}, time_cols={time_cols}, timedelta_cols={timedelta_cols})")
...
# TODO(gus): At some point, it would be nice to merge this with how
