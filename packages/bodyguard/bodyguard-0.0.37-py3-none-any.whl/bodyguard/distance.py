...
def compute_distance(a,b,metric="Lknorm",**kwargs):
    ...
    if metric in SCIKIT_DISTANCE_METRIC:
        ...
        if metric=="cosine":
            # Cosine similarity is defined between [-1,1]. Due to numerical stability, we enforce this
            distances.clip(lower=-1,
                           upper=+1,
                           inplace=True)
            
            # Convert cosine similarity to angular distance (https://en.wikipedia.org/wiki/Cosine_similarity#Angular_distance_and_similarity)
            # We round to avoid problems with numerical stability
            distances.iloc[:,:] = np.arccos(distances) / np.pi
...
def compute_similarity(a,b,metric="Lknorm",**kwargs):
    ...
    if metric in SCIKIT_DISTANCE_METRIC+OTHER_DISTANCE_METRIC:
        ...
        print("YES, WE COMPUTE DISTANCES ...")
        # Compute distances
        ...
        if isin(a=metric,b=["cosine","correlation"]):
            print("YES, WE COMPUTE SIMIL USING DISTANCES ...")
