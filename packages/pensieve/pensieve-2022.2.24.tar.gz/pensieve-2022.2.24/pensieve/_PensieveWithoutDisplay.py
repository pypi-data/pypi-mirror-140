...
class PensieveWithoutDisplay:
	...
	def performance(self):
		...
		"""
		result = self.function_durations.performance_summary.copy()
		result = result.rename(columns={'total_duration': 'sum_of_all_evaluations'})
		result['complete_evaluation_time'] = result.apply(
			lambda x: convert(delta=self.memories_dictionary[x['name']].total_time, to_unit=x['unit']),
			axis=1
		)
		result['precursor_evaluation_time'] = result['complete_evaluation_time'] - result['mean_duration']
