...
def PCA_analysis(coredata_no_outliers,Model_elements):
    ...
    scaler.fit(coredata_no_outliers[Model_elements].values)
    x_new = pca.fit_transform(scaler.transform(coredata_no_outliers[Model_elements].values)) #
    return x_new

def PCA_explained(coredata_no_outliers,Model_elements):
    scaler = StandardScaler() #create a standard scaler object
    pca = PCA() #create a PCA object called pca. could include pca = PCA(n_components=1)
    scaler.fit(coredata_no_outliers[Model_elements].values)
    pca.fit_transform(scaler.transform(coredata_no_outliers[Model_elements].values)) #
    pca_explained=pca.explained_variance_ratio_.cumsum()
    return pca_explained

def PCA_matrix_elements(coredata_no_outliers,Model_elements):
