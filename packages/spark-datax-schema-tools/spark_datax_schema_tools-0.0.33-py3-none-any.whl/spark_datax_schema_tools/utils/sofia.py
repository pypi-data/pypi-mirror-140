def generate_sofia(spark=None,
    ...
    df_datum = df2[df2["OBJECT_ID"] == f'{table_name}']
    description_name = "Required Description"
    logical_name = "Required Logical Name"
    if df_sofia.shape[0] > 0:
        description_name = df_datum["PHYSICAL_NAME_OBJECT"].values[0]
    ...
    if schema_write_or_read == "read":
        ...
        t.add_row(["QueryParamas_paths", f"x_{text_schema.lower()}_hdfs_{table_name2}_0" + "_{CUTOFF_DATE}"])
        t.add_row(["sqlfilter", ""])

    t.add_row(["Parameter", "CUTOFF_DATE - STRING - information_date"])
