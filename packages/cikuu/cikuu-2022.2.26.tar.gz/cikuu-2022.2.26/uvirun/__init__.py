...
@app.get('/')
def home(): 
	return HTMLResponse(content=f"<h2>{getattr(app,'title')}</h2><a href='/docs'> docs </a> | <a href='/redoc'> redoc </a><br>uvicorn uvirun:app --port 80 --host 0.0.0.0 --reload <br><br>last update: {app.tm} <br> started: {now()}")
# app.title = "hello" 
# app.tm = "2022.2.13"
