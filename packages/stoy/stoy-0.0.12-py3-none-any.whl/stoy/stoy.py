...
async def run(url, kernel_timeout, server_timeout):
    async with aiohttp.ClientSession() as session:
        running = True
        inactive_since = None
        while running:
            try:
                async with session.get(f"{url}/api/kernels", ssl=False) as resp:
                    ...
                    for kernel in kernels:
                        ...
                        if seconds > kernel_timeout:
                            async with session.delete(f"{url}/api/kernels/{kid}", ssl=False) as r2:
                                if r2.status == 204:
