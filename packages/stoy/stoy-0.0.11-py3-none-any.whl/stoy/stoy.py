...
def parse_arguments():
    ...
    parser.add_argument("--log", "-l",
                        help="Path to log directory. If not provided saved in ~/.stoy",
                        required=False,
                        type=str,
                        default="")
...
def get_token(jupyter_path):
    ...
    while not running:
        result = subprocess.run([cmd, "notebook", "list"], stdout=subprocess.PIPE)
...
def main():
    ...
    logging.basicConfig(filename=prepare_log(args.log),
                        filemode="w",
