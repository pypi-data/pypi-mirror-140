...
from dfwsgroup.common.custom import max_xpath_actions, max_xpath_actions_clear
...
def main_command(args):
    ...
    # 判断是否为真值
    if args.modulename:
        logging.info(f"获取默认指定json文件中的模块名：{args.modulename}")
        ...
        logging.info(f"获取默认指定json文件路径：{default_json_path}")
        logging.info(f"获取默认指定json文件路径及名称：{Path(default_json_path).joinpath('dfwsgroup.json')}")

        # 根据获取的路径json文件路径及模块名称，进行数据写入操作
        max_xpath_actions(default_json_path, "dfwsgroup.json", args.modulename)

    if args.package:
        logging.info(f"指定包名-> {args.package}")

        minutes, throttle = args.minutes, args.throttle

        if minutes or throttle:
            logging.info(f"指定分钟-> {minutes}")
            logging.info(f"指定事件频率-> {throttle}")
        else:
            minutes, throttle = 5, 500
            logging.info(f"默认分钟-> {minutes}")
            logging.info(f"默认事件频率-> {throttle}")

        if not args.log:
            logging.info(f"关闭日志获取-> {args.log}")

        logging.info(f"开始运行命令")
