...
# tested
def read_data_external(filepath_name,  main_df, date_column = 'DATE', fillna_limit_n=5):
...
# 7. merge dataframe
# tested
def merge_dfs(df_list = [], on_column = 'DATE'):
    if len(df_list) == 0:
        return pd.DataFrame()
    if len(df_list) == 1:
        return df_list[0]
...
def shift_row(df, target_columns, shift_n_list = []):
