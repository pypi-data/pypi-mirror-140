...
from framework.basecmd import PagemarksCommand
from framework.cmdline import CmdLine
from framework.repo import Repo
from framework.util import normalize_date, normalize_tags, PagemarksAbort, PagemarksError, prompt, PromptAnswer
...
class CmdAdd(PagemarksCommand):
    ...
    def execute(self, repo: Repo):
        ...
        record['date_added'] = normalize_date(datetime.now())
    ...
    def commit_file(git_repo: GitRepo, coll_name: str, rel_path: str, url: str) -> None:
        ...
        git_repo.index.commit(f"Add {msg_url} (collection: {coll_name})")
