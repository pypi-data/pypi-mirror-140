...
async def get_state(hub, ctx) -> Dict[str, Any]:
    ...
    async with aiofiles.open(cache_file, "r") as fh:
        data = await fh.read()
        if not data:
            return {}
...
async def set_state(hub, ctx, state: Dict[str, Any]):
    ...
    """
    cache_file = _cache_file(ctx)
    async with aiofiles.open(cache_file, "w+") as fh:
...
async def enter(hub, ctx):
    ...
    """
    pid_file = ctx.cache_dir / f"{ctx.run_name}.pid"
    ...
    if pid:
        ...
        raise ChildProcessError(
            f"idem run '{pid_file.stem}' is already active in process: {pid}"
        )
...
def _cache_file(ctx) -> pathlib.Path:
    ...
    """
    cache_file: pathlib.Path = ctx.cache_dir / "esm" / f"{ctx.run_name}.json"
