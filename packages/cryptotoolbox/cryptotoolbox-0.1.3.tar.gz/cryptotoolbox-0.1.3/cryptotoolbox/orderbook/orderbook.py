...
def process_orderbook_metrics(orderbook_df = None, id=False, amounts_to_pass = [100,500,1000,5000,10000,50000,100000], pair='free'):
    ...
    for amount_to_pass in amounts_to_pass:
        ...
        bid_slippage = abs((sold_price - best_bid_price)/best_bid_price)
        ask_slippage = abs((bought_price - best_ask_price)/best_ask_price)
        results.append({
                'id':id,
                'pair':pair,
                'amount':amount_to_pass,
                f'sold_max_rate_of_change':sold_max_rate_of_change,
                f'bid_max_rate_of_change':bought_max_rate_of_change,
