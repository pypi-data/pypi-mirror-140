...
class Vector:
    ...
    def write(self, path, index=True, overwrite=False):
        ...
        if index:
            create_index(self.layer, 
                         True, str(pathlib.Path(path).with_suffix(""))
                         )
    ...
    def set_index(self):
        ...
        if not hasattr(self, "idx") and self.has_rtree:
            ...
            self.idx = create_index(self.layer, self.quiet, self.filename)
    ...
    def write_index(self, path, quiet=True):
        ...
        create_index(self.layer, quiet, path)
...
def create_index(layer, quiet=True, path=None):
    """  creates an rtree index, closes and rereads if path is available"""
