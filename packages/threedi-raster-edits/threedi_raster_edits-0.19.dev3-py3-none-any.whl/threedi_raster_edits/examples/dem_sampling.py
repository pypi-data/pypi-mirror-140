...
import numpy as np
from threedi_raster_edits import Vector, Raster, Progress, RasterGroup
...
waterdeel_path = r"C:\Users\chris.kerklaan\Documents\Projecten\basis kaarten hhnk\processing\processed_waterdeel.shp"
bruggen_path = r"C:\Users\chris.kerklaan\Documents\Projecten\basis kaarten hhnk\processing\data\Bruggen_hhnk.shp"
...
bruggen = Vector(bruggen_path)
waterdelen = Vector(waterdeel_path)
buffer = 4

# make output
output = bruggen.copy(shell=True)
output.add_field("level", float)

dissolved = bruggen.dissolve(quiet=False)
singles = dissolve.to_single(quiet=False)


for brug in Progress(singles, "Sampling bruggen"):

    try:
        # get all waterareas
        waterdeel = waterdelen.spatial_filter(brug.geometry, return_feature=True)[0]

        # do a 1 meter buffer
        geometry_buffered = waterdeel.geometry.buffer(buffer)

        # difference with the original to create a hull
        difference = geometry_buffered.difference(waterdeel.geometry)

        # clip
        clipped = brug.geometry.clip(difference)[0]

    except Exception as e:
        print(e)
        clipped = brug.geometry

    # read
    data = dem.read(clipped)
    ...
    # write to output with new fields
    items = brug.items
    items["level"] = percentile
    output.add(items=items, geometry=brug.geometry)

output.write(
    r"C:\Users\chris.kerklaan\Documents\Projecten\basis kaarten hhnk\processing\bruggen_sampled.shp"
)
