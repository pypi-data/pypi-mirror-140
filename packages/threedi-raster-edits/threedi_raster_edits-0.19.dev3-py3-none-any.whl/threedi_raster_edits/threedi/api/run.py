    ...
    class Simulation:
        ...
        def add_saved_state_threshold(self, variable, value):
            ...
            threshold_state = StableThresholdSavedState(
                name="saved state", thresholds=[{"variable": variable, "value": value}],
            )
        ...
        def add_saved_state_timed(self, name, time):
            ...
            self.saved_state = self.simulation_api.simulations_create_saved_states_timed_create(
        ...
        def add_wind(self, event: list, offset=0, units="m/s", drag_coefficient=0.05):
            ...
            wind = TimeseriesWind(
                simulation=self.simulation.id, offset=offset, values=event, units=units,
            )
        ...
        def add_breach(
            self, breach_id, duration_till_max_depth=100, initial_width=15, offset=0,
        ):
        ...
        def add_boundary_conditions_file(self, file_path):
            ...
            url = self.simulation_api.simulations_events_boundaryconditions_file_create(
                simulation_pk=self.simulation.id, data={"filename": "bc.text"},
            )
        ...
        def set_timestep_settings(
            ...
            self.timestep_settings = self.simulation_api.simulations_settings_time_step_create(
                self.simulation.id,
                TimeStepSettings(
        ...
        def post_processing(
            ...
            if basic or arrival or damage:
                self.simulation_api.simulations_results_post_processing_lizard_basic_create(
                    self.simulation.id,
                    data={"process_basic_results": True, "scenario_name": name,},
                )
            if damage:
            ...
            if "results_3di.nc" in available:
                try:
                    self.simulation_api.simulations_results_files_download(
                        id=result_files.results[0].id, simulation_pk=self.simulation.id,
                    )
