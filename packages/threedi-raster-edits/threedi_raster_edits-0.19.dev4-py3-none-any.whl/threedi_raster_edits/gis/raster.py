...
class Raster:
    ...
    def __init__(
        self, raster_input, name=None, optimize_blocksize=True, **kwargs,
    ):
        """
    ...
    def smooth(
        self, method="gaussian", arguments={"sigma": 1, "mode": "nearest"}, quiet=False,
    ):
        """
    ...
    def push_vector(
        self, vector: Vector, value=None, field=None, set_to_nodata=False, quiet=False,
    ):
        """
    ...
    def polygonize(
        ...
        if polygonize_type == ogr.OFTReal:
            gdal.FPolygonize(
                band, no_data_band, layer_vector, idx, [], callback=callback,
            )
        else:
            gdal.Polygonize(
                band, no_data_band, layer_vector, idx, [], callback=callback,
...
def create_source(columns, rows, data_type):
    ...
    out = DRIVER_GDAL_MEM.Create(
        path, columns, rows, 1, data_type, options=WRITE_OPTIONS,
    )
    return out, path
