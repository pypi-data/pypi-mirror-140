...
if HAS_THREEDIGRID:
    ...
    class Grid:
            ...
            if len(grid_id) > 0:
                for gid in grid_id:
                    output_dict[gid] = list(
                        self.gr.nodes.filter(id=gid).s1.flatten()
                    )
        ...
        def discharge(self, grid_id=[], model_id=[], model_type=None, start_time=0, end_time=None):
            ...
            if type(model_id) == int:
                return list(self.gr.lines.filter(content_pk=model_id, content_type=model_type).timeseries(start_time, end_time).q.flatten())

            output_dict = {}
            if len(grid_id) > 0:
                for gid in grid_id:
                    output_dict[gid] = list(
                        self.gr.lines.filter(id=gid).q.flatten()
                    )
