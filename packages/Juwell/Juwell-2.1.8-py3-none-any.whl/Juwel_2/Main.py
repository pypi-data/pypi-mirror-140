...
from Juwel import Formatted_expiration_finder
from Juwel import text_parser
from Juwel import Widget_index_finder
import Formatted_expiration_finder
import text_parser
import Widget_index_finder
...
##############################################################
        ...
        self.menu_0.add_command(label="Select file to attach metadata", command=self.browse_file)
        self.menu_0.add_command(label="Select directory to attach metadata", command=self.browse_folder)
        self.menu_0.add_separator()
        self.menu_0.add_command(label="Save metadata", command=self.save)
        self.menu_0.add_command(label="Save metadata and exit", command=self.save_and_exit)
        self.menu_0.add_separator()
...
##################### Eventhandling der Menubar ###########################################################
    ...
    def save(self, *args):
        ...
        f = asksaveasfile(mode='w', defaultextension=self.extension_suggestion, initialfile=self.filename_suggestion, filetypes=[("Sidecarfile", "*.meta")])
        if f == None: 
            self.cancel = False
            return
...
############################################################################
    ...
    def make_checkbox(self, j, element):               
        ...
        for item in element['values']:
            ...
            self.checkbox[j][k] = Checkbutton(self.checkbox_frame[j], text=item, variable=self.variable[j][k], onvalue=item, offvalue="none")
