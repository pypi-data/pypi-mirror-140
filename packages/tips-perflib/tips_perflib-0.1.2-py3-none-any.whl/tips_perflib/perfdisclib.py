...
class Analysis:
    ...
    def read_task_list(self, build) -> any:
        ...
        while True:
            r = requests.get(
                f"{self.pd_info.evg_base}/{build}?skip={fetched}&limit={self.pd_info.batch}"
            )
    ...
    @staticmethod
    def filter_and_merge(dfa: pd.DataFrame, dfb: pd.DataFrame) -> any:
        ...
        def filter_canaries(df):
            ...
            df_filtered = df_filtered[
                ~df_filtered.test.str.contains("ActorFinished|ActorStarted|Setup")
            ]
            return df_filtered
        ...
        comparison = dfa.merge(
            dfb, on=["project", "variant", "task", "test", "measurement", "args"]
        )
    ...
    @staticmethod
    def get_stable_region(commit_date, ts, cps) -> any:
        """Algorithm to look up time series in order to characterize the stable region of results around build_a."""

        orders_without_false_positives = {
            cp["order"] for cp in cps if cp["triage"]["triage_status"] != "false_positive"
        }
        len_ts = len(ts["data"])
        stable_region_bounds = (
            ...
            + [
                idx
                for idx, datum in enumerate(ts["data"])
                if datum["order"] in orders_without_false_positives
            ]
            + [len_ts]
        )
    ...
    def process_stable_regions(self, found_ts) -> any:
        ...
        stable_median = []
        stable_median_absolute_deviation = []

        date_a = self.pd_info.client["expanded_metrics"]["versions"].find_one(
            {"version_id": self.pd_info.build_a}
        )["commit_date"]
        date_b = self.pd_info.client["expanded_metrics"]["versions"].find_one(
            {"version_id": self.pd_info.build_b}
        )["commit_date"]
        ...
        for index, row in found_ts.iterrows():
            ...
            cps = list(
                self.pd_info.client["expanded_metrics"]["change_points"].find(
                    {
            ...
            try:
                ...
                stable_median.append(np.median(stable_region))
                stable_median_absolute_deviation.append(stats.median_abs_deviation(stable_region))
                ...
                stable_median.append(np.nan)
                stable_median_absolute_deviation.append(np.nan)
        ...
        found_ts.insert(2, "stable_median", stable_median)
        found_ts.insert(3, "stable_median_absolute_deviation", stable_median_absolute_deviation)
        found_ts.insert(4, "stable_length", stable_length)
