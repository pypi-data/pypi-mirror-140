...
def _generate_transfer_mapping(
    ...
    if (
        ...
        source = pathlib.Path(source).as_posix().resolve()
        ...
        file_paths = (
            file_paths if file_paths else {pathlib.Path(source.name).as_posix()}
        )  # if specified a single file
        # recursively list objects
        existing_objects = {
            pathlib.Path(op.key.replace(destination, "")).as_posix()
        ...
        assert file_paths, "All files already exist in object store."
        object_paths = [pathlib.Path(destination, fp).as_posix() for fp in file_paths]
        file_paths = [
