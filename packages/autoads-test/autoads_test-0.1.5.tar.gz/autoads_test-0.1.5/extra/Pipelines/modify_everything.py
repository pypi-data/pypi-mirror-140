...
def remove_adgroup(client, ad_group_id):
...
def remove_campaign(client, campaign_id):
...
def update_adgroup(client, ad_group_id, **kwargs):
    if len(kwargs) > 0:
        ad_group_service = client.get_service("AdGroupService")
...
def update_campaign(client, campaign_id, **kwargs):
    if len(kwargs) > 0:
        campaign_service = client.get_service("CampaignService")
...
def update_keyword(client, ad_group_id, criterion_id, **kwargs):
    if len(kwargs) > 0:
        agc_service = client.get_service("AdGroupCriterionService")
...
if __name__ == '__main__':
    update_keyword(googleads_client, 134991344393,
