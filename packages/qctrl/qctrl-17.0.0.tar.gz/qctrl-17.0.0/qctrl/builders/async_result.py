...
# Do not use `tqdm.auto` if the user is running Jupyter (IPython is loaded)
# but ipywidgets is not available (its spec is None or its `has_location` is False).
# For more info, see: https://github.com/tqdm/tqdm/pull/1218
ipywidgets_spec = find_spec("ipywidgets")
if ("IPython" in modules) and (
    (ipywidgets_spec is None) or (not ipywidgets_spec.has_location)
):
    warn("For a better progress bar experience on Jupyter, install `ipywidgets`.")
