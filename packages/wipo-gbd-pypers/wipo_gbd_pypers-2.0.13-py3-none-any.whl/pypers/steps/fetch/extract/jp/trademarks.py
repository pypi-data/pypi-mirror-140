...
class Trademarks(ExtractBase):
    ...
    def list_update(self, l1, l2, key):
        ...
        for el in l2:
            try:
                ...
                l1[pos] = self.dict_update(el, l1[pos], replace=False)
    ...
    def dict_update(self, d1, d2, replace=True):
        """
        ...
        for k, v in d2.items():
            if isinstance(v, collections.Mapping):
                r = self.dict_update(d1.get(k, {}), v, replace)
                if replace or k not in d1:
                    d1[k] = r
            elif isinstance(v, list) and not replace:
                l2 = d1.get(k, [])
    ...
    def merge(self, num, bs, ap, rg):
        ...
        if bs:
            try:
                ...
                merged_data = self.dict_update(merged_data, from_data, replace=False)
            except: pass
