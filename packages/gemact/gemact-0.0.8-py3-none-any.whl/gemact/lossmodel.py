...
class Frequency:
    ...
    :param fdist: Frequency distribution. Provided as a string, it is then set to a discrete distribution in the setter property.
    :type fdist: ``str``
    :param fpar: It contains the frequency distribution parameters. Frequency distributions must be parametrized according to the scipy parametrizaton.
    :type fpar: ``dict``
    :param fq: Frequency distribution object.
    :type fq: ``scipy.stats._distn_infrastructure.rv_frozen``
    :param p0: When the frequency distribution is a ZM distribution, this is the mixture probability.
    :type p0: ``float``
    ...
    def fdist(self, var):
        ...
        assert var in frdict.keys(), "%r is not supported \n See our documentation: https://UnGiornoCiFacciamoPureIlSito.html " % var
...
class LossModel(Frequency,Severity):
    ...
    :param nsim: number of simulations, MC and QMC.
    :type nsim: ``int``
    :param skip: number of values to be dropped from the QMC simulated sequence.
    :type skip: ``int``
    :param tilt: whether tilting is present or not when computing the discrete Fourier transform.
    :type tilt: ``bool`` or ``float``
    :param setseed: seed parameter for MC and QMC.
    :type setseed: ``int``
    ...
    def method(self, var):
        if var is None:
            ...
            assert var.lower() in ['mcsimulation','recursive','fft','qmcsimulation'], "%r is not supported in our package \n See our documentation: https://UnGiornoCiFacciamoPureIlSito.html " % var
    ...
    @skip.setter
    def skip(self, var):
        assert isinstance(var, int), logger.error('skip=%r must be set to an integer' % var)
        self.__skip = var
    ...
    def __fft(self, tiltingp=0):
        ...
        if self.discretizationmethod == 'localmoments':
            fj = self.localMoments()
        else:
            fj = self.massDispersal()
    ...
    def __QMCsimulation(self):
        ...
        np.random.seed(self.setseed)
        fss = hfns.sobol_generator(n=self.nsim, dim=1,skip=self.skip).flatten()
        fqsample = self.fq.ppf(fss)
        sss = hfns.sobol_generator(n=int(np.sum(fqsample) + 1.1*np.ceil(p0 * self.nsim)), dim=1,skip=self.skip).flatten()
