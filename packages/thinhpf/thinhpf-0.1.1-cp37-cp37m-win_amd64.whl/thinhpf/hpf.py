...
def _create_class_name(base_name, capacity_type, label_order, root_order):
    ...
    if root_order not in root_order_lookup:
        raise ValueError(f"Unsupported root order '{root_order}'. Supported types are: {', '.join(root_order_lookup)}")

    return base_name + capacity_types_lookup[capacity_type] + label_order_lookup[label_order] + root_order_lookup[
        root_order]
