...
class Model(Configurable):
    ...
    def temporary_folder(
        ...
        parent_dir = os.path.join(*(path.split(os.sep)[-self.levels :]))
...
class LoadedModel:
    ...
    The class is instantiated from a
    :class:`~energinetml.core.model.ModelArtifact`-object.
    ...
    def from_artifact(cls, model_artifact: "ModelArtifact") -> "LoadedModel":
        ...
        module_path = os.path.join(model_artifact.path, module_name.replace(".", "/"))

        levels = Model.from_directory(module_path).levels

        model_class = import_model_class(module_path, levels)
        model = model_class.from_directory(module_path)

        trained_model_path = os.path.join(
            artifact_path, model_class._TRAINED_MODEL_FILE_NAME
        )
        trained_model = TrainedModel.load(trained_model_path)
...
def import_model_class(path, levels):
    """
    ...
    print(sys.path)
    module_to_import = ".".join(modules)
    print(f"Importing model from {module_to_import}")
