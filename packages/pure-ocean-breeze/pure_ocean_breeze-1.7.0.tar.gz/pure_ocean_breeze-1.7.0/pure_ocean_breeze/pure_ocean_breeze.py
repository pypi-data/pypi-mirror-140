...
def daily_factor_on300500(fac,hs300=False,zz500=False):
    ...
    if hs300:
        df=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/HS300Component.txt')
        df.time=pd.to_datetime(df.time,format='%Y%m%d')
        df=df.set_index('time')
        df=np.sign(df)
        df=df*fac
    elif zz500:
        df=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/CSI500Component.txt')
        df.time=pd.to_datetime(df.time,format='%Y%m%d')
        df=df.set_index('time')
        df=np.sign(df)
        df=df*fac
...
class pure_moon():
    ...
    def col_and_index(cls):
        ...
        cls.tradedays=list(map(lambda x:x[0],cls.tradedays))
...
class pure_fall():
    ...
    def minute_to_daily(self, func,add_priclose=False,add_tr=False):
        ...
        if add_priclose:
            for mat in tqdm.tqdm(self.minute_files):
                ...
                if add_tr:
                    share_this=share[code].to_frame('sharenum')
                    df=pd.concat([df,share_this],axis=1)
    ...
    def get_daily_factors(self, func, whole=False,add_priclose=False):
        '''调用分钟到日度方法，算出日频数据'''
        try:
            ...
            if whole:
                ...
                self.minute_to_daily(func,add_priclose=add_priclose)
