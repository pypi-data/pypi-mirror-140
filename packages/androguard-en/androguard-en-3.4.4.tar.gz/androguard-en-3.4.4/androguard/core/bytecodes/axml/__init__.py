...
class StringBlock:
    ...
    def __init__(self, buff, header):
        ...
        # fix current idx
        real_string_offset = self.stringsOffset + self.header.start
        if real_string_offset > buff.get_idx():
            buff.read(real_string_offset - buff.get_idx())
    ...
    def _decode8(self, offset):
        ...
        if self.m_charbuff[offset + encoded_bytes] != 0:
            ...
            log.error("UTF-8 String is not null terminated! At offset={}".format(offset))
    ...
    def _decode16(self, offset):
        ...
        if self.m_charbuff[offset + encoded_bytes:offset + encoded_bytes + 2] != b"\x00\x00":
            ...
            log.error("UTF-16 String is not null terminated! At offset={}".format(offset))
    ...
    def _decode_length(self, offset, sizeof_char):
        ...
        * 16 bit strings: maximum of 0x7FFFFFF bytes (See
        ...
        if (length1 & highbit) != 0:
            ...
            size = sizeof_2chars
            if length >= len(self.m_charbuff):
                log.warning("Illegal string length")
                length = length1
                size = sizeof_char
        else:
            length = length1
