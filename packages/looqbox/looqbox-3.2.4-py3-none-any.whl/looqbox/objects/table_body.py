...
class TableBody(LooqObject):
    ...
    def _insert_children_previous_row(self, data: list, row: list) -> list:
        ...
        if self.row_hierarchy[next_row_hierarchy] > self.row_hierarchy[next_row_hierarchy - 1]:
            counter = next_row_hierarchy - 1
            next_row_hierarchy = self._previous_row_same_hierarchy(counter, next_row_hierarchy)
        ...
        if data[max(next_row_hierarchy, 0)].get("tree_data") is None:
            data[max(next_row_hierarchy, 0)]["tree_data"] = [children_line]
        else:
            data[max(next_row_hierarchy, 0)]["tree_data"].extend([children_line])
            data[max(next_row_hierarchy, 0)]["tree_data"].reverse()
