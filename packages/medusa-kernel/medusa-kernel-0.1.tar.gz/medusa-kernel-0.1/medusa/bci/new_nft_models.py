...
class FeatureExtraction(components.ProcessingMethod):
    ...
    def __init__(self, l_baseline_t=5000,
                 fs=250, update_feature_window=2000, mode=None):
        """
        Class constructor

        l_baseline_t: int
            Time employed to calculate the number of samples to obtain baseline power parameter. In milliseconds.
        fs: int or float
            Sample rate of the recording.
        update_feature_window = int
            Length in milliseconds of the temporal window applied to calculate the feature.
        """
        ...
        self.w_signal_samples = int((update_feature_window / 1000) * self.fs)
        self.w_signal_samples_calibration = int(self.l_baseline_t * self.fs)
        self.baseline_power = None
        self.thresholds = None

    def set_baseline(self, signal, mode='single mode', ban_std_parameter=1):
        ...
        if mode is 'ban mode':
            ...
            #
            # for band_to_ban in signal[1:]:
            #     self.thresholds.append(mean_power(band_to_ban[-self.w_signal_samples_calibration:, :]) +
            #                            ban_std_parameter * std_power(
            #         band_to_ban[-self.w_signal_samples_calibration:, :]))

        elif mode is 'ratio mode':
            self.baseline_power = power[0] / power[1]
        elif mode is 'single mode':
            self.baseline_power = power[0]

        return self.baseline_power

    def band_power(self, signal, mode='single mode'):
        """
        ...
        signal: list or numpy.ndarray
            EEG already pre-processed. Its shape depends on the Neurofeedback modality:
            ([n_samples x n_channels] if SingleBandNFT mode, [2 x n_samples x n_channels] if RatioBandNFT mode
            and [n_bands_to_ban + n_samples x n_channels] if EnhanceAndBanBandsNFT mode). In the EnhanceAndBandBandsNFT
            mode, the target frequency band is passed previously as if the mode were SingleBandNFT
        mode: str
            -Set to 'ban mode' in EnhanceAndBanBandsNFT mode and only in case the signal parameter is the filtered
             signals in the banning frequency bands. Returns the power of these bands in order to compare with
             threshold values.
            -Set to 'ratio mode' in RatioBandsNFT mode. It returns the ratio between the power of the bands.
            -Set to 'single mode' in SingleBandNFT mode. It returns the band power baseline corrected.
    ...
    def ban_bands(self, signals, tolerance=1):
        ...
        """
        # Get band powers
        b_power_main_band, b_power_ban_bands = self.band_power(signals, mode='ban mode')
...
class ClassicalNFTModel(components.Algorithm):
    def __init__(self, fs, filter_bank, l_baseline_t, update_feature_window, montage,
        ...
        self.add_method('feat_ext_method',
                        FeatureExtraction(fs=fs, l_baseline_t=l_baseline_t, update_feature_window=update_feature_window))
...
class SingleBandNFT(ClassicalNFTModel):
    """
...
class RatioBandNFT(ClassicalNFTModel):
    """
...
class EnhanceAndBanBandsNFT(ClassicalNFTModel):
    """
