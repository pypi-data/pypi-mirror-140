...
class XnixScriptExecutor(ScriptExecutor):
  ...
  def _generate_inputs_outputs(self, script: Script, output_file: str):
    ...
    if completed_process.returncode != 0:
      raise Exception(f'Command exited with exit status {completed_process.returncode}')
  ...
  IRRELEVANT_CONTAIN_PATHS = ['/home/ubuntu/', '/site-packages', '/dev/tty']
  IRRELEVANT_BASEPATHS = ['/usr/', '/lib/', '/etc/', '/lib64/', '/proc/']
