...
def main():
  ...
  For help with a specific command run elio [command] --help.
  """
...
def login():
    ...
    """
    email = click.prompt("Your Elio account email address")
    password = click.prompt("Password to your Elio account", hide_input=True)
...
def ls():
    ...
    if details.in_workflow():
      click.echo(Fore.RED + "Unable to list the current directory because you are currently in a workflow. Run 'elio cd ..' to go back into a directory first.")
      return
...
def cd(dir_item_name):
    ...
    if details.in_workflow():
      if dir_item_name == '..':
        ...
        terminate(Fore.RED + "\nUnable to list the current directory because you are currently in a workflow. Run 'elio cd ..' to go back into a directory first.")
      return
...
def add_item(item_type, item_name):
  ...
  if details.in_workflow():
    click.echo(Fore.RED + "Unable to add the {t} because you are currently in a workflow. Run 'elio cd ..' to go back into a directory first.".format(t=item_type))
    return
...
def upload(replace, replace_files, message, command):
  ...
  re-read each file and generate a hash (i.e. checksum) of the file contents.
  ...
  if len(command_str) < 2:
    click.echo(Fore.RED + "Must enter a command to run. For help run 'elio upload --help'")
    return
