__all__ = ('ClientError', 'ServerError', 'NotFound404',
           'Point', 'ObjectPoint', 'ContractPoint', 'ContractObjectPoint', 'ServicePoint',
           'ListPointMixin', 'RetrievePointMixin', 'CreatePointMixin', 'UpdatePointMixin', 'UploadPointMixin',
           'ActionPointMixin',
           )
        ...
        if not HttpStatus.is_success(response.status_code):
            if HttpStatus.is_client_error(response.status_code):
                if HttpStatus.is_not_found(response.status_code):
                    ...
                    raise NotFound404(self._service_point.url)
                else:
                    raise ClientError(f'{response.status_code}:{self._service_point.url}:{response.json()}')
            else:
                raise ServerError(f'{response.status_code}:{self._service_point.url}:{response.text}')
