...
async def run_ws_client():
    async with websockets.connect(WSURL_NHEADS) as beatWS:
        async for msg in beatWS:
            ...
            for key in list(conns.keys()):
                ...
                try:
                    ...
                    del conns[key]
                    print('error happend for ', key, 'ignored', e)
...
async def run_server(host, port, endpoint, keystore, passcode, log, debug, chainid):
    ...
    ws_client = asyncio.create_task(run_ws_client())
    await ws_client
    while True:
