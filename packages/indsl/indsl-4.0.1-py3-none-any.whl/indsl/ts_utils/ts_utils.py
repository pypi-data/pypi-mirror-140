...
def get_fixed_freq(start_time, end_time, granularity):
    """Create a Timeseries with a fixed frequency.
    ...
    Returns:
        numpy.ndarray: Timeseries with fixed frequency.
    """
    return np.arange(
        pd.Timestamp(start_time, unit="ms"),
        pd.Timestamp(end_time, unit="ms"),
...
def time_parse(time_window: str = "60min", function_name: str = ""):
...
def time_to_points(data: pd.Series, time_window="60min"):
