...
def calculate_threshold(shut_valve: pd.Series):
    ...
    if 1.5 * min(mean_edge[max1_idx], mean_edge[max2_idx]) > max(mean_edge[max1_idx], mean_edge[max2_idx]):
        raise RuntimeError("Not enough data to detect the threshold")
    else:
