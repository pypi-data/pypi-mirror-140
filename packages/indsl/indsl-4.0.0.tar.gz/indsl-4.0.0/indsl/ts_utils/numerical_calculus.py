...
def integrate_windows(
    ...
    Args:
        values (np.ndarray): Values to integrate corrected to the integrand rate.
        dt (np.ndarray): The time in milliseconds between datapoints.
        from_to_index (np.ndarray): index is the start of a window and value is the end index of the window.
        number_of_windws(int): the number of windows to integrate
...
def sliding_window_integration(
    ...
    Args:
        series (pandas.Series): Time series.
        window_length (pandas.Timedelta): window_length
        ...
        integrand_rate (pandas.Timedelta): integrand_rate.
