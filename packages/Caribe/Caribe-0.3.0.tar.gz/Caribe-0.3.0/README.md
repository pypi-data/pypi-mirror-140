...
```
---
- ## First Parser for the Trinidad and Tobago English Creole

This model utilises T5-base pre-trained model. It was fine tuned using a combination of a custom dataset and creolised JFLEG dataset. JFLEG dataset was creolised using the file encoding feature of the library. 

Within Creole continuum, exists different levels of lects. These include: 

- Acrolect: The most standard version of the lexifier.
- Mesolect: The version that consists of a mixture of arcolectal and basilectal features.
- Basilect: The most broken or broadest version of the lexifier.

**This NLP task was difficult because the structure of local dialect is not codified but often rely on the structure of its lexifier (Standard English). Also spelling varies from speaker to speaker. In addition, dialect words/spelling are not initial present in the vector space which made training times longer as well as the optimization.**
