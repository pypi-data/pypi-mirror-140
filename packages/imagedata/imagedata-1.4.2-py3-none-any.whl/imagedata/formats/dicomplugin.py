...
class DICOMPlugin(AbstractPlugin):
    ...
    def write_slice(self, tag, slice, si, archive, filename, ifile):
        ...
        if safe_si.dtype in self.smallint or np.issubdtype(safe_si.dtype, np.bool_):
            ...
            _series_min = 0 if safe_si.color else self.smallestPixelValueInSeries
            _series_max = 255 if safe_si.color else self.largestPixelValueInSeries
            ...
            ds.add_new(tag_for_keyword('SmallestPixelValueInSeries'), _VR, _series_min)
            ds.add_new(tag_for_keyword('LargestPixelValueInSeries'), _VR, _series_max)
            if 'RescaleSlope' in ds:
                del ds.RescaleSlope
            if 'RescaleIntercept' in ds:
                del ds.RescaleIntercept
