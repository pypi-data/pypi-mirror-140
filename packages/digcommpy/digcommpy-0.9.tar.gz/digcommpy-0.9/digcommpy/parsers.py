...
def read_codebook_file(filename, wiretap=False, columns=None, asarrays=False, **kwargs):
    ...
    **kwargs : keyword-arguments, optional
    ...
    if asarrays:
        messages = np.array([ast.literal_eval(k) for k in data[columns['message']]])
        codewords = np.array([ast.literal_eval(k) for k in data[columns['codeword']]])
        if wiretap:
            random = np.array([ast.literal_eval(k) for k in data[columns['random']]])
            code_info['random_length'] = np.shape(random)[1]
            codebook = (messages, codewords, random)
        else:
            codebook = (messages, codewords)
