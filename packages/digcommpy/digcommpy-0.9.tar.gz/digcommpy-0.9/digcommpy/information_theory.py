...
def channel_capacity(channel, channelstate=None):
    if isinstance(channel, channels.Channel):
        ...
        capacity = channels.BscChannel(channelstate).capacity()
    else:
...
class GaussianMixtureRv(object):  # TODO: Change to support scipy rv interface
    ...
    def rvs(self, N=1, return_components=False):
        ...
        components = np.random.choice(range(num_comps), N, p=self.weights)
        if num_comps < N:
            ...
            for row, _component in enumerate(components):
                #_component = np.random.choice(range(num_comps),  p=self.weights)
