...
class AutomaticFilenameJob(Job):
    """
    Abstract class for Jobs providing automatic filename generation.

    In order for this to work, you need to:

    1. Include a keyword argument called ``subject_id`` when instantiating this Job.
    2. Set :attr:`base_data_folder` and :attr:`job_data_folder` as a class attribute.
    3. If you use :meth:`shall_run`, you need to do the super call.

    Attributes
    ----------
    base_data_folder : str or Path
        The base folder for the data. Is normally set once for all jobs of a project.

    job_data_folder : str or Path
    ...
    include_hash_in_fname : bool
        ...
        you excluded some keyword arguments from filename creation but still need
        to get distinct filename.
    ...
    def get_full_data_folder(cls):
        ...
        return Path(cls.base_data_folder, cls.job_data_folder)
    ...
    def output_folder(self):
        ...
        """
        return Path(self.get_full_data_folder(), self.subject_id)
    ...
    def output_filename(self):
        ...
        f_name = '__'.join(f_name_list) + '.dat'
