...
@LastEditTime: 2022-02-16 19:07:17
...
class ExchangeHandler(TopBaseHandler):
    ...
    def get_async(self):
        ...
        if update_result:
            if int(act_dict["task_currency_type"]["key"]) == 4:
                invoke_result_data = self.change_crm_point(open_id, 1, 1, int(exchange_info.need_value), access_token, exchange_info.id, f"兑换{exchange_info.exchange_name}", True)
                if invoke_result_data.success == False:
