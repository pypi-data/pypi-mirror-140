# perllib module for pythonizer, generated by makelib.py on Wed Mar  2 15:35:28 2022
#
...
def fileinput_next(*args, **kwargs):
...
def open_(file,mode,encoding=None,errors=None,checked=True):
    ...
    try:
        if mode == '|-':    # pipe to
            sp = subprocess.Popen(file, stdin=subprocess.PIPE, shell=need_sh(file), text=True, encoding=encoding, errors=errors)
            if sp.returncode:
            ...
            sp = subprocess.Popen(file, stdout=subprocess.PIPE, shell=need_sh(file), text=True, encoding=encoding, errors=errors)
            if sp.returncode:
                raise Die(f"open({file}|): failed with {sp.returncode}")
