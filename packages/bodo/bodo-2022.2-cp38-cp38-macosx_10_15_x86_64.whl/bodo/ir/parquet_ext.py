...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    jcu__evx = pq_node.out_vars[0].name
    fgvq__cbwc = pq_node.out_vars[1].name
    if jcu__evx not in lives and fgvq__cbwc not in lives:
        return None
    elif jcu__evx not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif fgvq__cbwc not in lives:
...
def pq_remove_dead_column(pq_node, column_live_map, equiv_vars, typemap):
    ...
    jmnk__uqsoo = pq_node.out_vars[0].name
    assert isinstance(typemap[jmnk__uqsoo], TableType
        ), 'Parquet Node Table must be a TableType'
    if pq_node.col_indices:
        dnogj__fyjz, fqsu__wzfod = get_live_column_nums_block(column_live_map,
            equiv_vars, jmnk__uqsoo)
        dnogj__fyjz = bodo.ir.connector.trim_extra_used_columns(dnogj__fyjz,
            len(pq_node.col_indices))
        if not fqsu__wzfod and not dnogj__fyjz:
            dnogj__fyjz = [0]
        if not fqsu__wzfod and len(dnogj__fyjz) != len(pq_node.
            type_usecol_offset):
            pq_node.type_usecol_offset = dnogj__fyjz
            return True
    return False
