...
def box_spark_session(typ, val, c):
    juu__akvvh = c.context.insert_const_string(c.builder.module, 'pyspark')
    ebbu__azik = c.pyapi.import_module_noblock(juu__akvvh)
    eqpt__gzyt = c.pyapi.object_getattr_string(ebbu__azik, 'sql')
    wsv__sswf = c.pyapi.object_getattr_string(eqpt__gzyt, 'SparkSession')
    fnqom__noz = c.pyapi.object_getattr_string(wsv__sswf, 'builder')
    plm__elsun = c.pyapi.call_method(fnqom__noz, 'getOrCreate', ())
    c.pyapi.decref(ebbu__azik)
    c.pyapi.decref(eqpt__gzyt)
    c.pyapi.decref(wsv__sswf)
    c.pyapi.decref(fnqom__noz)
    return plm__elsun
...
class RowType(types.BaseNamedTuple):
    ...
    def __init__(self, types, fields):
        ...
        iyr__wyyyl = 'Row({})'.format(', '.join(
            f'{pobdb__yqmx}:{kuff__psj}' for pobdb__yqmx, kuff__psj in zip(
            self.fields, self.types)))
        super(RowType, self).__init__(iyr__wyyyl)
...
class RowModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        rbb__opieh = [(pobdb__yqmx, kuff__psj) for pobdb__yqmx, kuff__psj in
            zip(fe_type.fields, fe_type.types)]
        super(RowModel, self).__init__(dmm, fe_type, rbb__opieh)
...
def typeof_row(val, c):
    ...
    return RowType(tuple(numba.typeof(zhix__soj) for zhix__soj in val), fields)


@box(RowType)
def box_row(typ, val, c):
    ostcx__urg = c.pyapi.unserialize(c.pyapi.serialize_object(pyspark.sql.
        types.Row))
    if all(pobdb__yqmx.startswith(ANON_SENTINEL) for pobdb__yqmx in typ.fields
        ):
        bgb__lntmc = [c.box(kuff__psj, c.builder.extract_value(val, i)) for
            i, kuff__psj in enumerate(typ.types)]
        hnu__gkwz = c.pyapi.call_function_objargs(ostcx__urg, bgb__lntmc)
        for obj in bgb__lntmc:
            c.pyapi.decref(obj)
        c.pyapi.decref(ostcx__urg)
        return hnu__gkwz
    args = c.pyapi.tuple_pack([])
    bgb__lntmc = []
    cfrvf__boz = []
    for i, kuff__psj in enumerate(typ.types):
        pekcs__isw = c.builder.extract_value(val, i)
        obj = c.box(kuff__psj, pekcs__isw)
        cfrvf__boz.append((typ.fields[i], obj))
        bgb__lntmc.append(obj)
    kws = c.pyapi.dict_pack(cfrvf__boz)
    hnu__gkwz = c.pyapi.call(ostcx__urg, args, kws)
    for obj in bgb__lntmc:
        c.pyapi.decref(obj)
    c.pyapi.decref(ostcx__urg)
    c.pyapi.decref(args)
...
class RowConstructor(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        wfsap__jtvw = ', '.join(f'arg{i}' for i in range(len(args)))
        bgo__iljli = ', '.join(f"{htl__yll} = ''" for htl__yll in kws)
        func_text = f'def row_stub({wfsap__jtvw}{bgo__iljli}):\n'
        func_text += '    pass\n'
        idyg__dkeay = {}
        exec(func_text, {}, idyg__dkeay)
        ptsq__ztn = idyg__dkeay['row_stub']
        lbrlk__dpdj = numba.core.utils.pysignature(ptsq__ztn)
        if args:
            ass__qfxr = RowType(args, tuple(f'{ANON_SENTINEL}{i}' for i in
                range(len(args))))
            return signature(ass__qfxr, *args).replace(pysig=lbrlk__dpdj)
        kws = dict(kws)
        ass__qfxr = RowType(tuple(kws.values()), tuple(kws.keys()))
        return signature(ass__qfxr, *kws.values()).replace(pysig=lbrlk__dpdj)
...
class SparkDataFrameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        rbb__opieh = [('df', fe_type.df)]
        super(SparkDataFrameModel, self).__init__(dmm, fe_type, rbb__opieh)
...
def overload_create_df(sp_session, data, schema=None, samplingRatio=None,
    ...
    orpsq__eilw = data.dtype.fields
    hfz__ceanl = len(data.dtype.types)
    func_text = (
        'def impl(sp_session, data, schema=None, samplingRatio=None, verifySchema=True):\n'
        )
    func_text += f'  n = len(data)\n'
    mhve__vkb = []
    for i, kuff__psj in enumerate(data.dtype.types):
        wjs__amk = dtype_to_array_type(kuff__psj)
        func_text += (
            f'  A{i} = bodo.utils.utils.alloc_type(n, arr_typ{i}, (-1,))\n')
        mhve__vkb.append(wjs__amk)
    func_text += f'  for i in range(n):\n'
    func_text += f'    r = data[i]\n'
    for i in range(hfz__ceanl):
        func_text += (
            f'    A{i}[i] = bodo.utils.conversion.unbox_if_timestamp(r[{i}])\n'
            )
    ltd__jnxcn = '({}{})'.format(', '.join(f'A{i}' for i in range(
        hfz__ceanl)), ',' if len(orpsq__eilw) == 1 else '')
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({ltd__jnxcn}, index, {orpsq__eilw})
"""
    func_text += f'  pdf = bodo.scatterv(pdf)\n'
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    idyg__dkeay = {}
    vuyt__zszr = {'bodo': bodo}
    for i in range(hfz__ceanl):
        vuyt__zszr[f'arr_typ{i}'] = mhve__vkb[i]
    exec(func_text, vuyt__zszr, idyg__dkeay)
    impl = idyg__dkeay['impl']
    return impl
...
def overload_df_to_rows(df):
    ...
    lrr__rziho = ', '.join(f'{c}=A{i}[i]' for i, c in enumerate(df.columns))
    func_text += f'    out.append(Row({lrr__rziho}))\n'
    func_text += '  return out\n'
    idyg__dkeay = {}
    vuyt__zszr = {'bodo': bodo, 'Row': pyspark.sql.types.Row}
    exec(func_text, vuyt__zszr, idyg__dkeay)
    impl = idyg__dkeay['impl']
    return impl
...
def _gen_df_select(spark_df, cols, avoid_stararg=False):
    ...
    for col in cols:
        ...
        data, cjhlm__ewxc = _gen_col_code(col, df_type)
        func_text += cjhlm__ewxc
        out_data.append(data)
    return _gen_init_spark_df(func_text, out_data, out_col_names)


def _gen_init_spark_df(func_text, out_data, out_col_names):
    ltd__jnxcn = '({}{})'.format(', '.join(out_data), ',' if len(out_data) ==
        1 else '')
    qmyt__nrjoc = '0' if not out_data else f'len({out_data[0]})'
    func_text += f'  n = {qmyt__nrjoc}\n'
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({ltd__jnxcn}, index, {tuple(out_col_names)})
"""
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    idyg__dkeay = {}
    vuyt__zszr = {'bodo': bodo, 'np': np}
    exec(func_text, vuyt__zszr, idyg__dkeay)
    impl = idyg__dkeay['impl']
    return impl
...
def overload_show(spark_df, n=20, truncate=True, vertical=False):
    ncebe__ogpca = dict(truncate=truncate, vertical=vertical)
    xoh__pepz = dict(truncate=True, vertical=False)
    check_unsupported_args('SparkDataFrameType.show', ncebe__ogpca, xoh__pepz)
...
def overload_with_column(spark_df, colName, col):
    ...
    nxcc__shtr = spark_df.df.columns
    xpa__ektzo = nxcc__shtr if col_name in nxcc__shtr else nxcc__shtr + (
        col_name,)
    gpyzq__ncdd, vxjs__ccz = _gen_col_code(col, spark_df.df)
    out_data = [(gpyzq__ncdd if c == col_name else
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {nxcc__shtr.index(c)})'
        ) for c in xpa__ektzo]
    func_text = 'def impl(spark_df, colName, col):\n'
    func_text += '  df = spark_df._df\n'
    func_text += vxjs__ccz
    return _gen_init_spark_df(func_text, out_data, xpa__ektzo)
...
def overload_with_column_renamed(spark_df, existing, new):
    ...
    xuc__ndb = get_overload_const_str(existing)
    gsiu__kby = get_overload_const_str(new)
    nxcc__shtr = spark_df.df.columns
    xpa__ektzo = tuple(gsiu__kby if c == xuc__ndb else c for c in nxcc__shtr)
    out_data = [f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})'
         for i in range(len(nxcc__shtr))]
...
def overload_dataframe_columns(spark_df):
    tcyhp__hdd = list(str(htl__yll) for htl__yll in spark_df.df.columns)
    func_text = 'def impl(spark_df):\n'
    func_text += f'  return {tcyhp__hdd}\n'
    idyg__dkeay = {}
    exec(func_text, {}, idyg__dkeay)
    impl = idyg__dkeay['impl']
    return impl
...
def init_col_from_name(typingctx, col=None):
    ...
    dqicj__cfqp = get_overload_const_str(col)
    hwfcv__vxy = ColumnType(ExprType('col', (dqicj__cfqp,)))

    def codegen(context, builder, signature, args):
        return context.get_constant_null(hwfcv__vxy)
    return hwfcv__vxy(col), codegen
...
def init_f_sum(typingctx, col=None):
    hwfcv__vxy = ColumnType(ExprType('sum', (col.expr,)))
...
def _gen_col_code_expr(expr, df_type):
    ...
    if expr.op == 'sum':
        dte__vgrze, sodur__smzlt = _gen_col_code_expr(expr.children[0], df_type
            )
        i = ir_utils.next_label()
        func_text = f"""  A{i} = np.asarray([bodo.libs.array_ops.array_op_sum({dte__vgrze}, True, 0)])
"""
        return f'A{i}', sodur__smzlt + func_text


def _gen_col_code_colname(col_name, df_type):
    xvafg__wpk = df_type.columns.index(col_name)
    i = ir_utils.next_label()
    func_text = (
        f'  A{i} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {xvafg__wpk})\n'
        )
