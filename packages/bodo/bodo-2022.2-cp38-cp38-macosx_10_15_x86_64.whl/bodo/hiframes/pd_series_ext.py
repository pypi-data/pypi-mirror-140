...
def series_getiter(context, builder, sig, args):
    xsyv__utlfd = get_series_payload(context, builder, sig.args[0], args[0])
    impl = context.get_function('getiter', sig.return_type(sig.args[0].data))
    return impl(builder, (xsyv__utlfd.data,))
...
class HeterSeriesAttribute(OverloadedKeyAttributeTemplate):
    ...
    def generic_resolve(self, S, attr):
        ...
        if isinstance(S.index, HeterogeneousIndexType
            ...
            rll__zxnfg = get_overload_const_tuple(S.index.data)
            if attr in rll__zxnfg:
                cpmv__gimq = rll__zxnfg.index(attr)
                return S.data[cpmv__gimq]
...
class SeriesPayloadModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ylvae__nin = [('data', fe_type.series_type.data), ('index', fe_type
            .series_type.index), ('name', fe_type.series_type.name_typ)]
        super(SeriesPayloadModel, self).__init__(dmm, fe_type, ylvae__nin)
...
class SeriesModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ...
        ylvae__nin = [('meminfo', types.MemInfoPointer(payload_type)), (
            'parent', types.pyobject)]
        super(SeriesModel, self).__init__(dmm, fe_type, ylvae__nin)


def define_series_dtor(context, builder, series_type, payload_type):
    qqrj__wwk = builder.module
    snruo__nunt = lir.FunctionType(lir.VoidType(), [cgutils.voidptr_t])
    zni__kxwn = cgutils.get_or_insert_function(qqrj__wwk, snruo__nunt, name
        ='.dtor.series.{}'.format(series_type))
    if not zni__kxwn.is_declaration:
        return zni__kxwn
    zni__kxwn.linkage = 'linkonce_odr'
    builder = lir.IRBuilder(zni__kxwn.append_basic_block())
    bfwk__uoyb = zni__kxwn.args[0]
    kvcvi__vpuxl = context.get_value_type(payload_type).as_pointer()
    zet__uegbu = builder.bitcast(bfwk__uoyb, kvcvi__vpuxl)
    gkktu__vtei = context.make_helper(builder, payload_type, ref=zet__uegbu)
    context.nrt.decref(builder, series_type.data, gkktu__vtei.data)
    context.nrt.decref(builder, series_type.index, gkktu__vtei.index)
    context.nrt.decref(builder, series_type.name_typ, gkktu__vtei.name)
    builder.ret_void()
    return zni__kxwn
...
def construct_series(context, builder, series_type, data_val, index_val,
    ...
    xsyv__utlfd = cgutils.create_struct_proxy(payload_type)(context, builder)
    xsyv__utlfd.data = data_val
    xsyv__utlfd.index = index_val
    xsyv__utlfd.name = name_val
    pxg__ubowi = context.get_value_type(payload_type)
    ilosk__emfhq = context.get_abi_sizeof(pxg__ubowi)
    eqdr__emtw = define_series_dtor(context, builder, series_type, payload_type
        )
    jgzow__necat = context.nrt.meminfo_alloc_dtor(builder, context.
        get_constant(types.uintp, ilosk__emfhq), eqdr__emtw)
    nvrz__hyec = context.nrt.meminfo_data(builder, jgzow__necat)
    qhfe__bpyt = builder.bitcast(nvrz__hyec, pxg__ubowi.as_pointer())
    builder.store(xsyv__utlfd._getvalue(), qhfe__bpyt)
    series = cgutils.create_struct_proxy(series_type)(context, builder)
    series.meminfo = jgzow__necat
...
def init_series(typingctx, data, index, name=None):
    ...
    def codegen(context, builder, signature, args):
        ...
        tlxwp__kfcf = construct_series(context, builder, series_type,
    ...
    if is_heterogeneous_tuple_type(data):
        chw__hbutb = HeterogeneousSeriesType(data, index, name)
    else:
        dtype = data.dtype
        data = if_series_to_array_type(data)
        chw__hbutb = SeriesType(dtype, data, index, name)
    sig = signature(chw__hbutb, data, index, name)
    return sig, codegen
...
def init_series_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    data = args[0]
    index = args[1]
    fih__oapmm = self.typemap[data.name]
    if is_heterogeneous_tuple_type(fih__oapmm) or isinstance(fih__oapmm,
        types.BaseTuple):
        return None
    cbphi__tloo = self.typemap[index.name]
    if not isinstance(cbphi__tloo, HeterogeneousIndexType
...
def get_series_payload(context, builder, series_type, value):
    jgzow__necat = cgutils.create_struct_proxy(series_type)(context,
        builder, value).meminfo
    payload_type = SeriesPayloadType(series_type)
    gkktu__vtei = context.nrt.meminfo_data(builder, jgzow__necat)
    kvcvi__vpuxl = context.get_value_type(payload_type).as_pointer()
    gkktu__vtei = builder.bitcast(gkktu__vtei, kvcvi__vpuxl)
    return context.make_helper(builder, payload_type, ref=gkktu__vtei)
...
def get_series_data(typingctx, series_typ=None):
    ...
    def codegen(context, builder, signature, args):
        xsyv__utlfd = get_series_payload(context, builder, signature.args[0
            ], args[0])
        return impl_ret_borrowed(context, builder, series_typ.data,
            xsyv__utlfd.data)
    chw__hbutb = series_typ.data
    sig = signature(chw__hbutb, series_typ)
    return sig, codegen
...
def get_series_data_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    vrijj__roghc = args[0]
    fih__oapmm = self.typemap[vrijj__roghc.name].data
    if is_heterogeneous_tuple_type(fih__oapmm) or isinstance(fih__oapmm,
        types.BaseTuple):
        return None
    if equiv_set.has_shape(vrijj__roghc):
        return ArrayAnalysis.AnalyzeResult(shape=vrijj__roghc, pre=[])
    return None
...
def get_series_index_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    assert len(args) == 1 and not kws
    vrijj__roghc = args[0]
    cbphi__tloo = self.typemap[vrijj__roghc.name].index
    if isinstance(cbphi__tloo, HeterogeneousIndexType):
        return None
    if equiv_set.has_shape(vrijj__roghc):
        return ArrayAnalysis.AnalyzeResult(shape=vrijj__roghc, pre=[])
    return None
...
def cast_series(context, builder, fromty, toty, val):
    if fromty.copy(index=toty.index) == toty and isinstance(fromty.index,
        ...
        xsyv__utlfd = get_series_payload(context, builder, fromty, val)
        kwjgy__hjyrp = context.cast(builder, xsyv__utlfd.index, fromty.
            index, toty.index)
        context.nrt.incref(builder, fromty.data, xsyv__utlfd.data)
        context.nrt.incref(builder, fromty.name_typ, xsyv__utlfd.name)
        return construct_series(context, builder, toty, xsyv__utlfd.data,
            kwjgy__hjyrp, xsyv__utlfd.name)
...
class SeriesAttribute(OverloadedKeyAttributeTemplate):
    ...
    def resolve_head(self, ary, args, kws):
        raegc__iuj = 'Series.head'
        ezviv__eqsnz = 'n',
        putio__iwx = {'n': 5}
        pysig, akeve__wnrf = bodo.utils.typing.fold_typing_args(raegc__iuj,
            args, kws, ezviv__eqsnz, putio__iwx)
        bblfs__rajbm = akeve__wnrf[0]
        if not is_overload_int(bblfs__rajbm):
            raise BodoError(f"{raegc__iuj}(): 'n' must be an Integer")
        jwbz__dek = ary
        return jwbz__dek(*akeve__wnrf).replace(pysig=pysig)
    ...
    def _resolve_map_func(self, ary, func, pysig, fname, f_args=None, kws=None
        ...
        if f_args is not None:
            kla__jis += tuple(f_args.types)
        if kws is None:
            kws = {}
        ydafr__rsfp = False
        flte__gtm = True
        if fname == 'map' and isinstance(func, types.DictType):
            cjd__eap = func.value_type
            ydafr__rsfp = True
        else:
            try:
                if types.unliteral(func) == types.unicode_type:
                    ...
                    cjd__eap = bodo.utils.transform.get_udf_str_return_type(ary
                        , get_overload_const_str(func), self.context,
                        'Series.apply')
                    flte__gtm = False
                elif bodo.utils.typing.is_numpy_ufunc(func):
                    cjd__eap = func.get_call_type(self.context, (ary,), {}
                        ).return_type
                    flte__gtm = False
                else:
                    cjd__eap = get_const_func_output_type(func, kla__jis,
                        kws, self.context, numba.core.registry.cpu_target.
                        target_context)
            except Exception as otg__qxtjb:
                raise BodoError(get_udf_error_msg(f'Series.{fname}()',
                    otg__qxtjb))
        if flte__gtm:
            if isinstance(cjd__eap, (SeriesType, HeterogeneousSeriesType)
                ) and cjd__eap.const_info is None:
                raise BodoError(
                    'Invalid Series output in UDF (Series with constant length and constant Index value expected)'
                    )
            if isinstance(cjd__eap, HeterogeneousSeriesType):
                nxm__ltl, tvzjs__ktmul = cjd__eap.const_info
                ygmfs__nbliu = tuple(dtype_to_array_type(t) for t in
                    cjd__eap.data.types)
                ulfk__dbb = bodo.DataFrameType(ygmfs__nbliu, ary.index,
                    tvzjs__ktmul)
            elif isinstance(cjd__eap, SeriesType):
                vyxn__aczkc, tvzjs__ktmul = cjd__eap.const_info
                ygmfs__nbliu = tuple(dtype_to_array_type(cjd__eap.dtype) for
                    nxm__ltl in range(vyxn__aczkc))
                ulfk__dbb = bodo.DataFrameType(ygmfs__nbliu, ary.index,
                    tvzjs__ktmul)
            else:
                yeif__njz = get_udf_out_arr_type(cjd__eap, ydafr__rsfp)
                ulfk__dbb = SeriesType(yeif__njz.dtype, yeif__njz, ary.
                    index, ary.name_typ)
        else:
            ulfk__dbb = cjd__eap
        return signature(ulfk__dbb, (func,)).replace(pysig=pysig)
    ...
    def resolve_map(self, ary, args, kws):
        ...
        na_action = args[1] if len(args) > 1 else kws.pop('na_action',
            types.none)
        rkrv__enqwr = dict(na_action=na_action)
        mhgbp__gevr = dict(na_action=None)
        check_unsupported_args('Series.map', rkrv__enqwr, mhgbp__gevr,
    ...
    def resolve_apply(self, ary, args, kws):
        ...
        kws.pop('func', None)
        kalzl__xfmic = args[1] if len(args) > 1 else kws.pop('convert_dtype',
            types.literal(True))
        f_args = args[2] if len(args) > 2 else kws.pop('args', None)
        rkrv__enqwr = dict(convert_dtype=kalzl__xfmic)
        pir__civhj = dict(convert_dtype=True)
        check_unsupported_args('Series.apply', rkrv__enqwr, pir__civhj,
            package_name='pandas', module_name='Series')
        mkzt__tgy = ', '.join("{} = ''".format(nvuuk__viy) for nvuuk__viy in
            kws.keys())
        ffsn__jhrm = (
            f'def apply_stub(func, convert_dtype=True, args=(), {mkzt__tgy}):\n'
            )
        ffsn__jhrm += '    pass\n'
        efdn__srne = {}
        exec(ffsn__jhrm, {}, efdn__srne)
        gtci__ifgs = efdn__srne['apply_stub']
        pysig = numba.core.utils.pysignature(gtci__ifgs)
    ...
    def _resolve_combine_func(self, ary, args, kws):
        ...
        agcu__ngau = ary.dtype
        if agcu__ngau == types.NPDatetime('ns'):
            agcu__ngau = pd_timestamp_type
        iqh__fvoz = other.dtype
        if iqh__fvoz == types.NPDatetime('ns'):
            iqh__fvoz = pd_timestamp_type
        cjd__eap = get_const_func_output_type(func, (agcu__ngau, iqh__fvoz),
            {}, self.context, numba.core.registry.cpu_target.target_context)
        sig = signature(SeriesType(cjd__eap, index=ary.index, name_typ=
...
def pd_series_overload(data=None, index=None, dtype=None, name=None, copy=
    ...
    jtiwz__ccy = is_overload_none(data)
    ddc__lxe = is_overload_none(index)
    kobq__zmz = is_overload_none(dtype)
    if jtiwz__ccy and ddc__lxe and kobq__zmz:
        raise BodoError(
            'pd.Series() requires at least 1 of data, index, and dtype to not be none'
            )
    if is_series_type(data) and not ddc__lxe:
        raise BodoError(
    ...
    if is_heterogeneous_tuple_type(data) and is_overload_none(dtype):
        ...
        def impl_heter(data=None, index=None, dtype=None, name=None, copy=
            False, fastpath=False):
            qfoyn__sdsud = bodo.utils.conversion.extract_index_if_none(data,
                index)
            odmkm__jlfkv = bodo.utils.conversion.to_tuple(data)
            return bodo.hiframes.pd_series_ext.init_series(odmkm__jlfkv,
                bodo.utils.conversion.convert_to_index(qfoyn__sdsud), name)
        return impl_heter
    if jtiwz__ccy:
        if kobq__zmz:
            ...
            def impl(data=None, index=None, dtype=None, name=None, copy=
                False, fastpath=False):
                pkeez__bod = bodo.utils.conversion.extract_name_if_none(data,
                    name)
                qfoyn__sdsud = bodo.utils.conversion.extract_index_if_none(data
                    , index)
                numba.parfors.parfor.init_prange()
                htp__rbhi = len(qfoyn__sdsud)
                odmkm__jlfkv = np.empty(htp__rbhi, np.float64)
                for furp__szlcb in numba.parfors.parfor.internal_prange(
                    htp__rbhi):
                    bodo.libs.array_kernels.setna(odmkm__jlfkv, furp__szlcb)
                return bodo.hiframes.pd_series_ext.init_series(odmkm__jlfkv,
                    bodo.utils.conversion.convert_to_index(qfoyn__sdsud),
                    pkeez__bod)
            return impl
        if bodo.utils.conversion._is_str_dtype(dtype):
            mvvek__zjkam = bodo.string_array_type
        else:
            nugz__lqk = bodo.utils.typing.parse_dtype(dtype, 'pandas.Series')
            if isinstance(nugz__lqk, bodo.libs.int_arr_ext.IntDtype):
                mvvek__zjkam = bodo.IntegerArrayType(nugz__lqk.dtype)
            elif nugz__lqk == bodo.libs.bool_arr_ext.boolean_dtype:
                mvvek__zjkam = bodo.boolean_array
            elif isinstance(nugz__lqk, types.Number) or nugz__lqk in [bodo.
                datetime64ns, bodo.timedelta64ns]:
                mvvek__zjkam = types.Array(nugz__lqk, 1, 'C')
            else:
                raise BodoError(
        ...
        if ddc__lxe:
            ...
            def impl(data=None, index=None, dtype=None, name=None, copy=
                ...
                pkeez__bod = bodo.utils.conversion.extract_name_if_none(data,
                    name)
                qfoyn__sdsud = bodo.hiframes.pd_index_ext.init_range_index(
                    0, 0, 1, None)
                numba.parfors.parfor.init_prange()
                htp__rbhi = len(qfoyn__sdsud)
                odmkm__jlfkv = bodo.utils.utils.alloc_type(htp__rbhi,
                    mvvek__zjkam, (-1,))
                return bodo.hiframes.pd_series_ext.init_series(odmkm__jlfkv,
                    qfoyn__sdsud, pkeez__bod)
            return impl
        else:
    ...
    def impl(data=None, index=None, dtype=None, name=None, copy=False,
        fastpath=False):
        pkeez__bod = bodo.utils.conversion.extract_name_if_none(data, name)
        qfoyn__sdsud = bodo.utils.conversion.extract_index_if_none(data, index)
        gnnqr__olzlh = bodo.utils.conversion.coerce_to_array(data, True,
            scalar_to_arr_len=len(qfoyn__sdsud))
        bbfw__ostyk = bodo.utils.conversion.fix_arr_dtype(gnnqr__olzlh,
            dtype, None, False)
        return bodo.hiframes.pd_series_ext.init_series(bbfw__ostyk, bodo.
            utils.conversion.convert_to_index(qfoyn__sdsud), pkeez__bod)
    return impl
...
def lower_constant_series(context, builder, series_type, pyval):
    ...
    name_val = context.get_constant_generic(builder, series_type.name_typ,
        pyval.name)
    gkktu__vtei = lir.Constant.literal_struct([data_val, index_val, name_val])
    gkktu__vtei = cgutils.global_constant(builder, '.const.payload',
        gkktu__vtei).bitcast(cgutils.voidptr_t)
    yctn__nkjaz = context.get_constant(types.int64, -1)
    luo__qpse = context.get_constant_null(types.voidptr)
    jgzow__necat = lir.Constant.literal_struct([yctn__nkjaz, luo__qpse,
        luo__qpse, gkktu__vtei, yctn__nkjaz])
    jgzow__necat = cgutils.global_constant(builder, '.const.meminfo',
        jgzow__necat).bitcast(cgutils.voidptr_t)
    tlxwp__kfcf = lir.Constant.literal_struct([jgzow__necat, luo__qpse])
    return tlxwp__kfcf
...
def _install_series_unsupported():
    for bewu__yhiii in series_unsupported_attrs:
        qrs__tuos = 'Series.' + bewu__yhiii
        overload_attribute(SeriesType, bewu__yhiii)(create_unsupported_overload
            (qrs__tuos))
    for fname in series_unsupported_methods:
        qrs__tuos = 'Series.' + fname
        overload_method(SeriesType, fname, no_unliteral=True)(
            create_unsupported_overload(qrs__tuos))
...
def _install_heter_series_unsupported():
    for bewu__yhiii in heter_series_unsupported_attrs:
        qrs__tuos = 'HeterogeneousSeries.' + bewu__yhiii
        overload_attribute(HeterogeneousSeriesType, bewu__yhiii)(
            create_unsupported_overload(qrs__tuos))
    for fname in heter_series_unsupported_methods:
        qrs__tuos = 'HeterogeneousSeries.' + fname
