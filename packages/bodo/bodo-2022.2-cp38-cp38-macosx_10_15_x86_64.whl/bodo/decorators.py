...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        kqnt__ohqw = self.overloads[sig]
        jink__fcawx = kqnt__ohqw.metadata.get('distributed_diagnostics', None)
        if jink__fcawx is None:
            lcpdw__inmzq = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(lcpdw__inmzq)
        jink__fcawx.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        sbekd__zyr = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(sbekd__zyr)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        sheb__lghm = options.pop('distributed')
        pipeline_class = (pipeline_class if sheb__lghm else bodo.compiler.
            BodoCompilerSeq)
    wxhi__ldczx = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(wxhi__ldczx, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(wxhi__ldczx)
        else:
            return master_mode_wrapper(wxhi__ldczx)
    else:
        return wxhi__ldczx
