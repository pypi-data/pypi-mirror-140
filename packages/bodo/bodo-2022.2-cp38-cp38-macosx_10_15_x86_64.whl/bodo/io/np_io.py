...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        xlwjp__lvpc = context.get_abi_sizeof(context.get_data_type(dtype.dtype)
            )
        return context.get_constant(types.intp, xlwjp__lvpc)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            utwj__zbbx = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(utwj__zbbx.dtype)
            file_write(unicode_to_utf8(fname), utwj__zbbx.ctypes, 
                dtype_size * utwj__zbbx.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            utwj__zbbx = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(utwj__zbbx.dtype)
            jvs__kwr = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                utwj__zbbx.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), utwj__zbbx.ctypes,
                start, count, jvs__kwr)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            lmu__hhxzw = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                lmu__hhxzw = min(lmu__hhxzw, count * dtype_size)
            if lmu__hhxzw < 0:
                return -1
            return lmu__hhxzw
        return impl
