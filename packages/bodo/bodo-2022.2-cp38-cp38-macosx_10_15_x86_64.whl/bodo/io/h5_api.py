...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        jtmq__xdqq = []
        vmugg__bgy = h5g_get_num_objs(obj_id)
        for xml__shhe in range(vmugg__bgy):
            zmxut__yuif = h5g_get_objname_by_idx(obj_id, xml__shhe)
            jtmq__xdqq.append(zmxut__yuif)
        return jtmq__xdqq
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    quq__ktgc = parse_dtype(dtype)
    ovsm__btvxn = np.int32(numba_to_c_type(quq__ktgc))
    ...
    def impl(obj_id, name, shape=None, dtype=None, data=None):
        ...
        return h5_create_dset(unify_h5_id(obj_id), unicode_to_utf8(name),
            ndim, counts.ctypes, ovsm__btvxn)
    return impl
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            adj__oxgy = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    jufb__mkip = tuple_to_ptr(starts)
    qwa__kioby = tuple_to_ptr(counts)
    gzn__tidm = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, jufb__mkip, qwa__kioby,
        is_parallel, out_arr.ctypes, gzn__tidm)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    qwa__kioby = tuple_to_ptr(counts)
    gzn__tidm = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, jufb__mkip, qwa__kioby,
        is_parallel, out_arr.ctypes, gzn__tidm)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        dyszd__ecedy = types.Array(dtype, ndim, 'C')
        pjalj__tbvk = args[3]
        dyszd__ecedy = self.context.resolve_function_type(operator.getitem,
            [dyszd__ecedy, pjalj__tbvk], {}).return_type.copy(layout='C')
        return signature(dyszd__ecedy, *args)
...
def get_filter_read_indices(bool_arr):
    jja__feno = bool_arr.nonzero()[0]
    ywd__shj = bodo.libs.distributed_api.get_rank()
    may__svnw = bodo.libs.distributed_api.get_size()
    umsdh__rzw = np.empty(may__svnw, np.int64)
    zdys__uyzxl = len(bool_arr)
    bodo.libs.distributed_api.allgather(umsdh__rzw, zdys__uyzxl)
    zhr__zbn = umsdh__rzw.cumsum()[ywd__shj] - zdys__uyzxl
    jja__feno += zhr__zbn
    yyh__min = bodo.libs.distributed_api.dist_reduce(len(jja__feno), np.
        int32(sum_op))
    hfutt__mnyd = bodo.libs.distributed_api.gatherv(jja__feno)
    if ywd__shj == 0:
        hywkm__poc = hfutt__mnyd
    else:
        hywkm__poc = np.empty(yyh__min, jja__feno.dtype)
    bodo.libs.distributed_api.bcast(hywkm__poc)
    xex__qwg = bodo.libs.distributed_api.get_start(yyh__min, may__svnw,
        ywd__shj)
    xvfw__xwgt = bodo.libs.distributed_api.get_end(yyh__min, may__svnw,
        ywd__shj)
    return hywkm__poc[xex__qwg:xvfw__xwgt]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        uggv__dadbx = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], uggv__dadbx)
        return builder.bitcast(uggv__dadbx, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    qwa__kioby = tuple_to_ptr(counts)
    gzn__tidm = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, jufb__mkip,
        qwa__kioby, is_parallel, out_arr.ctypes, gzn__tidm, read_indices.
