...
except ImportError as aogaz__ivtnc:
    pass


def check_sklearn_version():
    if not _is_sklearn_supported_version:
        vpncy__kun = f""" Bodo supports scikit-learn version >= {_min_sklearn_ver_str} and < {_max_sklearn_ver_str}.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(vpncy__kun)


def random_forest_model_fit(m, X, y):
    dzkv__rinw = m.n_estimators
    fhhf__zvug = MPI.Get_processor_name()
    kaw__okz = get_host_ranks()
    fwdnw__gnl = len(kaw__okz)
    lto__ody = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(dzkv__rinw,
        fwdnw__gnl, lto__ody)
    if lto__ody == kaw__okz[fhhf__zvug][0]:
        m.n_jobs = len(kaw__okz[fhhf__zvug])
    ...
    mmgrc__btr = create_subcomm_mpi4py(first_rank_node)
    if mmgrc__btr != MPI.COMM_NULL:
        dwe__qqvfz = 10
        dbrn__hwwj = bodo.libs.distributed_api.get_node_portion(dzkv__rinw,
            fwdnw__gnl, 0)
        gzjjb__inrx = dbrn__hwwj // dwe__qqvfz
        if dbrn__hwwj % dwe__qqvfz != 0:
            gzjjb__inrx += 1
        ges__ooaaq = []
        for pcelm__wbh in range(gzjjb__inrx):
            xmrp__wti = mmgrc__btr.gather(m.estimators_[pcelm__wbh *
                dwe__qqvfz:pcelm__wbh * dwe__qqvfz + dwe__qqvfz])
            if lto__ody == 0:
                ges__ooaaq += list(itertools.chain.from_iterable(xmrp__wti))
        if lto__ody == 0:
            m.estimators_ = ges__ooaaq
    ptj__rmpl = MPI.COMM_WORLD
    if lto__ody == 0:
        for pcelm__wbh in range(0, dzkv__rinw, 10):
            ptj__rmpl.bcast(m.estimators_[pcelm__wbh:pcelm__wbh + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            ptj__rmpl.bcast(m.n_classes_)
            ptj__rmpl.bcast(m.classes_)
        ptj__rmpl.bcast(m.n_outputs_)
    else:
        dwn__degq = []
        for pcelm__wbh in range(0, dzkv__rinw, 10):
            dwn__degq += ptj__rmpl.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = ptj__rmpl.bcast(None)
            m.classes_ = ptj__rmpl.bcast(None)
        m.n_outputs_ = ptj__rmpl.bcast(None)
        m.estimators_ = dwn__degq
    assert len(m.estimators_) == dzkv__rinw
    m.n_estimators = dzkv__rinw
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    czh__nbi = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
        ...
        sklearn.metrics._classification.multilabel_confusion_matrix = czh__nbi
    return result
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    wfnlz__dpjn = len(labels)
    evuiu__iqw = np.zeros(wfnlz__dpjn, np.int64)
    aefjf__jgz = np.zeros(wfnlz__dpjn, np.int64)
    lut__wxley = np.zeros(wfnlz__dpjn, np.int64)
    eeldc__hicd = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for pcelm__wbh in range(len(y_true)):
        aefjf__jgz[eeldc__hicd[y_true[pcelm__wbh]]] += 1
        if y_pred[pcelm__wbh] not in eeldc__hicd:
            continue
        hbpq__vnpg = eeldc__hicd[y_pred[pcelm__wbh]]
        lut__wxley[hbpq__vnpg] += 1
        if y_true[pcelm__wbh] == y_pred[pcelm__wbh]:
            evuiu__iqw[hbpq__vnpg] += 1
    evuiu__iqw = bodo.libs.distributed_api.dist_reduce(evuiu__iqw, np.int32
        (Reduce_Type.Sum.value))
    aefjf__jgz = bodo.libs.distributed_api.dist_reduce(aefjf__jgz, np.int32
        (Reduce_Type.Sum.value))
    lut__wxley = bodo.libs.distributed_api.dist_reduce(lut__wxley, np.int32
        (Reduce_Type.Sum.value))
    gfwq__oyf = lut__wxley - evuiu__iqw
    qccu__sfqhs = aefjf__jgz - evuiu__iqw
    luvpp__aqa = evuiu__iqw
    mqgpn__ghfa = y_true.shape[0] - luvpp__aqa - gfwq__oyf - qccu__sfqhs
    with numba.objmode(result='float64[:]'):
        MCM = np.array([mqgpn__ghfa, gfwq__oyf, qccu__sfqhs, luvpp__aqa]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        rtax__dggxz = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        rtax__dggxz = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    ptj__rmpl = MPI.COMM_WORLD
    fjh__kllna = ptj__rmpl.Get_size()
    if sample_weight is not None:
        pgabw__pxsb = np.sum(sample_weight)
    else:
        pgabw__pxsb = np.float64(y_true.shape[0])
    yhy__fqos = np.zeros(fjh__kllna, dtype=type(pgabw__pxsb))
    ptj__rmpl.Allgather(pgabw__pxsb, yhy__fqos)
    fywc__mfn = np.zeros((fjh__kllna, *rtax__dggxz.shape), dtype=
        rtax__dggxz.dtype)
    ptj__rmpl.Allgather(rtax__dggxz, fywc__mfn)
    ekkzf__mgtmc = np.average(fywc__mfn, weights=yhy__fqos, axis=0)
    if metric == 'mse' and not squared:
        ekkzf__mgtmc = np.sqrt(ekkzf__mgtmc)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return ekkzf__mgtmc
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(ekkzf__mgtmc)
    else:
        return np.average(ekkzf__mgtmc, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    ptj__rmpl = MPI.COMM_WORLD
    score = ptj__rmpl.allreduce(score, op=MPI.SUM)
    if normalize:
        mbmt__hajln = np.sum(sample_weight
            ) if sample_weight is not None else len(y_true)
        mbmt__hajln = ptj__rmpl.allreduce(mbmt__hajln, op=MPI.SUM)
        score = score / mbmt__hajln
    return score
...
def check_consistent_length_parallel(*arrays):
    ptj__rmpl = MPI.COMM_WORLD
    djb__ozn = True
    uya__pprgs = [len(tsnze__aco) for tsnze__aco in arrays if tsnze__aco is not
        None]
    if len(np.unique(uya__pprgs)) > 1:
        djb__ozn = False
    djb__ozn = ptj__rmpl.allreduce(djb__ozn, op=MPI.LAND)
    return djb__ozn
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    wwvt__ljip = y_true.shape[0]
    xyu__jwjn = ptj__rmpl.allreduce(wwvt__ljip, op=MPI.SUM)
    if xyu__jwjn < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        cdmw__pek = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        cdmw__pek = 1.0
    mdm__dxv = (cdmw__pek * (y_true - y_pred) ** 2).sum(axis=0, dtype=np.
        float64)
    ktv__sellg = np.zeros(mdm__dxv.shape, dtype=mdm__dxv.dtype)
    ptj__rmpl.Allreduce(mdm__dxv, ktv__sellg, op=MPI.SUM)
    abr__fxzgy = np.nansum(y_true * cdmw__pek, axis=0, dtype=np.float64)
    hxmqf__bvvjs = np.zeros_like(abr__fxzgy)
    ptj__rmpl.Allreduce(abr__fxzgy, hxmqf__bvvjs, op=MPI.SUM)
    sjio__mde = np.nansum(sample_weight, dtype=np.float64)
    wzu__fyk = ptj__rmpl.allreduce(sjio__mde, op=MPI.SUM)
    itbd__vkr = hxmqf__bvvjs / wzu__fyk
    hdn__qfs = (cdmw__pek * (y_true - itbd__vkr) ** 2).sum(axis=0, dtype=np
        .float64)
    bcm__pxiqu = np.zeros(hdn__qfs.shape, dtype=hdn__qfs.dtype)
    ptj__rmpl.Allreduce(hdn__qfs, bcm__pxiqu, op=MPI.SUM)
    hmyn__uznzk = bcm__pxiqu != 0
    xwg__vplf = ktv__sellg != 0
    eynv__zbym = hmyn__uznzk & xwg__vplf
    gywd__ksbwv = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    gywd__ksbwv[eynv__zbym] = 1 - ktv__sellg[eynv__zbym] / bcm__pxiqu[
        eynv__zbym]
    gywd__ksbwv[xwg__vplf & ~hmyn__uznzk] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            edri__fvlk = bcm__pxiqu
            if not np.any(hmyn__uznzk):
                if not np.any(xwg__vplf):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        edri__fvlk = multioutput
    return np.array([np.average(gywd__ksbwv, weights=edri__fvlk)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    ptj__rmpl = MPI.COMM_WORLD
    try:
        kcwaf__thqpt = sklearn.metrics.confusion_matrix(y_true, y_pred,
            labels=labels, sample_weight=sample_weight, normalize=None)
    except ValueError as nexja__vdulm:
        kcwaf__thqpt = nexja__vdulm
    jvz__pjvh = (isinstance(kcwaf__thqpt, ValueError) and 
        'At least one label specified must be in y_true' in kcwaf__thqpt.
        args[0])
    itbe__cjrt = ptj__rmpl.allreduce(jvz__pjvh, op=MPI.LAND)
    if itbe__cjrt:
        raise kcwaf__thqpt
    elif jvz__pjvh:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        ilrk__ckqzr = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        ilrk__ckqzr = kcwaf__thqpt
    gcotj__plzx = np.zeros_like(ilrk__ckqzr)
    ptj__rmpl.Allreduce(ilrk__ckqzr, gcotj__plzx)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            gcotj__plzx = gcotj__plzx / gcotj__plzx.sum(axis=1, keepdims=True)
        elif normalize == 'pred':
            gcotj__plzx = gcotj__plzx / gcotj__plzx.sum(axis=0, keepdims=True)
        elif normalize == 'all':
            gcotj__plzx = gcotj__plzx / gcotj__plzx.sum()
        gcotj__plzx = np.nan_to_num(gcotj__plzx)
    return gcotj__plzx
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    wxqwp__cygsj = 'def _confusion_matrix_impl(\n'
    wxqwp__cygsj += '    y_true, y_pred, labels=None,\n'
    wxqwp__cygsj += '    sample_weight=None, normalize=None,\n'
    wxqwp__cygsj += '    _is_data_distributed=False,\n'
    wxqwp__cygsj += '):\n'
    wxqwp__cygsj += (
        '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n')
    wxqwp__cygsj += (
        '    y_pred = bodo.utils.conversion.coerce_to_array(y_pred)\n')
    yhg__qnuxc = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        yhg__qnuxc = 'float64[:,:]', 'np.float64'
    if not is_overload_none(sample_weight):
        wxqwp__cygsj += """    sample_weight = bodo.utils.conversion.coerce_to_array(sample_weight)
"""
    ...
    if not is_overload_none(labels):
        ...
        wxqwp__cygsj += '    labels = bodo.allgatherv(labels, False)\n'
        wxqwp__cygsj += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    wxqwp__cygsj += f"    with numba.objmode(cm='{yhg__qnuxc[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        wxqwp__cygsj += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        wxqwp__cygsj += '      cm = confusion_matrix_dist_helper(\n'
    wxqwp__cygsj += '        y_true, y_pred, labels=labels,\n'
    wxqwp__cygsj += (
        '        sample_weight=sample_weight, normalize=normalize,\n')
    wxqwp__cygsj += f'      ).astype({yhg__qnuxc[1]})\n'
    wxqwp__cygsj += '    return cm\n'
    hso__ygnmz = {}
    exec(wxqwp__cygsj, globals(), hso__ygnmz)
    rinsl__tkjb = hso__ygnmz['_confusion_matrix_impl']
    return rinsl__tkjb
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    ptj__rmpl = MPI.COMM_WORLD
    fidgs__kimyq = ptj__rmpl.allreduce(len(X), op=MPI.SUM)
    uvgc__yks = len(X) / fidgs__kimyq
    szj__bukfy = ptj__rmpl.Get_size()
    m.n_jobs = 1
    m.early_stopping = False
    cpn__noc = np.inf
    srrev__oxh = 0
    if m.loss == 'hinge':
        zcneo__phrai = hinge_loss
    elif m.loss == 'log':
        zcneo__phrai = log_loss
    elif m.loss == 'squared_error':
        zcneo__phrai = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    fkwr__jvlm = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        fkwr__jvlm = True
    for did__svs in range(m.max_iter):
        if fkwr__jvlm:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * uvgc__yks
        m.coef_ = ptj__rmpl.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * uvgc__yks
        m.intercept_ = ptj__rmpl.allreduce(m.intercept_, op=MPI.SUM)
        if fkwr__jvlm:
            y_pred = m.predict(X)
            uhjfr__hwm = zcneo__phrai(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            uhjfr__hwm = zcneo__phrai(y, y_pred, labels=y_classes)
        zbrzz__mjzvz = ptj__rmpl.allreduce(uhjfr__hwm, op=MPI.SUM)
        uhjfr__hwm = zbrzz__mjzvz / szj__bukfy
        if m.tol > np.NINF and uhjfr__hwm > cpn__noc - m.tol * fidgs__kimyq:
            srrev__oxh += 1
        else:
            srrev__oxh = 0
        if uhjfr__hwm < cpn__noc:
            cpn__noc = uhjfr__hwm
        if srrev__oxh >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    ptj__rmpl = MPI.COMM_WORLD
    lto__ody = ptj__rmpl.Get_rank()
    fhhf__zvug = MPI.Get_processor_name()
    kaw__okz = get_host_ranks()
    erkc__smrj = m.n_jobs if hasattr(m, 'n_jobs') else None
    udvcy__cef = m._n_threads if hasattr(m, '_n_threads') else None
    m._n_threads = len(kaw__okz[fhhf__zvug])
    if lto__ody == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if lto__ody == 0:
        ptj__rmpl.bcast(m.cluster_centers_)
        ptj__rmpl.bcast(m.inertia_)
        ptj__rmpl.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = ptj__rmpl.bcast(None)
        m.inertia_ = ptj__rmpl.bcast(None)
        m.n_iter_ = ptj__rmpl.bcast(None)
    if _is_data_distributed:
        aqy__aks = ptj__rmpl.allgather(len_X)
        if lto__ody == 0:
            mgi__dcvu = np.empty(len(aqy__aks) + 1, dtype=int)
            np.cumsum(aqy__aks, out=mgi__dcvu[1:])
            mgi__dcvu[0] = 0
            sko__moiww = [m.labels_[mgi__dcvu[jsn__uxip]:mgi__dcvu[
                jsn__uxip + 1]] for jsn__uxip in range(len(aqy__aks))]
            aloj__zohxh = ptj__rmpl.scatter(sko__moiww)
        else:
            aloj__zohxh = ptj__rmpl.scatter(None)
        m.labels_ = aloj__zohxh
    elif lto__ody == 0:
        ptj__rmpl.bcast(m.labels_)
    else:
        m.labels_ = ptj__rmpl.bcast(None)
    m._n_threads = udvcy__cef
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                ptj__rmpl = MPI.COMM_WORLD
                result = ptj__rmpl.allreduce(result, op=MPI.SUM)
            m._n_threads = udvcy__cef
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        wxqwp__cygsj = 'def _model_multinomial_nb_fit_impl(\n'
        wxqwp__cygsj += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        wxqwp__cygsj += '):  # pragma: no cover\n'
        wxqwp__cygsj += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            wxqwp__cygsj += '    X = X.to_numpy()\n'
        else:
            wxqwp__cygsj += (
                '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n')
        wxqwp__cygsj += '    my_rank = bodo.get_rank()\n'
        wxqwp__cygsj += '    nranks = bodo.get_size()\n'
        wxqwp__cygsj += '    total_cols = X.shape[1]\n'
        wxqwp__cygsj += '    for i in range(nranks):\n'
        wxqwp__cygsj += """        start = bodo.libs.distributed_api.get_start(total_cols, nranks, i)
"""
        wxqwp__cygsj += (
            '        end = bodo.libs.distributed_api.get_end(total_cols, nranks, i)\n'
            )
        wxqwp__cygsj += '        if i == my_rank:\n'
        wxqwp__cygsj += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        wxqwp__cygsj += '        else:\n'
        wxqwp__cygsj += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        wxqwp__cygsj += '    y_train = bodo.allgatherv(y, False)\n'
        wxqwp__cygsj += '    with numba.objmode(m="multinomial_nb_type"):\n'
        wxqwp__cygsj += '        m = fit_multinomial_nb(\n'
        wxqwp__cygsj += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        wxqwp__cygsj += '        )\n'
        wxqwp__cygsj += '    bodo.barrier()\n'
        wxqwp__cygsj += '    return m\n'
        hso__ygnmz = {}
        exec(wxqwp__cygsj, globals(), hso__ygnmz)
        ukzr__lwlk = hso__ygnmz['_model_multinomial_nb_fit_impl']
        return ukzr__lwlk
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    did__svs, n_features = X_train.shape
    m.n_features_in_ = n_features
    xmv__sngp = LabelBinarizer()
    dstei__bzwq = xmv__sngp.fit_transform(y_train)
    m.classes_ = xmv__sngp.classes_
    if dstei__bzwq.shape[1] == 1:
        dstei__bzwq = np.concatenate((1 - dstei__bzwq, dstei__bzwq), axis=1)
    if sample_weight is not None:
        dstei__bzwq = dstei__bzwq.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        dstei__bzwq *= sample_weight.T
    class_prior = m.class_prior
    uhdby__cbzgv = dstei__bzwq.shape[1]
    m._init_counters(uhdby__cbzgv, n_features)
    m._count(X_train.astype('float64'), dstei__bzwq)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    ukle__sgwl = m.feature_count_ + alpha
    kqrdi__ycr = ukle__sgwl.sum(axis=1)
    ptj__rmpl = MPI.COMM_WORLD
    szj__bukfy = ptj__rmpl.Get_size()
    cow__zhhgx = np.zeros(uhdby__cbzgv)
    ptj__rmpl.Allreduce(kqrdi__ycr, cow__zhhgx, op=MPI.SUM)
    icwq__grdik = np.log(ukle__sgwl) - np.log(cow__zhhgx.reshape(-1, 1))
    mggv__xnx = icwq__grdik.T.reshape(n_features * uhdby__cbzgv)
    uyaq__tbemp = np.ones(szj__bukfy) * (total_cols // szj__bukfy)
    uud__wvspc = total_cols % szj__bukfy
    for zfaig__agv in range(uud__wvspc):
        uyaq__tbemp[zfaig__agv] += 1
    uyaq__tbemp *= uhdby__cbzgv
    jhbj__cwdc = np.zeros(szj__bukfy, dtype=np.int32)
    jhbj__cwdc[1:] = np.cumsum(uyaq__tbemp)[:-1]
    cjs__yww = np.zeros((total_cols, uhdby__cbzgv), dtype=np.float64)
    ptj__rmpl.Allgatherv(mggv__xnx, [cjs__yww, uyaq__tbemp, jhbj__cwdc, MPI
        .DOUBLE_PRECISION])
    m.feature_log_prob_ = cjs__yww.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    ptj__rmpl = MPI.COMM_WORLD
    fjh__kllna = ptj__rmpl.Get_size()
    wgyey__atdw = m.with_std
    gpk__nula = m.with_mean
    m.with_std = False
    if wgyey__atdw:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = wgyey__atdw
    m.with_mean = gpk__nula
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    mbpv__ftd = np.zeros((fjh__kllna, *m.n_samples_seen_.shape), dtype=m.
        n_samples_seen_.dtype)
    ptj__rmpl.Allgather(m.n_samples_seen_, mbpv__ftd)
    maq__dozlq = np.sum(mbpv__ftd, axis=0)
    m.n_samples_seen_ = maq__dozlq
    if m.with_mean or m.with_std:
        frfj__ryui = np.zeros((fjh__kllna, *m.mean_.shape), dtype=m.mean_.dtype
            )
        ptj__rmpl.Allgather(m.mean_, frfj__ryui)
        frfj__ryui[np.isnan(frfj__ryui)] = 0
        dzcwf__ymemz = np.average(frfj__ryui, axis=0, weights=mbpv__ftd)
        m.mean_ = dzcwf__ymemz
    if m.with_std:
        xfg__ksp = sklearn_safe_accumulator_op(np.nansum, (X - dzcwf__ymemz
            ) ** 2, axis=0) / maq__dozlq
        wvf__wtodu = np.zeros_like(xfg__ksp)
        ptj__rmpl.Allreduce(xfg__ksp, wvf__wtodu, op=MPI.SUM)
        m.var_ = wvf__wtodu
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    zmn__qko = ptj__rmpl.allreduce(zmn__qko, op=MPI.LAND)
    if zmn__qko:
...
def get_data_slice_parallel(data, labels, len_train):
    kghqr__fabw = data[:len_train]
    vlv__ipylm = data[len_train:]
    kghqr__fabw = bodo.rebalance(kghqr__fabw)
    vlv__ipylm = bodo.rebalance(vlv__ipylm)
    dge__srht = labels[:len_train]
    fjzvc__itfa = labels[len_train:]
    dge__srht = bodo.rebalance(dge__srht)
    fjzvc__itfa = bodo.rebalance(fjzvc__itfa)
    return kghqr__fabw, vlv__ipylm, dge__srht, fjzvc__itfa
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    jab__ldq = {'stratify': stratify}
    fehy__ynie = {'stratify': None}
    check_unsupported_args('train_test_split', jab__ldq, fehy__ynie, 'ml')
    if is_overload_false(_is_data_distributed):
        rtbn__kdk = f'data_split_type_{numba.core.ir_utils.next_label()}'
        xpic__lygt = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for wuqr__ojt, sqtye__izdb in ((data, rtbn__kdk), (labels, xpic__lygt)
            ):
            if isinstance(wuqr__ojt, (DataFrameType, SeriesType)):
                xows__ske = wuqr__ojt.copy(index=NumericIndexType(types.int64))
                setattr(types, sqtye__izdb, xows__ske)
            else:
                setattr(types, sqtye__izdb, wuqr__ojt)
        wxqwp__cygsj = 'def _train_test_split_impl(\n'
        wxqwp__cygsj += '    data,\n'
        wxqwp__cygsj += '    labels=None,\n'
        wxqwp__cygsj += '    train_size=None,\n'
        wxqwp__cygsj += '    test_size=None,\n'
        wxqwp__cygsj += '    random_state=None,\n'
        wxqwp__cygsj += '    shuffle=True,\n'
        wxqwp__cygsj += '    stratify=None,\n'
        wxqwp__cygsj += '    _is_data_distributed=False,\n'
        wxqwp__cygsj += '):  # pragma: no cover\n'
        wxqwp__cygsj += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(rtbn__kdk, rtbn__kdk, xpic__lygt, xpic__lygt))
        wxqwp__cygsj += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        wxqwp__cygsj += '            data,\n'
        wxqwp__cygsj += '            labels,\n'
        wxqwp__cygsj += '            train_size=train_size,\n'
        wxqwp__cygsj += '            test_size=test_size,\n'
        wxqwp__cygsj += '            random_state=random_state,\n'
        wxqwp__cygsj += '            shuffle=shuffle,\n'
        wxqwp__cygsj += '            stratify=stratify,\n'
        wxqwp__cygsj += '        )\n'
        wxqwp__cygsj += (
            '    return data_train, data_test, labels_train, labels_test\n')
        hso__ygnmz = {}
        exec(wxqwp__cygsj, globals(), hso__ygnmz)
        _train_test_split_impl = hso__ygnmz['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            xtq__brwu = bodo.libs.distributed_api.dist_reduce(len(data), np
                .int32(Reduce_Type.Sum.value))
            len_train = int(xtq__brwu * train_size)
            sdgw__xwwq = xtq__brwu - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                lto__ody = bodo.get_rank()
                szj__bukfy = bodo.get_size()
                vqc__mqiy = np.empty(szj__bukfy, np.int64)
                bodo.libs.distributed_api.allgather(vqc__mqiy, len(data))
                bof__fnvin = np.cumsum(vqc__mqiy[0:lto__ody + 1])
                xei__emg = np.full(xtq__brwu, True)
                xei__emg[:sdgw__xwwq] = False
                np.random.seed(42)
                np.random.permutation(xei__emg)
                if lto__ody:
                    hirxv__wuzz = bof__fnvin[lto__ody - 1]
                else:
                    hirxv__wuzz = 0
                yxgii__vjsjb = bof__fnvin[lto__ody]
                kdl__hcj = xei__emg[hirxv__wuzz:yxgii__vjsjb]
                kghqr__fabw = data[kdl__hcj]
                vlv__ipylm = data[~kdl__hcj]
                dge__srht = labels[kdl__hcj]
                fjzvc__itfa = labels[~kdl__hcj]
                kghqr__fabw = bodo.random_shuffle(kghqr__fabw, seed=
                    random_state, parallel=True)
                vlv__ipylm = bodo.random_shuffle(vlv__ipylm, seed=
                    random_state, parallel=True)
                dge__srht = bodo.random_shuffle(dge__srht, seed=
                    random_state, parallel=True)
                fjzvc__itfa = bodo.random_shuffle(fjzvc__itfa, seed=
                    random_state, parallel=True)
                dge__srht = reset_labels_type(dge__srht, label_type)
                fjzvc__itfa = reset_labels_type(fjzvc__itfa, label_type)
            else:
                kghqr__fabw, vlv__ipylm, dge__srht, fjzvc__itfa = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    fjh__kllna = ptj__rmpl.Get_size()
    m = m.fit(X)
    maq__dozlq = ptj__rmpl.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = maq__dozlq
    xzu__fbe = np.zeros((fjh__kllna, *m.data_min_.shape), dtype=m.data_min_
        .dtype)
    ptj__rmpl.Allgather(m.data_min_, xzu__fbe)
    bzph__iyos = np.nanmin(xzu__fbe, axis=0)
    seh__qjih = np.zeros((fjh__kllna, *m.data_max_.shape), dtype=m.
        data_max_.dtype)
    ptj__rmpl.Allgather(m.data_max_, seh__qjih)
    ugkp__lbfqa = np.nanmax(seh__qjih, axis=0)
    waftx__frm = ugkp__lbfqa - bzph__iyos
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(waftx__frm)
    m.min_ = m.feature_range[0] - bzph__iyos * m.scale_
    m.data_min_ = bzph__iyos
    m.data_max_ = ugkp__lbfqa
    m.data_range_ = waftx__frm
    return m
...
def overload_rf_classifier_model_fit(m, X, y, sample_weight=None,
    _is_data_distributed=False):
    giwu__dys = 'RandomForestClassifier'
    if isinstance(m, BodoRandomForestRegressorType):
        giwu__dys = 'RandomForestRegressor'
    if not is_overload_none(sample_weight):
        raise BodoError(
            f"sklearn.ensemble.{giwu__dys}.fit() : 'sample_weight' is not supported for distributed data."
            )
    ...
    def _model_fit_impl(m, X, y, sample_weight=None, _is_data_distributed=False
        ...
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            fwdnw__gnl = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if fwdnw__gnl > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=fwdnw__gnl)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        ycfzu__jfa = True
    return ycfzu__jfa, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                fig__pnx = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                fig__pnx = bodo.allgatherv(fig__pnx, False)
                fig__pnx = bodo.libs.array_kernels.sort(fig__pnx, ascending
                    =True, inplace=True)
                esifg__cdzw = {}
                for pcelm__wbh in range(len(fig__pnx)):
                    esifg__cdzw[fig__pnx[pcelm__wbh]] = pcelm__wbh
            else:
                esifg__cdzw = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = esifg__cdzw
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X
