...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    npkkc__pjiz = pq_node.out_vars[0].name
    prazn__gpixu = pq_node.out_vars[1].name
    if npkkc__pjiz not in lives and prazn__gpixu not in lives:
        return None
    elif npkkc__pjiz not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif prazn__gpixu not in lives:
...
def pq_remove_dead_column(pq_node, column_live_map, equiv_vars, typemap):
    ...
    djd__tbkzz = pq_node.out_vars[0].name
    assert isinstance(typemap[djd__tbkzz], TableType
        ), 'Parquet Node Table must be a TableType'
    if pq_node.col_indices:
        kfa__hus, lksw__sfif = get_live_column_nums_block(column_live_map,
            equiv_vars, djd__tbkzz)
        kfa__hus = bodo.ir.connector.trim_extra_used_columns(kfa__hus, len(
            pq_node.col_indices))
        if not lksw__sfif and not kfa__hus:
            kfa__hus = [0]
        if not lksw__sfif and len(kfa__hus) != len(pq_node.type_usecol_offset):
            pq_node.type_usecol_offset = kfa__hus
            return True
    return False
