...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        nxn__eio = self.overloads[sig]
        xks__xqyti = nxn__eio.metadata.get('distributed_diagnostics', None)
        if xks__xqyti is None:
            hqxb__biymb = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(hqxb__biymb)
        xks__xqyti.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        kgi__dyrw = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(kgi__dyrw)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        gzr__tonan = options.pop('distributed')
        pipeline_class = (pipeline_class if gzr__tonan else bodo.compiler.
            BodoCompilerSeq)
    koi__hhq = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(koi__hhq, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(koi__hhq)
        else:
            return master_mode_wrapper(koi__hhq)
    else:
        return koi__hhq
