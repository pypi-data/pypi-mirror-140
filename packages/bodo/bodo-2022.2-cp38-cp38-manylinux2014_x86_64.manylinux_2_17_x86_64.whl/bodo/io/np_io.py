...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        idro__vnwrr = context.get_abi_sizeof(context.get_data_type(dtype.dtype)
            )
        return context.get_constant(types.intp, idro__vnwrr)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            dsfa__mpno = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(dsfa__mpno.dtype)
            file_write(unicode_to_utf8(fname), dsfa__mpno.ctypes, 
                dtype_size * dsfa__mpno.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            dsfa__mpno = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(dsfa__mpno.dtype)
            pmgb__ixoyr = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(dsfa__mpno.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), dsfa__mpno.ctypes,
                start, count, pmgb__ixoyr)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            krkne__kwtoy = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                krkne__kwtoy = min(krkne__kwtoy, count * dtype_size)
            if krkne__kwtoy < 0:
                return -1
            return krkne__kwtoy
        return impl
