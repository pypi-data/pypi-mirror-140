...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        ommlp__oymb = self.overloads[sig]
        qqs__vtr = ommlp__oymb.metadata.get('distributed_diagnostics', None)
        if qqs__vtr is None:
            guanu__achs = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(guanu__achs)
        qqs__vtr.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        pslvt__xufm = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(pslvt__xufm)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        tdxf__yfqlz = options.pop('distributed')
        pipeline_class = (pipeline_class if tdxf__yfqlz else bodo.compiler.
            BodoCompilerSeq)
    zzej__dcxez = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(zzej__dcxez, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(zzej__dcxez)
        else:
            return master_mode_wrapper(zzej__dcxez)
    else:
        return zzej__dcxez
