...
class DataFrameGetItemTemplate(AbstractTemplate):
    ...
    def typecheck_loc_getitem(self, args):
        ...
        if is_list_like_index_type(idx) and idx.dtype == types.bool_:
            ...
            fvcka__aej = df.data
            ktf__qxi = df.columns
            nei__sxwa = self.replace_range_with_numeric_idx_if_needed(df,
                sxwi__dvo)
            mxfp__imez = DataFrameType(fvcka__aej, nei__sxwa, ktf__qxi)
            return mxfp__imez(*args)
        if isinstance(idx, types.BaseTuple) and len(idx) == 2:
            xpin__hrgei = idx.types[0]
            rku__nlw = idx.types[1]
            if isinstance(xpin__hrgei, types.Integer):
                ...
                if is_overload_constant_str(rku__nlw):
                    losw__wwpqm = get_overload_const_str(rku__nlw)
                    if losw__wwpqm not in df.columns:
                        raise_bodo_error(
                            'dataframe {} does not include column {}'.
                            format(df, losw__wwpqm))
                    nsg__onms = df.columns.index(losw__wwpqm)
                    return df.data[nsg__onms].dtype(*args)
                if isinstance(rku__nlw, types.UnicodeType):
                    raise_bodo_error(
            ...
            if is_list_like_index_type(xpin__hrgei
                ) and xpin__hrgei.dtype == types.bool_ or isinstance(
                xpin__hrgei, types.SliceType):
                nei__sxwa = self.replace_range_with_numeric_idx_if_needed(df,
                    xpin__hrgei)
                if is_overload_constant_str(rku__nlw):
                    txyi__jtj = get_overload_const_str(rku__nlw)
                    if txyi__jtj not in df.columns:
                        raise_bodo_error(
                            f'dataframe {df} does not include column {txyi__jtj}'
                            )
                    nsg__onms = df.columns.index(txyi__jtj)
                    wnyxk__byt = df.data[nsg__onms]
                    mjymi__zblf = wnyxk__byt.dtype
                    xgnqp__lrc = types.literal(df.columns[nsg__onms])
                    mxfp__imez = bodo.SeriesType(mjymi__zblf, wnyxk__byt,
                        nei__sxwa, xgnqp__lrc)
                    return mxfp__imez(*args)
                if isinstance(rku__nlw, types.UnicodeType):
                    raise_bodo_error(
                        f'DataFrame.loc[] getitem (location-based indexing) requires constant column names. For more information, see https://docs.bodo.ai/latest/source/programming_with_bodo/require_constants.html.'
                        )
                elif is_overload_constant_list(rku__nlw):
                    nxl__xwugy = get_overload_const_list(rku__nlw)
                    zli__mehh = types.unliteral(rku__nlw)
                    if zli__mehh.dtype == types.bool_:
                        if len(df.columns) != len(nxl__xwugy):
                            raise_bodo_error(
                                f'dataframe {df} has {len(df.columns)} columns, but boolean array used with DataFrame.loc[] {nxl__xwugy} has {len(nxl__xwugy)} values'
                                )
                        lcy__iub = []
                        cxpb__jyhmv = []
                        for wfn__ipdvu in range(len(nxl__xwugy)):
                            if nxl__xwugy[wfn__ipdvu]:
                                lcy__iub.append(df.columns[wfn__ipdvu])
                                cxpb__jyhmv.append(df.data[wfn__ipdvu])
                        ruo__qej = tuple()
                        mxfp__imez = DataFrameType(tuple(cxpb__jyhmv),
                            nei__sxwa, tuple(lcy__iub))
                        return mxfp__imez(*args)
                    elif zli__mehh.dtype == bodo.string_type:
                        ruo__qej, cxpb__jyhmv = self.get_kept_cols_and_data(df,
                            nxl__xwugy)
                        mxfp__imez = DataFrameType(cxpb__jyhmv, nei__sxwa,
                            ruo__qej)
                        return mxfp__imez(*args)
        raise_bodo_error(
    ...
    def typecheck_df_getitem(self, args):
        ...
        if is_overload_constant_str(ind) or is_overload_constant_int(ind):
            ...
            if isinstance(df.columns[0], tuple):
                lcy__iub = []
                cxpb__jyhmv = []
                for wfn__ipdvu, hkjik__sca in enumerate(df.columns):
                    if hkjik__sca[0] != ind_val:
                        continue
                    lcy__iub.append(hkjik__sca[1] if len(hkjik__sca) == 2 else
                        hkjik__sca[1:])
                    cxpb__jyhmv.append(df.data[wfn__ipdvu])
                wnyxk__byt = tuple(cxpb__jyhmv)
                piie__xqmf = df.index
                ubyhv__mjd = tuple(lcy__iub)
                mxfp__imez = DataFrameType(wnyxk__byt, piie__xqmf, ubyhv__mjd)
                return mxfp__imez(*args)
            else:
                if ind_val not in df.columns:
                    raise_bodo_error('dataframe {} does not include column {}'
                        .format(df, ind_val))
                nsg__onms = df.columns.index(ind_val)
                wnyxk__byt = df.data[nsg__onms]
                mjymi__zblf = wnyxk__byt.dtype
                piie__xqmf = df.index
                xgnqp__lrc = types.literal(df.columns[nsg__onms])
                mxfp__imez = bodo.SeriesType(mjymi__zblf, wnyxk__byt,
                    piie__xqmf, xgnqp__lrc)
                return mxfp__imez(*args)
        ...
        if is_list_like_index_type(ind
            ...
            wnyxk__byt = df.data
            piie__xqmf = self.replace_range_with_numeric_idx_if_needed(df, ind)
            ubyhv__mjd = df.columns
            mxfp__imez = DataFrameType(wnyxk__byt, piie__xqmf, ubyhv__mjd,
                is_table_format=df.is_table_format)
            return mxfp__imez(*args)
        elif is_overload_constant_list(ind):
            tvdto__yaen = get_overload_const_list(ind)
            ubyhv__mjd, wnyxk__byt = self.get_kept_cols_and_data(df,
                tvdto__yaen)
            piie__xqmf = df.index
            mxfp__imez = DataFrameType(wnyxk__byt, piie__xqmf, ubyhv__mjd)
            return mxfp__imez(*args)
    ...
    def get_kept_cols_and_data(self, df, cols_to_keep_list):
        for pzsex__zyy in cols_to_keep_list:
            if pzsex__zyy not in df.columns:
                raise_bodo_error('Column {} not found in dataframe columns {}'
                    .format(pzsex__zyy, df.columns))
        ubyhv__mjd = tuple(cols_to_keep_list)
        wnyxk__byt = tuple(df.data[df.columns.index(xxv__khoxk)] for
            xxv__khoxk in ubyhv__mjd)
        return ubyhv__mjd, wnyxk__byt

    def replace_range_with_numeric_idx_if_needed(self, df, ind):
        nei__sxwa = bodo.hiframes.pd_index_ext.NumericIndexType(types.int64,
...
def df_getitem_overload(df, ind):
        ...
        if isinstance(df.columns[0], tuple):
            ...
            for wfn__ipdvu, hkjik__sca in enumerate(df.columns):
                ...
                cxpb__jyhmv.append(
                    ...
                    .format(wfn__ipdvu))
            llc__qshk = 'def impl(df, ind):\n'
            jqe__vul = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)'
            return bodo.hiframes.dataframe_impl._gen_init_df(llc__qshk,
                lcy__iub, ', '.join(cxpb__jyhmv), jqe__vul)
        if ind_val not in df.columns:
            raise_bodo_error('dataframe {} does not include column {}'.
                format(df, ind_val))
    ...
    if is_overload_constant_list(ind):
        tvdto__yaen = get_overload_const_list(ind)
        for pzsex__zyy in tvdto__yaen:
            if pzsex__zyy not in df.columns:
                raise_bodo_error('Column {} not found in dataframe columns {}'
                    .format(pzsex__zyy, df.columns))
        cxpb__jyhmv = ', '.join(
            'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}).copy()'
            .format(df.columns.index(pzsex__zyy)) for pzsex__zyy in tvdto__yaen
            )
        llc__qshk = 'def impl(df, ind):\n'
        jqe__vul = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)'
        return bodo.hiframes.dataframe_impl._gen_init_df(llc__qshk,
            tvdto__yaen, cxpb__jyhmv, jqe__vul)
    if is_list_like_index_type(ind) and ind.dtype == types.bool_ or isinstance(
        ...
        if df.is_table_format:
            ...
            cxpb__jyhmv = ', '.join(
                f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.columns.index(pzsex__zyy)})[ind]'
                 for pzsex__zyy in df.columns)
        return bodo.hiframes.dataframe_impl._gen_init_df(llc__qshk, df.
            columns, cxpb__jyhmv, jqe__vul, out_df_type=df)
...
class DataFrameILocType(types.Type):
    ...
    def __init__(self, df_type):
        ...
        xxv__khoxk = 'DataFrameILocType({})'.format(df_type)
        super(DataFrameILocType, self).__init__(xxv__khoxk)
...
class DataFrameILocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        wuh__rchec = [('obj', fe_type.df_type)]
        super(DataFrameILocModel, self).__init__(dmm, fe_type, wuh__rchec)
...
def init_dataframe_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fatsf__ctu, = args
        uys__jcde = signature.return_type
        dkj__rfdk = cgutils.create_struct_proxy(uys__jcde)(context, builder)
        dkj__rfdk.obj = fatsf__ctu
        context.nrt.incref(builder, signature.args[0], fatsf__ctu)
        return dkj__rfdk._getvalue()
...
def _gen_iloc_getitem_bool_slice_impl(df, col_names, idx_typ, idx,
    is_out_series):
    llc__qshk = 'def impl(I, idx):\n'
    llc__qshk += '  df = I._obj\n'
    if isinstance(idx_typ, types.SliceType):
        llc__qshk += f'  idx_t = {idx}\n'
    else:
        llc__qshk += (
            f'  idx_t = bodo.utils.conversion.coerce_to_ndarray({idx})\n')
    jqe__vul = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)[idx_t]'
    cxpb__jyhmv = ', '.join(
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.columns.index(pzsex__zyy)})[idx_t]'
         for pzsex__zyy in col_names)
    if is_out_series:
        jdcj__wlkep = f"'{col_names[0]}'" if isinstance(col_names[0], str
            ) else f'{col_names[0]}'
        llc__qshk += f"""  return bodo.hiframes.pd_series_ext.init_series({cxpb__jyhmv}, {jqe__vul}, {jdcj__wlkep})
"""
        avlp__lltk = {}
        exec(llc__qshk, {'bodo': bodo}, avlp__lltk)
        return avlp__lltk['impl']
    return bodo.hiframes.dataframe_impl._gen_init_df(llc__qshk, col_names,
        cxpb__jyhmv, jqe__vul)
...
def _gen_iloc_getitem_row_impl(df, col_names, idx):
    ...
    llc__qshk += '  df = I._obj\n'
    kblkt__zxjq = ', '.join(
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.columns.index(pzsex__zyy)})[{idx}]'
         for pzsex__zyy in col_names)
    llc__qshk += f"""  row_idx = bodo.hiframes.pd_index_ext.init_heter_index({gen_const_tup(col_names)}, None)
"""
    llc__qshk += f"""  return bodo.hiframes.pd_series_ext.init_series(({kblkt__zxjq},), row_idx, None)
"""
    avlp__lltk = {}
    exec(llc__qshk, {'bodo': bodo}, avlp__lltk)
    impl = avlp__lltk['impl']
    return impl
...
class DataFrameLocType(types.Type):
    ...
    def __init__(self, df_type):
        self.df_type = df_type
        xxv__khoxk = 'DataFrameLocType({})'.format(df_type)
        super(DataFrameLocType, self).__init__(xxv__khoxk)
...
class DataFrameLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        wuh__rchec = [('obj', fe_type.df_type)]
        super(DataFrameLocModel, self).__init__(dmm, fe_type, wuh__rchec)
...
def init_dataframe_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fatsf__ctu, = args
        kkyfl__bfrt = signature.return_type
        lhv__luxh = cgutils.create_struct_proxy(kkyfl__bfrt)(context, builder)
        lhv__luxh.obj = fatsf__ctu
        context.nrt.incref(builder, signature.args[0], fatsf__ctu)
        return lhv__luxh._getvalue()
...
def overload_loc_getitem(I, idx):
    ...
    if is_list_like_index_type(idx) and idx.dtype == types.bool_:
        llc__qshk = 'def impl(I, idx):\n'
        llc__qshk += '  df = I._obj\n'
        llc__qshk += '  idx_t = bodo.utils.conversion.coerce_to_ndarray(idx)\n'
        jqe__vul = (
            'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)[idx_t]')
        cxpb__jyhmv = ', '.join(
            'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {})[idx_t]'
            .format(df.columns.index(pzsex__zyy)) for pzsex__zyy in df.columns)
        return bodo.hiframes.dataframe_impl._gen_init_df(llc__qshk, df.
            columns, cxpb__jyhmv, jqe__vul)
    if isinstance(idx, types.BaseTuple) and len(idx) == 2:
        ouxc__ykn = idx.types[1]
        if is_overload_constant_str(ouxc__ykn):
            nwnsh__kuw = get_overload_const_str(ouxc__ykn)
            rwsi__dit = df.columns.index(nwnsh__kuw)

            def impl_col_name(I, idx):
                df = I._obj
                jqe__vul = bodo.hiframes.pd_dataframe_ext.get_dataframe_index(
                    df)
                qgn__ivz = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df
                    , rwsi__dit)
                return bodo.hiframes.pd_series_ext.init_series(qgn__ivz,
                    jqe__vul, nwnsh__kuw).loc[idx[0]]
            return impl_col_name
        if is_overload_constant_list(ouxc__ykn):
            col_idx_list = get_overload_const_list(ouxc__ykn)
            if len(col_idx_list) > 0 and not isinstance(col_idx_list[0], (
                bool, np.bool_)) and not all(pzsex__zyy in df.columns for
                pzsex__zyy in col_idx_list):
                raise_bodo_error(
...
def gen_df_loc_col_select_impl(df, col_idx_list):
    ...
    cxpb__jyhmv = ', '.join(
        ...
        .format(df.columns.index(pzsex__zyy)) for pzsex__zyy in col_idx_list)
    jqe__vul = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)[idx[0]]'
    llc__qshk = 'def impl(I, idx):\n'
    llc__qshk += '  df = I._obj\n'
    return bodo.hiframes.dataframe_impl._gen_init_df(llc__qshk,
        col_idx_list, cxpb__jyhmv, jqe__vul)
...
class DataFrameIatType(types.Type):
    ...
    def __init__(self, df_type):
        self.df_type = df_type
        xxv__khoxk = 'DataFrameIatType({})'.format(df_type)
        super(DataFrameIatType, self).__init__(xxv__khoxk)
...
class DataFrameIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        wuh__rchec = [('obj', fe_type.df_type)]
        super(DataFrameIatModel, self).__init__(dmm, fe_type, wuh__rchec)
...
def init_dataframe_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fatsf__ctu, = args
        iqe__hquhu = signature.return_type
        azna__can = cgutils.create_struct_proxy(iqe__hquhu)(context, builder)
        azna__can.obj = fatsf__ctu
        context.nrt.incref(builder, signature.args[0], fatsf__ctu)
        return azna__can._getvalue()
...
def overload_iat_setitem(I, idx, val):
    ...
    if isinstance(idx, types.BaseTuple) and len(idx) == 2:
        ...
        rwsi__dit = get_overload_const_int(idx.types[1])
        if is_immutable_array(I.df_type.data[rwsi__dit]):
            raise BodoError(
        ...
        def impl_col_ind(I, idx, val):
            ...
            qgn__ivz = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df,
                rwsi__dit)
            qgn__ivz[idx[0]] = bodo.utils.conversion.unbox_if_timestamp(val)
        return impl_col_ind
...
def cast_series_iat(context, builder, fromty, toty, val):
    azna__can = cgutils.create_struct_proxy(fromty)(context, builder, val)
    kxb__kes = context.cast(builder, azna__can.obj, fromty.df_type, toty.
        df_type)
    htp__uvsmv = cgutils.create_struct_proxy(toty)(context, builder)
    htp__uvsmv.obj = kxb__kes
    return htp__uvsmv._getvalue()
