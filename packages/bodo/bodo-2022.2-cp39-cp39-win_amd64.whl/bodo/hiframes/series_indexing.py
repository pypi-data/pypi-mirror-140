...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        kru__ijeo = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(kru__ijeo)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        bqpey__hwyxd = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, bqpey__hwyxd)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        kfl__wxpid, = args
        bjfev__zcx = signature.return_type
        ttidk__ohn = cgutils.create_struct_proxy(bjfev__zcx)(context, builder)
        ttidk__ohn.obj = kfl__wxpid
        context.nrt.incref(builder, signature.args[0], kfl__wxpid)
        return ttidk__ohn._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                jucd__gbqnp = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        kru__ijeo = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(kru__ijeo)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        bqpey__hwyxd = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, bqpey__hwyxd)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        kfl__wxpid, = args
        oltfp__udan = signature.return_type
        qyj__xqgz = cgutils.create_struct_proxy(oltfp__udan)(context, builder)
        qyj__xqgz.obj = kfl__wxpid
        context.nrt.incref(builder, signature.args[0], kfl__wxpid)
        return qyj__xqgz._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                ubyqe__qbkmc = bodo.utils.conversion.coerce_to_ndarray(idx)
                hqj__epown = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    ubyqe__qbkmc]
                onlg__xpw = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    ubyqe__qbkmc]
                kru__ijeo = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(hqj__epown,
                    onlg__xpw, kru__ijeo)
            return impl
        if isinstance(idx, types.SliceType):
            ...
            def impl_slice(I, idx):
                S = I._obj
                hqj__epown = bodo.hiframes.pd_series_ext.get_series_data(S)[idx
                    ]
                onlg__xpw = bodo.hiframes.pd_series_ext.get_series_index(S)[idx
                    ]
                kru__ijeo = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(hqj__epown,
                    onlg__xpw, kru__ijeo)
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        ubyqe__qbkmc = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        jucd__gbqnp = (bodo.hiframes.datetime_timedelta_ext
                            ._to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(jucd__gbqnp))
                        ubyqe__qbkmc = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            ubyqe__qbkmc] = s
            ...
            def impl_arr(I, idx, val):
                ubyqe__qbkmc = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                    ubyqe__qbkmc] = bodo.utils.conversion.coerce_to_array(val,
                    False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        kru__ijeo = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(kru__ijeo)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        bqpey__hwyxd = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, bqpey__hwyxd)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        kfl__wxpid, = args
        zxtc__uvsw = signature.return_type
        nnkgt__uhrr = cgutils.create_struct_proxy(zxtc__uvsw)(context, builder)
        nnkgt__uhrr.obj = kfl__wxpid
        context.nrt.incref(builder, signature.args[0], kfl__wxpid)
        return nnkgt__uhrr._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            hqj__epown = bodo.hiframes.pd_series_ext.get_series_data(S)
            onlg__xpw = bodo.hiframes.pd_series_ext.get_series_index(S)
            rznwb__iamww = (idx - onlg__xpw._start) // onlg__xpw._step
            return hqj__epown[rznwb__iamww]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    ubyqe__qbkmc = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(hqj__epown[
                        ubyqe__qbkmc])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    bhxh__tee = bodo.hiframes.pd_series_ext.get_series_index(S)
                    chgss__ptvy = bhxh__tee.get_loc(idx)
                    if chgss__ptvy is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(chgss__ptvy)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                ubyqe__qbkmc = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[ubyqe__qbkmc
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        fii__umgby = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(abt__wrf,
            int) for abt__wrf in fii__umgby):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            npo__rek = get_literal_value(idx)
            if npo__rek not in fii__umgby:
                raise_bodo_error(
                    f"Series label-based getitem: '{npo__rek}' not in {fii__umgby}"
                    )
            arr_ind = fii__umgby.index(npo__rek)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    ttidk__ohn = cgutils.create_struct_proxy(fromty)(context, builder, val)
    nfeb__yemo = context.cast(builder, ttidk__ohn.obj, fromty.stype, toty.stype
        )
    rio__uzssq = cgutils.create_struct_proxy(toty)(context, builder)
    rio__uzssq.obj = nfeb__yemo
    return rio__uzssq._getvalue()
