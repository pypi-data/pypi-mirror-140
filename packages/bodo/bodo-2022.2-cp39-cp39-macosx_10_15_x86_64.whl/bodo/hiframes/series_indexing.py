...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        nrz__mzau = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(nrz__mzau)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ibjy__dpypc = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, ibjy__dpypc)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        rcm__bomlv, = args
        urs__fks = signature.return_type
        goac__xcyn = cgutils.create_struct_proxy(urs__fks)(context, builder)
        goac__xcyn.obj = rcm__bomlv
        context.nrt.incref(builder, signature.args[0], rcm__bomlv)
        return goac__xcyn._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                yfjop__almd = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        nrz__mzau = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(nrz__mzau)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ibjy__dpypc = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, ibjy__dpypc)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        rcm__bomlv, = args
        map__ajjyl = signature.return_type
        rsyq__ejsfp = cgutils.create_struct_proxy(map__ajjyl)(context, builder)
        rsyq__ejsfp.obj = rcm__bomlv
        context.nrt.incref(builder, signature.args[0], rcm__bomlv)
        return rsyq__ejsfp._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                rvto__xnrvj = bodo.utils.conversion.coerce_to_ndarray(idx)
                gkkg__hdcwj = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    rvto__xnrvj]
                qwypv__btsh = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    rvto__xnrvj]
                nrz__mzau = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(gkkg__hdcwj,
                    qwypv__btsh, nrz__mzau)
            return impl
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        rvto__xnrvj = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        yfjop__almd = (bodo.hiframes.datetime_timedelta_ext
                            ._to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(yfjop__almd))
                        rvto__xnrvj = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            rvto__xnrvj] = s
            ...
            def impl_arr(I, idx, val):
                rvto__xnrvj = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[rvto__xnrvj
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        nrz__mzau = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(nrz__mzau)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ibjy__dpypc = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, ibjy__dpypc)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        rcm__bomlv, = args
        xntg__ppbca = signature.return_type
        ias__dxfpv = cgutils.create_struct_proxy(xntg__ppbca)(context, builder)
        ias__dxfpv.obj = rcm__bomlv
        context.nrt.incref(builder, signature.args[0], rcm__bomlv)
        return ias__dxfpv._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            gkkg__hdcwj = bodo.hiframes.pd_series_ext.get_series_data(S)
            qwypv__btsh = bodo.hiframes.pd_series_ext.get_series_index(S)
            nli__vzygo = (idx - qwypv__btsh._start) // qwypv__btsh._step
            return gkkg__hdcwj[nli__vzygo]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    rvto__xnrvj = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(gkkg__hdcwj[
                        rvto__xnrvj])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    gxy__amiva = bodo.hiframes.pd_series_ext.get_series_index(S
                        )
                    ziwoa__nfkq = gxy__amiva.get_loc(idx)
                    if ziwoa__nfkq is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(ziwoa__nfkq)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                rvto__xnrvj = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[rvto__xnrvj
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        rjf__nxcpn = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(
            kjuft__lucf, int) for kjuft__lucf in rjf__nxcpn):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            kkksg__zeb = get_literal_value(idx)
            if kkksg__zeb not in rjf__nxcpn:
                raise_bodo_error(
                    f"Series label-based getitem: '{kkksg__zeb}' not in {rjf__nxcpn}"
                    )
            arr_ind = rjf__nxcpn.index(kkksg__zeb)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    goac__xcyn = cgutils.create_struct_proxy(fromty)(context, builder, val)
    dhve__rkor = context.cast(builder, goac__xcyn.obj, fromty.stype, toty.stype
        )
    pshac__wdrxx = cgutils.create_struct_proxy(toty)(context, builder)
    pshac__wdrxx.obj = dhve__rkor
    return pshac__wdrxx._getvalue()
