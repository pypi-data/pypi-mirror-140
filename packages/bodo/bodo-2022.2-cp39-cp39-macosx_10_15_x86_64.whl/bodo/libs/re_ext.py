...
def box_regex_flag(typ, val, c):
    qamwx__bcivr = c.box(types.int64, val)
    acm__tmslh = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(acm__tmslh, (qamwx__bcivr,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    qamwx__bcivr = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, qamwx__bcivr)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    txe__ltgms = context.get_python_api(builder)
    ogjxd__gazed = builder.icmp_signed('==', val, txe__ltgms.borrow_none())
    with builder.if_then(ogjxd__gazed):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    czdfk__olflv = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(czdfk__olflv)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        vadf__gjmkf = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if vadf__gjmkf > 1:
            typ = types.List(types.Tuple([string_type] * vadf__gjmkf))
...
def overload_match_group(m, *args):
    ...
    dawd__jgin = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            efy__gudd = args[0]
            with numba.objmode(out=dawd__jgin):
                out = m.group(efy__gudd)
            return out
        return _match_group_impl_one
    zavs__qolha = 'tuple_str_{}'.format(len(args))
    setattr(types, zavs__qolha, types.Tuple([dawd__jgin] * len(args)))
    ivr__odytr = ', '.join('group{}'.format(evh__ztjh + 1) for evh__ztjh in
        range(len(args)))
    evrri__rjc = 'def _match_group_impl(m, *args):\n'
    evrri__rjc += '  ({}) = args\n'.format(ivr__odytr)
    evrri__rjc += "  with numba.objmode(out='{}'):\n".format(zavs__qolha)
    evrri__rjc += '    out = m.group({})\n'.format(ivr__odytr)
    evrri__rjc += '  return out\n'
    pitt__aanh = {}
    exec(evrri__rjc, globals(), pitt__aanh)
    szz__ttn = pitt__aanh['_match_group_impl']
    return szz__ttn
...
def overload_match_groups(m, default=None):
    geq__plp = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=geq__plp):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(mif__jdug is None for mif__jdug in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=dawd__jgin):
