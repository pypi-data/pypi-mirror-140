...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        rvyd__hcd = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, rvyd__hcd)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            pemcb__fcuky = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(pemcb__fcuky.dtype)
            file_write(unicode_to_utf8(fname), pemcb__fcuky.ctypes, 
                dtype_size * pemcb__fcuky.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            pemcb__fcuky = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(pemcb__fcuky.dtype)
            guwqc__ydykd = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(pemcb__fcuky.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), pemcb__fcuky.
                ctypes, start, count, guwqc__ydykd)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            qtj__hyrfo = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                qtj__hyrfo = min(qtj__hyrfo, count * dtype_size)
            if qtj__hyrfo < 0:
                return -1
            return qtj__hyrfo
        return impl
