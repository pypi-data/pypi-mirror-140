...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    szp__uafth = pq_node.out_vars[0].name
    eadqa__cfl = pq_node.out_vars[1].name
    if szp__uafth not in lives and eadqa__cfl not in lives:
        return None
    elif szp__uafth not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif eadqa__cfl not in lives:
...
def pq_remove_dead_column(pq_node, column_live_map, equiv_vars, typemap):
    ...
    nmdpf__cipf = pq_node.out_vars[0].name
    assert isinstance(typemap[nmdpf__cipf], TableType
        ), 'Parquet Node Table must be a TableType'
    if pq_node.col_indices:
        uuy__ddvdi, adev__gjx = get_live_column_nums_block(column_live_map,
            equiv_vars, nmdpf__cipf)
        uuy__ddvdi = bodo.ir.connector.trim_extra_used_columns(uuy__ddvdi,
            len(pq_node.col_indices))
        if not adev__gjx and not uuy__ddvdi:
            uuy__ddvdi = [0]
        if not adev__gjx and len(uuy__ddvdi) != len(pq_node.type_usecol_offset
            ):
            pq_node.type_usecol_offset = uuy__ddvdi
            return True
    return False
