...
def box_regex_flag(typ, val, c):
    fxgu__hrmu = c.box(types.int64, val)
    xkm__idtqw = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(xkm__idtqw, (fxgu__hrmu,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    fxgu__hrmu = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, fxgu__hrmu)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    udsq__muw = context.get_python_api(builder)
    nov__pocw = builder.icmp_signed('==', val, udsq__muw.borrow_none())
    with builder.if_then(nov__pocw):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    ayba__hax = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(ayba__hax)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        qzo__jbrw = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if qzo__jbrw > 1:
            typ = types.List(types.Tuple([string_type] * qzo__jbrw))
...
def overload_match_group(m, *args):
    ...
    qsjpn__bdbm = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            fxi__skumh = args[0]
            with numba.objmode(out=qsjpn__bdbm):
                out = m.group(fxi__skumh)
            return out
        return _match_group_impl_one
    qbfq__qfsd = 'tuple_str_{}'.format(len(args))
    setattr(types, qbfq__qfsd, types.Tuple([qsjpn__bdbm] * len(args)))
    lje__wti = ', '.join('group{}'.format(iny__hxzf + 1) for iny__hxzf in
        range(len(args)))
    qmfwd__blnl = 'def _match_group_impl(m, *args):\n'
    qmfwd__blnl += '  ({}) = args\n'.format(lje__wti)
    qmfwd__blnl += "  with numba.objmode(out='{}'):\n".format(qbfq__qfsd)
    qmfwd__blnl += '    out = m.group({})\n'.format(lje__wti)
    qmfwd__blnl += '  return out\n'
    lilxn__voay = {}
    exec(qmfwd__blnl, globals(), lilxn__voay)
    nnex__mqf = lilxn__voay['_match_group_impl']
    return nnex__mqf
...
def overload_match_groups(m, default=None):
    oije__cfyf = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=oije__cfyf):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(tgktn__myf is None for tgktn__myf in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=qsjpn__bdbm):
