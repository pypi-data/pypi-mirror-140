...
except ImportError as nea__pgx:
    pass


def check_sklearn_version():
    if not _is_sklearn_supported_version:
        auhv__uxjp = f""" Bodo supports scikit-learn version >= {_min_sklearn_ver_str} and < {_max_sklearn_ver_str}.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(auhv__uxjp)


def random_forest_model_fit(m, X, y):
    qgrx__suklf = m.n_estimators
    zxma__ras = MPI.Get_processor_name()
    loi__ydi = get_host_ranks()
    extb__kcqz = len(loi__ydi)
    pbneu__jjxms = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(qgrx__suklf,
        extb__kcqz, pbneu__jjxms)
    if pbneu__jjxms == loi__ydi[zxma__ras][0]:
        m.n_jobs = len(loi__ydi[zxma__ras])
    ...
    bkw__ipmf = create_subcomm_mpi4py(first_rank_node)
    if bkw__ipmf != MPI.COMM_NULL:
        ydthw__hyz = 10
        zgjlq__armud = bodo.libs.distributed_api.get_node_portion(qgrx__suklf,
            extb__kcqz, 0)
        cikls__ciq = zgjlq__armud // ydthw__hyz
        if zgjlq__armud % ydthw__hyz != 0:
            cikls__ciq += 1
        eont__iskcy = []
        for nii__kcolc in range(cikls__ciq):
            ufn__linbl = bkw__ipmf.gather(m.estimators_[nii__kcolc *
                ydthw__hyz:nii__kcolc * ydthw__hyz + ydthw__hyz])
            if pbneu__jjxms == 0:
                eont__iskcy += list(itertools.chain.from_iterable(ufn__linbl))
        if pbneu__jjxms == 0:
            m.estimators_ = eont__iskcy
    rzjuy__jbutw = MPI.COMM_WORLD
    if pbneu__jjxms == 0:
        for nii__kcolc in range(0, qgrx__suklf, 10):
            rzjuy__jbutw.bcast(m.estimators_[nii__kcolc:nii__kcolc + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            rzjuy__jbutw.bcast(m.n_classes_)
            rzjuy__jbutw.bcast(m.classes_)
        rzjuy__jbutw.bcast(m.n_outputs_)
    else:
        xhhw__ykudn = []
        for nii__kcolc in range(0, qgrx__suklf, 10):
            xhhw__ykudn += rzjuy__jbutw.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = rzjuy__jbutw.bcast(None)
            m.classes_ = rzjuy__jbutw.bcast(None)
        m.n_outputs_ = rzjuy__jbutw.bcast(None)
        m.estimators_ = xhhw__ykudn
    assert len(m.estimators_) == qgrx__suklf
    m.n_estimators = qgrx__suklf
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    who__mho = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
        ...
        sklearn.metrics._classification.multilabel_confusion_matrix = who__mho
    return result
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    clh__ece = len(labels)
    nhyzd__xid = np.zeros(clh__ece, np.int64)
    mno__ngs = np.zeros(clh__ece, np.int64)
    sagyu__jrf = np.zeros(clh__ece, np.int64)
    aikj__bshwk = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for nii__kcolc in range(len(y_true)):
        mno__ngs[aikj__bshwk[y_true[nii__kcolc]]] += 1
        if y_pred[nii__kcolc] not in aikj__bshwk:
            continue
        ldqsl__npzgt = aikj__bshwk[y_pred[nii__kcolc]]
        sagyu__jrf[ldqsl__npzgt] += 1
        if y_true[nii__kcolc] == y_pred[nii__kcolc]:
            nhyzd__xid[ldqsl__npzgt] += 1
    nhyzd__xid = bodo.libs.distributed_api.dist_reduce(nhyzd__xid, np.int32
        (Reduce_Type.Sum.value))
    mno__ngs = bodo.libs.distributed_api.dist_reduce(mno__ngs, np.int32(
        Reduce_Type.Sum.value))
    sagyu__jrf = bodo.libs.distributed_api.dist_reduce(sagyu__jrf, np.int32
        (Reduce_Type.Sum.value))
    alwav__ihmye = sagyu__jrf - nhyzd__xid
    aur__cfq = mno__ngs - nhyzd__xid
    ayh__kywlb = nhyzd__xid
    gny__rbc = y_true.shape[0] - ayh__kywlb - alwav__ihmye - aur__cfq
    with numba.objmode(result='float64[:]'):
        MCM = np.array([gny__rbc, alwav__ihmye, aur__cfq, ayh__kywlb]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        fimf__kpjok = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        fimf__kpjok = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    rzjuy__jbutw = MPI.COMM_WORLD
    inrxd__hwcg = rzjuy__jbutw.Get_size()
    if sample_weight is not None:
        mdzm__xttkk = np.sum(sample_weight)
    else:
        mdzm__xttkk = np.float64(y_true.shape[0])
    zcnq__yyuyk = np.zeros(inrxd__hwcg, dtype=type(mdzm__xttkk))
    rzjuy__jbutw.Allgather(mdzm__xttkk, zcnq__yyuyk)
    ofz__kwce = np.zeros((inrxd__hwcg, *fimf__kpjok.shape), dtype=
        fimf__kpjok.dtype)
    rzjuy__jbutw.Allgather(fimf__kpjok, ofz__kwce)
    mcua__czq = np.average(ofz__kwce, weights=zcnq__yyuyk, axis=0)
    if metric == 'mse' and not squared:
        mcua__czq = np.sqrt(mcua__czq)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return mcua__czq
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(mcua__czq)
    else:
        return np.average(mcua__czq, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    rzjuy__jbutw = MPI.COMM_WORLD
    score = rzjuy__jbutw.allreduce(score, op=MPI.SUM)
    if normalize:
        qwfr__vutim = np.sum(sample_weight
            ) if sample_weight is not None else len(y_true)
        qwfr__vutim = rzjuy__jbutw.allreduce(qwfr__vutim, op=MPI.SUM)
        score = score / qwfr__vutim
    return score
...
def check_consistent_length_parallel(*arrays):
    rzjuy__jbutw = MPI.COMM_WORLD
    nmuaf__jyli = True
    jily__pgdo = [len(xvcl__dcpqb) for xvcl__dcpqb in arrays if xvcl__dcpqb
         is not None]
    if len(np.unique(jily__pgdo)) > 1:
        nmuaf__jyli = False
    nmuaf__jyli = rzjuy__jbutw.allreduce(nmuaf__jyli, op=MPI.LAND)
    return nmuaf__jyli
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    ygm__oap = y_true.shape[0]
    qdju__zgai = rzjuy__jbutw.allreduce(ygm__oap, op=MPI.SUM)
    if qdju__zgai < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        pmizm__dtt = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        pmizm__dtt = 1.0
    awzr__swis = (pmizm__dtt * (y_true - y_pred) ** 2).sum(axis=0, dtype=np
        .float64)
    badt__cnml = np.zeros(awzr__swis.shape, dtype=awzr__swis.dtype)
    rzjuy__jbutw.Allreduce(awzr__swis, badt__cnml, op=MPI.SUM)
    qnjbb__cdjp = np.nansum(y_true * pmizm__dtt, axis=0, dtype=np.float64)
    svaei__wmvv = np.zeros_like(qnjbb__cdjp)
    rzjuy__jbutw.Allreduce(qnjbb__cdjp, svaei__wmvv, op=MPI.SUM)
    kdsaa__kns = np.nansum(sample_weight, dtype=np.float64)
    oqcg__kwh = rzjuy__jbutw.allreduce(kdsaa__kns, op=MPI.SUM)
    edplj__aemct = svaei__wmvv / oqcg__kwh
    lpt__nsliv = (pmizm__dtt * (y_true - edplj__aemct) ** 2).sum(axis=0,
        dtype=np.float64)
    uxn__wlyft = np.zeros(lpt__nsliv.shape, dtype=lpt__nsliv.dtype)
    rzjuy__jbutw.Allreduce(lpt__nsliv, uxn__wlyft, op=MPI.SUM)
    khb__qbjb = uxn__wlyft != 0
    hfzz__pyhx = badt__cnml != 0
    gpiva__mvtwp = khb__qbjb & hfzz__pyhx
    cijzb__xxgx = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    cijzb__xxgx[gpiva__mvtwp] = 1 - badt__cnml[gpiva__mvtwp] / uxn__wlyft[
        gpiva__mvtwp]
    cijzb__xxgx[hfzz__pyhx & ~khb__qbjb] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            ljoe__dxbf = uxn__wlyft
            if not np.any(khb__qbjb):
                if not np.any(hfzz__pyhx):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        ljoe__dxbf = multioutput
    return np.array([np.average(cijzb__xxgx, weights=ljoe__dxbf)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    rzjuy__jbutw = MPI.COMM_WORLD
    try:
        ywmmh__vaz = sklearn.metrics.confusion_matrix(y_true, y_pred,
            labels=labels, sample_weight=sample_weight, normalize=None)
    except ValueError as azpf__zuii:
        ywmmh__vaz = azpf__zuii
    dys__xqm = isinstance(ywmmh__vaz, ValueError
        ) and 'At least one label specified must be in y_true' in ywmmh__vaz.args[
        0]
    wgqw__aiz = rzjuy__jbutw.allreduce(dys__xqm, op=MPI.LAND)
    if wgqw__aiz:
        raise ywmmh__vaz
    elif dys__xqm:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        pgsld__gvq = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        pgsld__gvq = ywmmh__vaz
    yvxjg__ltr = np.zeros_like(pgsld__gvq)
    rzjuy__jbutw.Allreduce(pgsld__gvq, yvxjg__ltr)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            yvxjg__ltr = yvxjg__ltr / yvxjg__ltr.sum(axis=1, keepdims=True)
        elif normalize == 'pred':
            yvxjg__ltr = yvxjg__ltr / yvxjg__ltr.sum(axis=0, keepdims=True)
        elif normalize == 'all':
            yvxjg__ltr = yvxjg__ltr / yvxjg__ltr.sum()
        yvxjg__ltr = np.nan_to_num(yvxjg__ltr)
    return yvxjg__ltr
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    mfbzw__lpuo = 'def _confusion_matrix_impl(\n'
    mfbzw__lpuo += '    y_true, y_pred, labels=None,\n'
    mfbzw__lpuo += '    sample_weight=None, normalize=None,\n'
    mfbzw__lpuo += '    _is_data_distributed=False,\n'
    mfbzw__lpuo += '):\n'
    mfbzw__lpuo += (
        '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n')
    mfbzw__lpuo += (
        '    y_pred = bodo.utils.conversion.coerce_to_array(y_pred)\n')
    cph__hqjln = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        cph__hqjln = 'float64[:,:]', 'np.float64'
    ...
    if not is_overload_none(labels):
        ...
        mfbzw__lpuo += '    labels = bodo.allgatherv(labels, False)\n'
        mfbzw__lpuo += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    mfbzw__lpuo += f"    with numba.objmode(cm='{cph__hqjln[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        mfbzw__lpuo += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        mfbzw__lpuo += '      cm = confusion_matrix_dist_helper(\n'
    mfbzw__lpuo += '        y_true, y_pred, labels=labels,\n'
    mfbzw__lpuo += (
        '        sample_weight=sample_weight, normalize=normalize,\n')
    mfbzw__lpuo += f'      ).astype({cph__hqjln[1]})\n'
    mfbzw__lpuo += '    return cm\n'
    bnb__xshoc = {}
    exec(mfbzw__lpuo, globals(), bnb__xshoc)
    iiy__vtv = bnb__xshoc['_confusion_matrix_impl']
    return iiy__vtv
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    rzjuy__jbutw = MPI.COMM_WORLD
    jphi__kne = rzjuy__jbutw.allreduce(len(X), op=MPI.SUM)
    lkozk__xiu = len(X) / jphi__kne
    jiioz__wgub = rzjuy__jbutw.Get_size()
    m.n_jobs = 1
    m.early_stopping = False
    hdud__vdut = np.inf
    jvyv__bwcd = 0
    if m.loss == 'hinge':
        ckyl__qvg = hinge_loss
    elif m.loss == 'log':
        ckyl__qvg = log_loss
    elif m.loss == 'squared_error':
        ckyl__qvg = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    obr__mjjt = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        obr__mjjt = True
    for dqpj__dtz in range(m.max_iter):
        if obr__mjjt:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * lkozk__xiu
        m.coef_ = rzjuy__jbutw.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * lkozk__xiu
        m.intercept_ = rzjuy__jbutw.allreduce(m.intercept_, op=MPI.SUM)
        if obr__mjjt:
            y_pred = m.predict(X)
            wdneg__ewqs = ckyl__qvg(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            wdneg__ewqs = ckyl__qvg(y, y_pred, labels=y_classes)
        srcd__yce = rzjuy__jbutw.allreduce(wdneg__ewqs, op=MPI.SUM)
        wdneg__ewqs = srcd__yce / jiioz__wgub
        if m.tol > np.NINF and wdneg__ewqs > hdud__vdut - m.tol * jphi__kne:
            jvyv__bwcd += 1
        else:
            jvyv__bwcd = 0
        if wdneg__ewqs < hdud__vdut:
            hdud__vdut = wdneg__ewqs
        if jvyv__bwcd >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    rzjuy__jbutw = MPI.COMM_WORLD
    pbneu__jjxms = rzjuy__jbutw.Get_rank()
    zxma__ras = MPI.Get_processor_name()
    loi__ydi = get_host_ranks()
    rgggr__wvlhl = m.n_jobs if hasattr(m, 'n_jobs') else None
    xvw__exxy = m._n_threads if hasattr(m, '_n_threads') else None
    m._n_threads = len(loi__ydi[zxma__ras])
    if pbneu__jjxms == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if pbneu__jjxms == 0:
        rzjuy__jbutw.bcast(m.cluster_centers_)
        rzjuy__jbutw.bcast(m.inertia_)
        rzjuy__jbutw.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = rzjuy__jbutw.bcast(None)
        m.inertia_ = rzjuy__jbutw.bcast(None)
        m.n_iter_ = rzjuy__jbutw.bcast(None)
    if _is_data_distributed:
        gocvh__kpyn = rzjuy__jbutw.allgather(len_X)
        if pbneu__jjxms == 0:
            lepp__rww = np.empty(len(gocvh__kpyn) + 1, dtype=int)
            np.cumsum(gocvh__kpyn, out=lepp__rww[1:])
            lepp__rww[0] = 0
            kczal__ewbf = [m.labels_[lepp__rww[gffkg__jtyh]:lepp__rww[
                gffkg__jtyh + 1]] for gffkg__jtyh in range(len(gocvh__kpyn))]
            lgn__lxe = rzjuy__jbutw.scatter(kczal__ewbf)
        else:
            lgn__lxe = rzjuy__jbutw.scatter(None)
        m.labels_ = lgn__lxe
    elif pbneu__jjxms == 0:
        rzjuy__jbutw.bcast(m.labels_)
    else:
        m.labels_ = rzjuy__jbutw.bcast(None)
    m._n_threads = xvw__exxy
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                rzjuy__jbutw = MPI.COMM_WORLD
                result = rzjuy__jbutw.allreduce(result, op=MPI.SUM)
            m._n_threads = xvw__exxy
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        mfbzw__lpuo = 'def _model_multinomial_nb_fit_impl(\n'
        mfbzw__lpuo += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        mfbzw__lpuo += '):  # pragma: no cover\n'
        mfbzw__lpuo += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            mfbzw__lpuo += '    X = X.to_numpy()\n'
        else:
            mfbzw__lpuo += (
                '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n')
        mfbzw__lpuo += '    my_rank = bodo.get_rank()\n'
        mfbzw__lpuo += '    nranks = bodo.get_size()\n'
        mfbzw__lpuo += '    total_cols = X.shape[1]\n'
        mfbzw__lpuo += '    for i in range(nranks):\n'
        mfbzw__lpuo += """        start = bodo.libs.distributed_api.get_start(total_cols, nranks, i)
"""
        mfbzw__lpuo += (
            '        end = bodo.libs.distributed_api.get_end(total_cols, nranks, i)\n'
            )
        mfbzw__lpuo += '        if i == my_rank:\n'
        mfbzw__lpuo += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        mfbzw__lpuo += '        else:\n'
        mfbzw__lpuo += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        mfbzw__lpuo += '    y_train = bodo.allgatherv(y, False)\n'
        mfbzw__lpuo += '    with numba.objmode(m="multinomial_nb_type"):\n'
        mfbzw__lpuo += '        m = fit_multinomial_nb(\n'
        mfbzw__lpuo += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        mfbzw__lpuo += '        )\n'
        mfbzw__lpuo += '    bodo.barrier()\n'
        mfbzw__lpuo += '    return m\n'
        bnb__xshoc = {}
        exec(mfbzw__lpuo, globals(), bnb__xshoc)
        esv__nop = bnb__xshoc['_model_multinomial_nb_fit_impl']
        return esv__nop
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    dqpj__dtz, n_features = X_train.shape
    m.n_features_in_ = n_features
    dao__famrx = LabelBinarizer()
    qsl__huzwe = dao__famrx.fit_transform(y_train)
    m.classes_ = dao__famrx.classes_
    if qsl__huzwe.shape[1] == 1:
        qsl__huzwe = np.concatenate((1 - qsl__huzwe, qsl__huzwe), axis=1)
    if sample_weight is not None:
        qsl__huzwe = qsl__huzwe.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        qsl__huzwe *= sample_weight.T
    class_prior = m.class_prior
    zfy__gavd = qsl__huzwe.shape[1]
    m._init_counters(zfy__gavd, n_features)
    m._count(X_train.astype('float64'), qsl__huzwe)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    vsbwx__kzlv = m.feature_count_ + alpha
    nqi__ltz = vsbwx__kzlv.sum(axis=1)
    rzjuy__jbutw = MPI.COMM_WORLD
    jiioz__wgub = rzjuy__jbutw.Get_size()
    skfgz__uukb = np.zeros(zfy__gavd)
    rzjuy__jbutw.Allreduce(nqi__ltz, skfgz__uukb, op=MPI.SUM)
    knp__ovow = np.log(vsbwx__kzlv) - np.log(skfgz__uukb.reshape(-1, 1))
    ssja__nxuv = knp__ovow.T.reshape(n_features * zfy__gavd)
    nvhfp__azt = np.ones(jiioz__wgub) * (total_cols // jiioz__wgub)
    riztn__usrlk = total_cols % jiioz__wgub
    for wcc__vqpb in range(riztn__usrlk):
        nvhfp__azt[wcc__vqpb] += 1
    nvhfp__azt *= zfy__gavd
    fwzr__rqgvx = np.zeros(jiioz__wgub, dtype=np.int32)
    fwzr__rqgvx[1:] = np.cumsum(nvhfp__azt)[:-1]
    lpa__tyq = np.zeros((total_cols, zfy__gavd), dtype=np.float64)
    rzjuy__jbutw.Allgatherv(ssja__nxuv, [lpa__tyq, nvhfp__azt, fwzr__rqgvx,
        MPI.DOUBLE_PRECISION])
    m.feature_log_prob_ = lpa__tyq.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    rzjuy__jbutw = MPI.COMM_WORLD
    inrxd__hwcg = rzjuy__jbutw.Get_size()
    kqnb__csee = m.with_std
    bvpz__gkktj = m.with_mean
    m.with_std = False
    if kqnb__csee:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = kqnb__csee
    m.with_mean = bvpz__gkktj
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    rslt__eglrk = np.zeros((inrxd__hwcg, *m.n_samples_seen_.shape), dtype=m
        .n_samples_seen_.dtype)
    rzjuy__jbutw.Allgather(m.n_samples_seen_, rslt__eglrk)
    hyj__umy = np.sum(rslt__eglrk, axis=0)
    m.n_samples_seen_ = hyj__umy
    if m.with_mean or m.with_std:
        yjyvg__zip = np.zeros((inrxd__hwcg, *m.mean_.shape), dtype=m.mean_.
            dtype)
        rzjuy__jbutw.Allgather(m.mean_, yjyvg__zip)
        yjyvg__zip[np.isnan(yjyvg__zip)] = 0
        emj__ece = np.average(yjyvg__zip, axis=0, weights=rslt__eglrk)
        m.mean_ = emj__ece
    if m.with_std:
        uwv__wny = sklearn_safe_accumulator_op(np.nansum, (X - emj__ece) **
            2, axis=0) / hyj__umy
        ezzi__fub = np.zeros_like(uwv__wny)
        rzjuy__jbutw.Allreduce(uwv__wny, ezzi__fub, op=MPI.SUM)
        m.var_ = ezzi__fub
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    jljgz__dwd = rzjuy__jbutw.allreduce(jljgz__dwd, op=MPI.LAND)
    if jljgz__dwd:
...
def get_data_slice_parallel(data, labels, len_train):
    zhdc__tzxqh = data[:len_train]
    vjs__zvtd = data[len_train:]
    zhdc__tzxqh = bodo.rebalance(zhdc__tzxqh)
    vjs__zvtd = bodo.rebalance(vjs__zvtd)
    jqfc__tjogj = labels[:len_train]
    sngjh__lydu = labels[len_train:]
    jqfc__tjogj = bodo.rebalance(jqfc__tjogj)
    sngjh__lydu = bodo.rebalance(sngjh__lydu)
    return zhdc__tzxqh, vjs__zvtd, jqfc__tjogj, sngjh__lydu
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    cpuat__agr = {'stratify': stratify}
    bvq__vwj = {'stratify': None}
    check_unsupported_args('train_test_split', cpuat__agr, bvq__vwj, 'ml')
    if is_overload_false(_is_data_distributed):
        xloyt__ypb = f'data_split_type_{numba.core.ir_utils.next_label()}'
        kbpf__wsiai = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for lbg__fpypc, bdpv__xqp in ((data, xloyt__ypb), (labels, kbpf__wsiai)
            ):
            if isinstance(lbg__fpypc, (DataFrameType, SeriesType)):
                svkg__kczs = lbg__fpypc.copy(index=NumericIndexType(types.
                    int64))
                setattr(types, bdpv__xqp, svkg__kczs)
            else:
                setattr(types, bdpv__xqp, lbg__fpypc)
        mfbzw__lpuo = 'def _train_test_split_impl(\n'
        mfbzw__lpuo += '    data,\n'
        mfbzw__lpuo += '    labels=None,\n'
        mfbzw__lpuo += '    train_size=None,\n'
        mfbzw__lpuo += '    test_size=None,\n'
        mfbzw__lpuo += '    random_state=None,\n'
        mfbzw__lpuo += '    shuffle=True,\n'
        mfbzw__lpuo += '    stratify=None,\n'
        mfbzw__lpuo += '    _is_data_distributed=False,\n'
        mfbzw__lpuo += '):  # pragma: no cover\n'
        mfbzw__lpuo += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(xloyt__ypb, xloyt__ypb, kbpf__wsiai, kbpf__wsiai))
        mfbzw__lpuo += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        mfbzw__lpuo += '            data,\n'
        mfbzw__lpuo += '            labels,\n'
        mfbzw__lpuo += '            train_size=train_size,\n'
        mfbzw__lpuo += '            test_size=test_size,\n'
        mfbzw__lpuo += '            random_state=random_state,\n'
        mfbzw__lpuo += '            shuffle=shuffle,\n'
        mfbzw__lpuo += '            stratify=stratify,\n'
        mfbzw__lpuo += '        )\n'
        mfbzw__lpuo += (
            '    return data_train, data_test, labels_train, labels_test\n')
        bnb__xshoc = {}
        exec(mfbzw__lpuo, globals(), bnb__xshoc)
        _train_test_split_impl = bnb__xshoc['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            mbuhu__yriy = bodo.libs.distributed_api.dist_reduce(len(data),
                np.int32(Reduce_Type.Sum.value))
            len_train = int(mbuhu__yriy * train_size)
            hjihm__uokbv = mbuhu__yriy - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                pbneu__jjxms = bodo.get_rank()
                jiioz__wgub = bodo.get_size()
                yyi__lsua = np.empty(jiioz__wgub, np.int64)
                bodo.libs.distributed_api.allgather(yyi__lsua, len(data))
                nkaib__ysc = np.cumsum(yyi__lsua[0:pbneu__jjxms + 1])
                mkt__ryfp = np.full(mbuhu__yriy, True)
                mkt__ryfp[:hjihm__uokbv] = False
                np.random.seed(42)
                np.random.permutation(mkt__ryfp)
                if pbneu__jjxms:
                    xzboz__qywye = nkaib__ysc[pbneu__jjxms - 1]
                else:
                    xzboz__qywye = 0
                spk__uxn = nkaib__ysc[pbneu__jjxms]
                cyp__ghmqd = mkt__ryfp[xzboz__qywye:spk__uxn]
                zhdc__tzxqh = data[cyp__ghmqd]
                vjs__zvtd = data[~cyp__ghmqd]
                jqfc__tjogj = labels[cyp__ghmqd]
                sngjh__lydu = labels[~cyp__ghmqd]
                zhdc__tzxqh = bodo.random_shuffle(zhdc__tzxqh, seed=
                    random_state, parallel=True)
                vjs__zvtd = bodo.random_shuffle(vjs__zvtd, seed=
                    random_state, parallel=True)
                jqfc__tjogj = bodo.random_shuffle(jqfc__tjogj, seed=
                    random_state, parallel=True)
                sngjh__lydu = bodo.random_shuffle(sngjh__lydu, seed=
                    random_state, parallel=True)
                jqfc__tjogj = reset_labels_type(jqfc__tjogj, label_type)
                sngjh__lydu = reset_labels_type(sngjh__lydu, label_type)
            else:
                zhdc__tzxqh, vjs__zvtd, jqfc__tjogj, sngjh__lydu = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    inrxd__hwcg = rzjuy__jbutw.Get_size()
    m = m.fit(X)
    hyj__umy = rzjuy__jbutw.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = hyj__umy
    joi__lyn = np.zeros((inrxd__hwcg, *m.data_min_.shape), dtype=m.
        data_min_.dtype)
    rzjuy__jbutw.Allgather(m.data_min_, joi__lyn)
    yeedq__egf = np.nanmin(joi__lyn, axis=0)
    wca__moup = np.zeros((inrxd__hwcg, *m.data_max_.shape), dtype=m.
        data_max_.dtype)
    rzjuy__jbutw.Allgather(m.data_max_, wca__moup)
    goq__gah = np.nanmax(wca__moup, axis=0)
    dcu__dcun = goq__gah - yeedq__egf
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(dcu__dcun)
    m.min_ = m.feature_range[0] - yeedq__egf * m.scale_
    m.data_min_ = yeedq__egf
    m.data_max_ = goq__gah
    m.data_range_ = dcu__dcun
    return m
...
def overload_rf_classifier_model_fit(m, X, y, sample_weight=None,
    _is_data_distributed=False):
    olquf__tqq = 'RandomForestClassifier'
    if isinstance(m, BodoRandomForestRegressorType):
        olquf__tqq = 'RandomForestRegressor'
    if not is_overload_none(sample_weight):
        raise BodoError(
            f"sklearn.ensemble.{olquf__tqq}.fit() : 'sample_weight' is not supported for distributed data."
            )
    ...
    def _model_fit_impl(m, X, y, sample_weight=None, _is_data_distributed=False
        ...
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            extb__kcqz = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if extb__kcqz > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=extb__kcqz)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        htkm__dtmop = True
    return htkm__dtmop, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                xbqsw__ovucr = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                xbqsw__ovucr = bodo.allgatherv(xbqsw__ovucr, False)
                xbqsw__ovucr = bodo.libs.array_kernels.sort(xbqsw__ovucr,
                    ascending=True, inplace=True)
                fykji__sos = {}
                for nii__kcolc in range(len(xbqsw__ovucr)):
                    fykji__sos[xbqsw__ovucr[nii__kcolc]] = nii__kcolc
            else:
                fykji__sos = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = fykji__sos
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X
