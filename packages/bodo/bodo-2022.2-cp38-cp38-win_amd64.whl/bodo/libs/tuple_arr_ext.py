...
class TupleArrayType(types.ArrayCompatible):
    ...
    @property
    def dtype(self):
        return types.BaseTuple.from_types(tuple(xhha__hpg.dtype for
            xhha__hpg in self.data))
...
class TupleArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vlajq__zfbqs = [('data', StructArrayType(fe_type.data))]
        models.StructModel.__init__(self, dmm, fe_type, vlajq__zfbqs)
...
def init_tuple_arr(typingctx, data_typ=None):
    ...
    uxzl__ibq = TupleArrayType(data_typ.data)

    def codegen(context, builder, sig, args):
        bth__axf, = args
        zgll__eqadc = context.make_helper(builder, uxzl__ibq)
        zgll__eqadc.data = bth__axf
        context.nrt.incref(builder, data_typ, bth__axf)
        return zgll__eqadc._getvalue()
    return uxzl__ibq(data_typ), codegen


@unbox(TupleArrayType)
def unbox_tuple_array(typ, val, c):
    data_typ = StructArrayType(typ.data)
    usuc__ylzxd = unbox_struct_array(data_typ, val, c, is_tuple_array=True)
    bth__axf = usuc__ylzxd.value
    zgll__eqadc = c.context.make_helper(c.builder, typ)
    zgll__eqadc.data = bth__axf
    umf__nuqq = usuc__ylzxd.is_error
    return NativeValue(zgll__eqadc._getvalue(), is_error=umf__nuqq)


@box(TupleArrayType)
def box_tuple_arr(typ, val, c):
    data_typ = StructArrayType(typ.data)
    zgll__eqadc = c.context.make_helper(c.builder, typ, val)
    arr = box_struct_arr(data_typ, zgll__eqadc.data, c, is_tuple_array=True)
    return arr
...
def tuple_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        pryy__ardb = 'def impl(arr, ind):\n'
        lczf__uwrkq = ','.join(f'get_data(arr._data)[{jyxr__iews}][ind]' for
            jyxr__iews in range(len(arr.data)))
        pryy__ardb += f'  return ({lczf__uwrkq})\n'
        hkhei__nzxep = {}
        exec(pryy__ardb, {'get_data': bodo.libs.struct_arr_ext.get_data},
            hkhei__nzxep)
        bvbw__osyn = hkhei__nzxep['impl']
        return bvbw__osyn
...
def tuple_arr_setitem(arr, ind, val):
    ...
    if val == types.none or isinstance(val, types.optional):
        return
    if isinstance(ind, types.Integer):
        edw__tswv = len(arr.data)
        pryy__ardb = 'def impl(arr, ind, val):\n'
        pryy__ardb += '  data = get_data(arr._data)\n'
        pryy__ardb += '  null_bitmap = get_null_bitmap(arr._data)\n'
        pryy__ardb += '  set_bit_to_arr(null_bitmap, ind, 1)\n'
        for jyxr__iews in range(edw__tswv):
            pryy__ardb += f'  data[{jyxr__iews}][ind] = val[{jyxr__iews}]\n'
        hkhei__nzxep = {}
        exec(pryy__ardb, {'get_data': bodo.libs.struct_arr_ext.get_data,
