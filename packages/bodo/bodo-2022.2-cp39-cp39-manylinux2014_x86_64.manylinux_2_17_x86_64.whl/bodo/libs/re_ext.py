...
def box_regex_flag(typ, val, c):
    cjb__garmu = c.box(types.int64, val)
    kmbpy__hve = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(kmbpy__hve, (cjb__garmu,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    cjb__garmu = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, cjb__garmu)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    ewqij__yxm = context.get_python_api(builder)
    pby__xegp = builder.icmp_signed('==', val, ewqij__yxm.borrow_none())
    with builder.if_then(pby__xegp):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    daa__tfchp = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(daa__tfchp)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        aouf__els = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if aouf__els > 1:
            typ = types.List(types.Tuple([string_type] * aouf__els))
...
def overload_match_group(m, *args):
    ...
    qpb__vfsyf = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            fhtiy__iua = args[0]
            with numba.objmode(out=qpb__vfsyf):
                out = m.group(fhtiy__iua)
            return out
        return _match_group_impl_one
    fqgnj__hyr = 'tuple_str_{}'.format(len(args))
    setattr(types, fqgnj__hyr, types.Tuple([qpb__vfsyf] * len(args)))
    fzy__anbtf = ', '.join('group{}'.format(myi__ropf + 1) for myi__ropf in
        range(len(args)))
    nlwz__rhgq = 'def _match_group_impl(m, *args):\n'
    nlwz__rhgq += '  ({}) = args\n'.format(fzy__anbtf)
    nlwz__rhgq += "  with numba.objmode(out='{}'):\n".format(fqgnj__hyr)
    nlwz__rhgq += '    out = m.group({})\n'.format(fzy__anbtf)
    nlwz__rhgq += '  return out\n'
    icbk__dab = {}
    exec(nlwz__rhgq, globals(), icbk__dab)
    wqc__umj = icbk__dab['_match_group_impl']
    return wqc__umj
...
def overload_match_groups(m, default=None):
    spv__fru = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=spv__fru):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(oqjrq__naw is None for oqjrq__naw in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=qpb__vfsyf):
