...
def install_mpl_class(types_name, python_type):
    iucdr__idsrk = ''.join(map(str.title, types_name.split('_')))
    ufhte__zay = f'class {iucdr__idsrk}(types.Opaque):\n'
    ufhte__zay += '    def __init__(self):\n'
    ufhte__zay += (
        f"       types.Opaque.__init__(self, name='{iucdr__idsrk}')\n")
    ufhte__zay += '    def __reduce__(self):\n'
    ufhte__zay += (
        f"        return (types.Opaque, ('{iucdr__idsrk}',), self.__dict__)\n")
    zqx__wjnbq = {}
    exec(ufhte__zay, {'types': types, 'bodo': bodo}, zqx__wjnbq)
    cnglh__palo = zqx__wjnbq[iucdr__idsrk]
    khvo__npkwz = sys.modules[__name__]
    setattr(khvo__npkwz, iucdr__idsrk, cnglh__palo)
    class_instance = cnglh__palo()
    setattr(types, types_name, class_instance)
    register_model(cnglh__palo)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(cnglh__palo)(unbox_mpl_obj)
    box(cnglh__palo)(box_mpl_obj)
...
def _install_mpl_types():
    oaub__dkjc = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for vofw__vhkpk, yoeyu__rleu in oaub__dkjc:
        install_mpl_class(vofw__vhkpk, yoeyu__rleu)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    mlcqm__yoj = ', '.join(f'e{jpim__axt}' for jpim__axt in range(len(args)))
    if mlcqm__yoj:
        mlcqm__yoj += ', '
    lsplq__anc = ', '.join(f"{hnoe__afyqq} = ''" for hnoe__afyqq in kws.keys())
    erkao__yjn = 'matplotlib_obj, ' if obj_typ is not None else ''
    fph__cnhoe = f'def mpl_stub({erkao__yjn} {mlcqm__yoj} {lsplq__anc}):\n'
    fph__cnhoe += '    pass\n'
    esdr__wwko = {}
    exec(fph__cnhoe, {}, esdr__wwko)
    ifeh__ljt = esdr__wwko['mpl_stub']
    vwnaq__wouvb = numba.core.utils.pysignature(ifeh__ljt)
    slw__dgf = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(slw__dgf)).replace(pysig=
        vwnaq__wouvb)


def generate_axes_typing(mod_name, nrows, ncols):
    gnxpn__xmmom = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(gnxpn__xmmom.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(gnxpn__xmmom.format(mod_name, 'ncols'))
    wgp__tgex = get_overload_const_int(nrows)
    ucozk__hbta = get_overload_const_int(ncols)
    if wgp__tgex < 1:
        raise BodoError(gnxpn__xmmom.format(mod_name, 'nrows'))
    if ucozk__hbta < 1:
        raise BodoError(gnxpn__xmmom.format(mod_name, 'ncols'))
    if wgp__tgex == 1 and ucozk__hbta == 1:
        vedsi__wcf = types.mpl_axes_type
    else:
        if ucozk__hbta == 1:
            rrjt__ykqgr = types.mpl_axes_type
        else:
            rrjt__ykqgr = types.Tuple([types.mpl_axes_type] * ucozk__hbta)
        vedsi__wcf = types.Tuple([rrjt__ykqgr] * wgp__tgex)
    return vedsi__wcf


def generate_pie_return_type(args, kws):
    bigdq__kaepc = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if bigdq__kaepc == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    vgb__szef = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(vgb__szef):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(vgb__szef):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        askkv__gce = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, askkv__gce]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    askkv__gce = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    vofw__vhkpk = str(askkv__gce)
    if not hasattr(types, vofw__vhkpk):
        vofw__vhkpk = f'objmode_type{ir_utils.next_label()}'
        setattr(types, vofw__vhkpk, askkv__gce)
    fph__cnhoe = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{vofw__vhkpk}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    esdr__wwko = {}
    exec(fph__cnhoe, {'numba': numba, 'np': np}, esdr__wwko)
    impl = esdr__wwko['impl']
    return impl
