...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        eprbu__bnoa = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(eprbu__bnoa)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oyx__qtzj = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, oyx__qtzj)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        wmoif__lmjfn, = args
        tzhc__yvzk = signature.return_type
        rjn__gwiv = cgutils.create_struct_proxy(tzhc__yvzk)(context, builder)
        rjn__gwiv.obj = wmoif__lmjfn
        context.nrt.incref(builder, signature.args[0], wmoif__lmjfn)
        return rjn__gwiv._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                bxs__ykp = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        eprbu__bnoa = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(eprbu__bnoa)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oyx__qtzj = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, oyx__qtzj)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        wmoif__lmjfn, = args
        pze__irr = signature.return_type
        ucedy__hpdk = cgutils.create_struct_proxy(pze__irr)(context, builder)
        ucedy__hpdk.obj = wmoif__lmjfn
        context.nrt.incref(builder, signature.args[0], wmoif__lmjfn)
        return ucedy__hpdk._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                awb__lotj = bodo.utils.conversion.coerce_to_ndarray(idx)
                pyf__xikk = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    awb__lotj]
                haq__crgn = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    awb__lotj]
                eprbu__bnoa = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(pyf__xikk,
                    haq__crgn, eprbu__bnoa)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                pyf__xikk = bodo.hiframes.pd_series_ext.get_series_data(S)[idx]
                haq__crgn = bodo.hiframes.pd_series_ext.get_series_index(S)[idx
                    ]
                eprbu__bnoa = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(pyf__xikk,
                    haq__crgn, eprbu__bnoa)
    ...
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        bxs__ykp = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(bxs__ykp))
                        awb__lotj = bodo.utils.conversion.coerce_to_ndarray(idx
                            )
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            awb__lotj] = s
            ...
            def impl_arr(I, idx, val):
                awb__lotj = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[awb__lotj
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        eprbu__bnoa = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(eprbu__bnoa)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oyx__qtzj = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, oyx__qtzj)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        wmoif__lmjfn, = args
        exm__cme = signature.return_type
        mzh__ipax = cgutils.create_struct_proxy(exm__cme)(context, builder)
        mzh__ipax.obj = wmoif__lmjfn
        context.nrt.incref(builder, signature.args[0], wmoif__lmjfn)
        return mzh__ipax._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            pyf__xikk = bodo.hiframes.pd_series_ext.get_series_data(S)
            haq__crgn = bodo.hiframes.pd_series_ext.get_series_index(S)
            nnup__rrw = (idx - haq__crgn._start) // haq__crgn._step
            return pyf__xikk[nnup__rrw]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    awb__lotj = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(pyf__xikk[
                        awb__lotj])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    luo__kyzd = bodo.hiframes.pd_series_ext.get_series_index(S)
                    impg__xuq = luo__kyzd.get_loc(idx)
                    if impg__xuq is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(impg__xuq)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                awb__lotj = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[awb__lotj
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        ipke__ehdc = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(qfyp__amnw,
            int) for qfyp__amnw in ipke__ehdc):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            pgwd__piwve = get_literal_value(idx)
            if pgwd__piwve not in ipke__ehdc:
                raise_bodo_error(
                    f"Series label-based getitem: '{pgwd__piwve}' not in {ipke__ehdc}"
                    )
            arr_ind = ipke__ehdc.index(pgwd__piwve)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    rjn__gwiv = cgutils.create_struct_proxy(fromty)(context, builder, val)
    cnim__kgcdn = context.cast(builder, rjn__gwiv.obj, fromty.stype, toty.stype
        )
    jgoo__zctz = cgutils.create_struct_proxy(toty)(context, builder)
    jgoo__zctz.obj = cnim__kgcdn
    return jgoo__zctz._getvalue()
