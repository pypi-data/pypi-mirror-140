...
class BaseListView(ListModelMixin, RetrieveModelMixin, GenericAPIView):
    ...
    * API: BaseModel的ListView和RetrieveView接口
    ...
    def get_only_get_distinct_field(self):
        ...
        only_get_distinct_field = pure.convert_query_parameter_to_bool(value) if value else self.only_get_distinct_field
        return only_get_distinct_field

    def get_distinct_field_ls(self):
        query_dc = self.get_request_data()
        distinct_field_ls = get_list(query_dc, 'distinct_field_ls')
        distinct_field_ls = distinct_field_ls if distinct_field_ls else self.distinct_field_ls
        return distinct_field_ls

    def get_base_fields(self):
        query_dc = self.get_request_data()
        base_fields = get_list(query_dc, 'base_fields')
        base_fields = base_fields if base_fields else self.base_fields
        return base_fields
