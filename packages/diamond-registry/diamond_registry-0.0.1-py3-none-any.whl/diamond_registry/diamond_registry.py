...
def configure(
    ...
    try: 
        ...
        if _tableNotExist(cursor,rdb_database,'stores'):
        ...
        if _tableNotExist(cursor,rdb_database,'diamonds'):
...
#Count all tables in the given database the same as the given table name. If it is 0 (does not exist), return True.
def _tableNotExist(cursor, db, tableName) -> bool:
    ...
    try:
        ...
        print('Error while connecting to MySQL: ', e)
    return foundTable[0]==0



#Add a new diamond to the registry
def add_diamond(
    carat: float,
    cut: str,
    color: str,
    ...
    if id != None: #if the ID was given and doesn't exist in the database, add the diamond to the database
        if _isIdUnique(cursor,id):
            _addDiamondToDatabase(cursor, diamondToAdd)
            connection.commit()
            connection.close()
        else:
            raise ValueError('ID already exists in database')
    else:
        _addDiamondToDatabaseNewID(cursor, diamondToAdd)
        connection.commit()
...
def _addDiamondToDatabase(cursor, diamond):
    storeId = _getStoreID(cursor,diamond.getStore().getCity(),diamond.getStore().getCountry())
    if storeId == None:#if the store doesn't exist in the database, add it
        sqlCommand = f"""INSERT INTO stores (city,country)
        VALUES ('{_fixAppostrophe(diamond.getStore().getCity())}',
        '{_fixAppostrophe(diamond.getStore().getCountry())}');"""
        try:
            cursor.execute(sqlCommand)
            sqlCommand = """SELECT LAST_INSERT_ID();"""
            cursor.execute(sqlCommand)
...
#Add given diamond to the database and generate an ID for it
def _addDiamondToDatabaseNewID(cursor, diamond):
    storeId = _getStoreID(cursor,diamond.getStore().getCity(),diamond.getStore().getCountry())
    if storeId == None:#if the store doesn't exist in the database, add it
        sqlCommand = f"""INSERT INTO stores 
        VALUES ('{_fixAppostrophe(diamond.getStore().getCity())}',
        '{_fixAppostrophe(diamond.getStore().getCountry())}');"""
        try:
            cursor.execute(sqlCommand)
...
#Checks that the given ID number doesn't already exist in the database
def _isIdUnique(cursor,id) -> bool:
    sqlCommand = f"SELECT COUNT(*) FROM diamonds WHERE ID={id}"
    try:
        cursor.execute(sqlCommand)
...
#Checks that the given store doesn't already exist in the database (only 1 store per city so the city is the unique identifyer of the store)
def _getStoreID(cursor,city,country) -> bool:
    sqlCommand = f"SELECT ID FROM stores WHERE city='{_fixAppostrophe(city)}' AND country='{_fixAppostrophe(country)}';"
    result = None
    try:
        cursor.execute(sqlCommand)
        result = cursor.fetchone()
...
def get_diamonds(
    ...
    if store_city == None and store_country == None:
        if required_certificates == None: #if no filter was given
        ...
        storeIDs = _getAllMatchingStoreIDs(cursor, store_city, store_country)
        sqlCommand = """SELECT * FROM diamonds WHERE ("""
        for store in storeIDs: #if only country was provided, multiple stores are returned
...
def _getAllMatchingStoreIDs(cursor,city,country) -> List[int]:
    ...
    if city != None:
        sqlCommand += f"city='{_fixAppostrophe(city)}'"
    if city != None and country != None:
        sqlCommand += " AND "
    if country != None:
        sqlCommand += f"country='{_fixAppostrophe(country)}'"
    sqlCommand+=";"
    try:
        cursor.execute(sqlCommand)
        results = cursor.fetchall()
...
def _fixAppostrophe(word: str) -> str:
    if "'" in word:
        parts = word.split("'")
...
#configure('localhost',3306,'diamonds','diamonds_user','myuserpassword')
#print(add_diamond(0.23,'Ideal','J','SI2',61.5,55.0,326,3.95,3.98,2.43,['IGI'],'Palermo','Italy'))
#delete_all()
#batch_import('https://datarevenue-public.s3.eu-central-1.amazonaws.com/code-challenge/diamonds/dr_diamonds_challenge.csv')
#print(get_diamonds(required_certificates=['IGI','GIA'])[1]['id'])
