...
def validate(model, val_index_map, validation_loader, device, system_meter, conf_thres, nms_iou_threshold,
             tile_predictions_nms_thresh, tiling_merge_predictions_time, tiling_nms_time, print_freq=100):
    ...
    for epoch in range(number_of_epochs):
        ...
        if epoch % eval_freq == 0 or final_epoch:
            eval_bounding_boxes = validate(model=ema.ema, val_index_map=val_index_map,
                                           validation_loader=validation_loader, device=device,
