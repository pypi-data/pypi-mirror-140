...
from azureml.automl.dnn.vision.object_detection.eval.incremental_voc_evaluator import IncrementalVocEvaluator, \
    _calculate_pr_metrics, _match_boxes
...
def _make_random_objects(width, height, num_classes, num_boxes, is_ground_truth):
    ...
    labels = np.random.randint(num_classes, size=(len(boxes), 1))
    if is_ground_truth:
        return np.concatenate((boxes, labels), axis=1)

    scores = np.random.uniform(size=(len(boxes), 1))
    return np.concatenate((boxes, labels, scores), axis=1)


def xyxy2xywh(np_box):
    return [
        float(np_box[0]), float(np_box[1]),
        float(np_box[2]) - float(np_box[0]), float(np_box[3]) - float(np_box[1])
    ]
...
class TestIncrementalVocEvaluator:
    ...
    def test_match_boxes(self):
        ...
        tp_fp_labels = _match_boxes(gt_boxes, is_crowd, predicted_boxes, predicted_scores, 0.5)
    ...
    def test_single_image_no_gt_no_pred(self):
        ive = IncrementalVocEvaluator(3, 0.5)

        meta_info_per_image = [{"width": 640, "height": 480, "iscrowd": np.zeros((0, 1), dtype=bool)}]
        gt_objects_per_image, predicted_objects_per_image = [np.zeros((0, 5))], [np.zeros((0, 6))]
    ...
    def test_single_image_no_gt_one_pred(self):
        ...
        meta_info_per_image = [{"width": 640, "height": 480, "iscrowd": np.zeros((0, 1), dtype=bool)}]
        gt_objects_per_image = [np.zeros((0, 5))]
        predicted_objects_per_image = [
            np.array([
                [160, 120, 320, 240, 1, 0.75],
            ])
        ]
        ive.evaluate_batch(gt_objects_per_image, predicted_objects_per_image, meta_info_per_image)
    ...
    def test_single_image_one_gt_one_pred_sufficient_overlap(self):
        ...
        ive = IncrementalVocEvaluator(3, 0.25)
    ...
    def test_single_image_two_gt_two_pred_good_overlap(self):
        ...
        predicted_objects_per_image = [
            np.array([
                [160, 120, 300, 220, 0, 0.999],
                [320, 240, 460, 340, 0, 0.888],
            ])
        ]
        ive.evaluate_batch(gt_objects_per_image, predicted_objects_per_image, meta_info_per_image)
    ...
    def test_single_image_two_gt_two_pred_one_match(self):
        ...
        pred_objects_per_image = [
            np.array([
                [100, 100, 200, 200, 2, 0.75],
                [135, 100, 210, 200, 2, 0.75],
            ])
        ]
    ...
    def test_single_image_1K_gt_1K_pred_random(self):
        ive = IncrementalVocEvaluator(10, 0.5)
        ...
        classes = np.random.randint(0, 10, size=(1000, 1))
        gt_objects_per_image = [np.concatenate((boxes, classes), axis=1)]
        ...
        boxes = np.concatenate((x1, y1, x2, y2), axis=1)
        classes = np.random.randint(0, 10, size=(1000, 1))
        scores = np.random.uniform(size=(1000, 1))
        pred_objects_per_image = [np.concatenate((boxes, classes, scores), axis=1)]
    ...
    def test_multi_image_one_gt_one_pred_good_overlap(self):
        ...
        predicted_objects_per_image = [
            # first image
            np.array([
                [160, 120, 300, 220, 0, 0.991],
            ]),
            # second image
            np.array([
                [320, 240, 600, 440, 1, 0.995],
            ]),
        ]
        ive.evaluate_batch(gt_objects_per_image, predicted_objects_per_image, meta_info_per_image)
    ...
    def test_multi_image_one_gt_one_pred_different_class(self):
        ...
        predicted_objects_per_image = [
            # first image
            np.array([
                [160, 120, 300, 220, 1, 0.991],
            ]),
            # second image
            np.array([
                [320, 240, 600, 440, 2, 0.995],
            ]),
        ]
        ive.evaluate_batch(gt_objects_per_image, predicted_objects_per_image, meta_info_per_image)
    ...
    def test_compare_random_against_vocmap(self, iou_threshold):
        ...
        ive = IncrementalVocEvaluator(num_classes, iou_threshold)
        ...
        for class_index, class_str in enumerate(class_strs):
            vocmap._label_gts[class_str] = [
                self._create_annotation(
                    image_ids[i], ground_truth_object[:4], class_strs[int(ground_truth_object[4])], False
                )
                for i in range(num_images)
                for ground_truth_object in gt_objects_per_image[i] if ground_truth_object[4] == class_index
            ]
        # b. make the predictions
        vocmap_predictions = [
            self._create_prediction(
                image_ids[i], predicted_object[:4], class_strs[int(predicted_object[4])], predicted_object[5]
            )
            for i in range(num_images)
            for predicted_object in predicted_objects_per_image[i]
        ]
        # c. run calculations
