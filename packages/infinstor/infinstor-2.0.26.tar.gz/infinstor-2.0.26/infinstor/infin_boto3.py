...
class InfinBotoClient():
    ...
    def list_objects_v2(self, *args, **kwargs):
        print('list_objects_v2: Entered. args=' + str(args) + ", kwargs=" + str(kwargs))
        if self.timespec_info['type'] == 'infinsnap' or self.timespec_info['type'] == 'infinslice':
            if self.bucket:
                request_bucket = self.bucket
                request_prefix = self.prefix
    ...
    def download_file(self, *args, **kwargs):
        if self.timespec_info['type'] == 'infinsnap' or self.timespec_info['type'] == 'infinslice'\
            ...
            request_file = args[1]
            print("download_file: request_bucket=" + request_bucket\
                    + ", request_file=" + request_file\
                    + ", ts.bucketlist=" + str(self.timespec_info['bucketlist']))
            if request_bucket in self.timespec_info['bucketlist']:
                ...
                if (versionId != None):
                    if self.timespec_info['use_presigned_url_for_infinsnap']:
                        print("Using presigned url for download_file", flush=True)
                        urlretrieve(presignedUrl, args[2])
                        return
                    else:
                        print("Using versionid for download_file", flush=True)
