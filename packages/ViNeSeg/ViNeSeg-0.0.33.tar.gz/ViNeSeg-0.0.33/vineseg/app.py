...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        startCASCADE = action(
            self.tr("CASCADE Spike Detection"),
            self.start_CASCADE,
            icon="curveBin",
            tip=self.tr("Start CASCADE Spike Detection"),
        )
    ...
    def autosegmentation(self):
        ...
        if self.imagePath != None:
            ...
            if self.imagePath.endswith((".png", ".tiff", "tif")):
                ...
                pred_main(im_path, model=self.currentModel, minimal_area_neuron_in_pixels=minArea, maximal_area_neuron_in_pixels=maxArea)
...
#            self.frameRate = frameRate
    ...
    def start_trace_extraction(self):
        ...
        if c == mb.No:
            return
        # print('found %s images!', len(imagesArray))
        """
        we dont need the frequency
        # ask user for frequency
        # todo
        dia = QtWidgets.QInputDialog
        freq, ok = dia.getDouble(self, "Frequency of the recording",
        ...
        # extract
        self.extractTraces(imagesArray, freq)
        """
    ...
    def extractTraces(self, imagesArray):  # , frequency):
        ...
        with Pool() as pool:
            ...
            #todo: check if traces are in correct order or got mixed up in multiprocessing process
        ...
        dff_calc(self.filename.replace(".json", "_traces.tsv"))
        return
