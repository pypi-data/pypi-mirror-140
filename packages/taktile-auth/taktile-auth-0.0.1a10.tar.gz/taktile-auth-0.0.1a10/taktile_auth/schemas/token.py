...
class TaktileIdToken(BaseModel):
    ...
    def _is_allowed(self, permission_str: str, roles: t.List[Role]):
    ...
    def assert_access(self, permission: t.Union[str, t.Sequence[str]]):
        if isinstance(permission, str):
            self._is_allowed(permission, self.auth_roles)
        else:
            for p in permission:
