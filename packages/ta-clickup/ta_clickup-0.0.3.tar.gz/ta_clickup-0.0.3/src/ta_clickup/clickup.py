...
class ClickUp:
    ...
    def login(self, email, password):
        ...
        response = requests.request(method="GET", url=self.server + uri, data=data).json()
    ...
    def send_request(self, method="GET", uri=None, version="v1", **kwargs):
        ...
        response = requests.request(method=method, url=self.server + uri, headers=headers, **kwargs)
    ...
    def get_tasks_by_team(self, team_id, space_id=None, include_closed="true", version="v1"):
        ...
        if version == "v1":
            uri = "api/{}/team/{}/task?include_closed={}".format(version, team_id, include_closed)
            if space_id:
    ...
    def get_task_ids(self, team_id, project_id, category_id, show_all=False):
        ...
        uri = "v2/taskId?team_id={}&project_ids%5B%5D={}&category_ids%5B%5D={}".format(team_id, project_id, category_id)
        if not show_all:
    ...
    def create_task(self, subcategory, name, timestamp, estimate=None):
        ...
        if estimate:
            ...
            uri = "v1/task/{}".format(task_id)
            response = self.send_request("PUT", uri=uri, version="v2", data=estimate_time)
        return response
    ...
    def change_task_status(self, task_id, status, **params):
        """Change task status

         Possible status {str}:
         -autoflow
         -ready for assignment
         -ready for ta download
         -ready for signature pages
         -associate prep

        Arguments:
        task_id {str}
        status {str} - future status for the task

        KeywordArguments:
        mobile {bool}
        resolve_all_items {bool}
        check_resolved {bool}
        check_dependencies {bool}

        """
        uri = f"v1/task/{task_id}"
        data = {"status": status}
        response = self.send_request("PUT", uri=uri, version="v2", params=params, data=data)
