  ...
  describe('getRenderedDiff', function () {
    it('Without interdiffs', function () {
      ...
      spyOn($, 'ajax').and.callFake(function (request) {
        ...
        expect(request.url).toBe('/r/1/diff/2/fragment/3/?index=4&' + TEMPLATE_SERIAL);
        request.success('abc');
    ...
    it('With interdiffs', function () {
      ...
      spyOn($, 'ajax').and.callFake(function (request) {
        expect(request.type).toBe('GET');
        expect(request.url).toBe('/r/1/diff/2-3/fragment/3/?index=4&' + TEMPLATE_SERIAL);
        request.success('abc');
        request.complete('abc', 'success');
      });
      diffReviewable.getRenderedDiff(callbacks);
    ...
    it('With base FileDiff', function () {
      ...
      spyOn($, 'ajax').and.callFake(function (request) {
        expect(request.type).toBe('GET');
        expect(request.url).toBe('/r/1/diff/2/fragment/3/?base-filediff-id=1&index=4&' + TEMPLATE_SERIAL);
        request.success('abc');
        request.complete('abc', 'success');
      });
      diffReviewable.getRenderedDiff(callbacks);
