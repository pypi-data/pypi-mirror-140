...
def _tables_or_sql(replacement: dict, table_functions=False) -> set:
    try:
...
class InvalidResource(ValueError):
    def __init__(self, database, table):
        ...
        super().__init__(f"Invalid resource '{self.msg}'")
...
def replace_tables(sql: str, replacements: dict, default_database: str = '', check_functions: bool = False):
    ...
    while _tables:
        table = _tables.pop()
        if len(table) == 3:
            if table[2] and table[2] not in ENABLED_TABLE_FUNCTIONS and not(table[2] == 'remote' and allow_remote):
        ...
        if table in _replacements:
            ...
            dependent_tables = _tables_or_sql(replacement, table_functions=check_functions)
            deps[table] |= {(d[0], d[1]) for d in dependent_tables}
            for dependent_table in list(dependent_tables):
                if len(dependent_table) == 3:
                    if dependent_table[2] and dependent_table[2] not in ENABLED_TABLE_FUNCTIONS and not(dependent_table[2] == 'remote' and allow_remote):
    ...
    for current_deps in deps_sorted:
        ...
        for r in current_deps:
            if r in _replacements:
                ...
                if (table_name and default_database != '' and database != default_database and r not in _replaced_with):
